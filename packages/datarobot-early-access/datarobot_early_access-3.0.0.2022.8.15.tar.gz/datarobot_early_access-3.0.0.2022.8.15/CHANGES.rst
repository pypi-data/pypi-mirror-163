...
New Features
************
- The Python client no longer supports Python 3.6 and earlier versions. All future versions of the Python SDK will support Python 3.7 and newer versions.

- Added support for calculating and retrieving feature impact for backtests other than zero and holdout for datetime aware models. :meth:`DatetimeModel.get_feature_impact <datarobot.models.DatetimeModel.get_feature_impact>`,
  :meth:`DatetimeModel.request_feature_impact <datarobot.models.DatetimeModel.request_feature_impact>` and
  :meth:`DatetimeModel.get_or_request_feature_impact <datarobot.models.DatetimeModel.get_or_request_feature_impact>`.

- Added `backtest` field to feature impact metadata :meth:`Model.get_or_request_feature_impact <datarobot.models.Model.get_feature_impact>`. This field is null for not datetime aware models and greater or equal than zero or 'holdout' for datetime aware models.

- Added experimental support for retrieving document thumbnails :class:`DocumentThumbnail <datarobot._experimental.models.documentai.document.DocumentThumbnail>` and :class:`DocumentPageFile <datarobot._experimental.models.documentai.document.DocumentPageFile>`

- Added a new method for getting the canonical URI for a class :meth:`Project.get_uri <datarobot.models.Project.get_uri>`, :meth:`Model.get_uri <datarobot.models.Model.get_uri>`, :meth:`Deployment.get_uri <datarobot.models.Deployment.get_uri>` & :meth:`Dataset.get_uri <datarobot.models.Dataset.get_uri>`

- Added a new method for opening a class in a browser per their URI :meth:`Project.get_uri <datarobot.models.Project.open_in_browser>`, :meth:`Model.open_in_browser <datarobot.models.Model.open_in_browser>`, :meth:`Deployment.open_in_browser <datarobot.models.Deployment.open_in_browser>` & :meth:`Dataset.open_in_browser <datarobot.models.Dataset.open_in_browser>`

- Altered method :meth:`Project.create_featurelist <datarobot.models.Project.create_featurelist>` to accept five new parameters: ``starting_featurelist``, ``starting_featurelist_id``, ``starting_featurelist_name``, ``features_to_include``, ``features_to_exclude`` - please see docs for usage

- Added a new method :meth:`Project.get_featurelist_by_name <datarobot.models.Project.get_featurelist_by_name>`

- Added a new convenience method to create Datasets :meth:`Dataset.upload <datarobot.models.Dataset.upload>`

- Altered method :meth:`Model.request_predictions <datarobot.models.Model.request_predictions>` to accept four new parameters ``dataset``, ``file``, ``file_path`` and ``dataframe`` - these are in addition to the existing parameter ``dataset_id`` - all data source parameters are mutually exclusive

- Added experimental support for retrieving document text extraction samples using: :class:`DocumentTextExtractionSample <datarobot._experimental.models.documentai.document.DocumentTextExtractionSample>`,
  :class:`DocumentTextExtractionSamplePage <datarobot._experimental.models.documentai.document.DocumentTextExtractionSamplePage>` and :class:`DocumentTextExtractionSampleDocument <datarobot._experimental.models.documentai.document.DocumentTextExtractionSampleDocument>`
...
- Added a new method to :class:`datarobot.models.Dataset` :meth:`Dataset.share <datarobot.models.Dataset.share>` which allows sharing of a Dataset with another user.

- Added new convenience methods to :class:`datarobot.models.Project` for dealing with Partition classes. Both should be called before :meth:`Project.set_target <datarobot.models.Project.set_target>`.
  - :meth:`Project.set_partitioning_method <datarobot.models.Project.set_partitioning_method>` will intelligently create the right Partition class for a regular project, based on input arguments.
  - :meth:`Project.set_datetime_partitioning <datarobot.models.Project.set_datetime_partitioning>` will create the right Partition class for a time series project.

- Added new method to :class:`datarobot.models.Project` :meth:`Project.get_top_model <datarobot.models.Project.get_top_model>` which returns highest scoring model for a metric of the user's choice.
...
- Added a new method to :class:`RetrainingPolicy <datarobot._experimental.models.retraining.RetrainingPolicy>`
  - :meth:`RetrainingPolicy.get <datarobot._experimental.models.retraining.RetrainingPolicy.get>` to get a retraining policy associated with a deployment

- Added new convenience method to :class:`datarobot.models.Deployment` :meth:`Deployment.predict_batch <datarobot.models.Deployment.predict_batch>` which allows passing a file, file path or dataframe to easily make batch predictions and return the results as a DataFrame.
...
- Added support for creating monotonic featurelists for Time Series projects. Set ``skip_datetime_partition_column`` to
  True to create monotonic featurelist. See :meth:`datarobot.models.Project.create_modeling_featurelist`
- Added information about vertex to advanced tuning parameters :meth:`datarobot.models.Model.get_advanced_tuning_parameters`

Bugfixes
********
- Fixed an issue with creation of BatchPredictionJobDefinitions containing a schedule
- Fixed error handling in ``datarobot.helpers.partitioning_methods.get_class``

API Changes
***********
- The Python client will now output a `DataRobotProjectDeprecationWarning` when you attempt to access certain resources (projects, models, deployments, etc.) that are deprecated or disabled by the DataRobot platform migration to Python 3.
- The Python client will now raise a `TypeError` when you try to retrieve labelwise ROC on a binary model or binary ROC on a multilabel model.
...
- :class:`Project <datarobot.models.Project>`'s ``scaleout_max_train_pct`` & ``scaleout_max_train_rows`` parameters have been removed.
- ``ComplianceDocumentation`` has been removed.
  Please use :class:`AutomatedDocument <datarobot.models.automated_documentation.AutomatedDocument>` instead.
...
- ``PredictJob.create`` has been removed. Please use :meth:`Model.request_predictions <datarobot.models.Model.request_predictions>` instead.
- ``Model.fetch_resource_data`` has been removed. Please use :meth:`Model.get <datarobot.models.Model.get>` instead.
- The class ``CustomInferenceImage`` was removed. Please use :class:`CustomModelVersion <datarobot.CustomModelVersion>` with ``base_environment_id`` instead.


Configuration Changes
*********************
- Added a context manager :meth:`client_configuration <datarobot.client.client_configuration>` that can be used to change the connection configuration
  temporarily, for use in asynchronous or multithreaded code.
