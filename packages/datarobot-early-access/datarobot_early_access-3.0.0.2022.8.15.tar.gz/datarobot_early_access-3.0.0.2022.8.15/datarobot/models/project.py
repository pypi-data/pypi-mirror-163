...
from datarobot.utils import assert_single_or_zero_parameter
...
from .job import Job
from .model import BiasMitigatedModelInfo, BiasMitigationFeatureInfo, CombinedModel, Model
...
class Project(APIObject, BrowserMixin):
    ...
    partitioning_method : PartitioningMethod, optional
        ...
        with newly-created projects and before calling `Project.set_target()`. After the project has been
    ...
    def _load_advanced_options(self, opts, payload):
        if opts is None:
    ...
    @classmethod
    def get(cls, project_id) -> Project:
        """
    ...
    def start(
        ...
        .. note:: While this function provides a simple means to get started, it does not expose
            ...
            and ``set_target`` directly is recommended.
    ...
    def set_target(
        ...
        if advanced_options is not None:
            self.set_advanced_options(advanced_options=advanced_options)
        self._load_advanced_options(self.advanced_options, aim_payload)
        ...
        if partitioning_method is not None:
            if self.partitioning_method is not None:
                warnings.warn(
                    "The `partitioning_method` passed directly into `set_target` will be used, but \
                        ...
                        partitioning method directly to `set_target`.",
                    PartitioningMethodWarning,
                )
            self.partitioning_method = partitioning_method
        if self.partitioning_method is not None:
    ...
    def get_top_model(self, metric: Optional[str] = None) -> Optional[Model]:
        """Obtain DR recommended model for a given metric.
        If no metric is passed in, it returns the recommended model for the project's default metric.
    ...
    Please try one of the following instead: {all_metrics_used_on_lb}."
                )
            ...
            is_ascending = next(
                m["ascending"] for m in metric_details if m["metric_name"] == metric
            )
            ...
            if sorted_filtered_scores_and_models:
                ...
                raise ValueError(
                    "Unable to retrieve models and their scores for this metric. \
    Please choose a different metric."
                )
    ...
    def restore_discarded_features(self, features: List[str], max_wait: int = DEFAULT_MAX_WAIT):
    ...
    def set_partitioning_method(
        ...
        if partitioning_method is not None:
            ...
            if cv_method is None or validation_type is None:
                raise TypeError(
    ...
    def set_datetime_partitioning(
        ...
        **kwargs,
        ...
        :meth:`Project.set_target <datarobot.models.Project.set_target>` via the
    ...
    def list_datetime_partition_spec(self) -> Optional[Dict]:
        ...
        stage, i.e. if `set_target` (autopilot) has already been called.

        If `set_target` has not yet been called, this method will instead simply print
    ...
    def set_feature_settings(
        self, feature_settings: List[FeatureSettings]
    ) -> List[FeatureSettings]:
        """Set time series feature settings.

        This method makes an API call to save feature settings for use with time series modeling.

        .. versionadded:: v3.0

        Returns
        -------
        list of all FeatureSettings objects on a project
        """
        project_options = ProjectOptions.get(project_id=self.id)
        if not project_options.target:
            raise ValueError(
                "Feature settings can not be set until a target has been selected, and EDA1 and 2 have finished."
            )
        existing_feature_settings = project_options.feature_settings
        if existing_feature_settings:
            new_features = [feature.feature_name for feature in feature_settings]
            unique_feature_settings = [
                feature
                for feature in existing_feature_settings
                if feature.feature_name not in new_features
            ]
            feature_settings.extend(unique_feature_settings)
        project_options.update_individual_options(feature_settings=feature_settings)
        project_options = project_options.update_options()
        return project_options.feature_settings
