...
try:
    # Literal is only available from typing in python 3.8+. For 3.7 we need to use typing_extensions
...
class ProjectOptions(AdvancedOptions, APIObject):
    ...
    quickrun : boolean, optional
        Whether modeling will use quickrun
    ...
    _converter = t.Dict(
        {
            ...
            t.Key("feature_engineering_options", optional=True): t.Null()
            | t.Dict({})
            | FeatureEngineeringOptions._converter,
            ...
            t.Key("holdout_duration", optional=True): t.Null() | String() | Duration._converter,
        ...
        **kwargs,
        ...
        if feature_settings and isinstance(feature_settings[0], dict):
            self.feature_settings = [
                FeatureSettings(**feature_setting) for feature_setting in feature_settings
            ]
        else:
    ...
    @classmethod
    def get(cls, project_id: str) -> ProjectOptions:
