...
def evaluate_report(report, question=None, qitem=None, passall=False, verbose=False,  show_expected=False, show_computed=False,unmute=False, show_help_flag=True, silent=False,
    ...
    for n, (q, w) in enumerate(report.questions):
        ...
        for s, msg in res.successes + res.failures + res.errors:
            ...
            # print(s)
            from unittest.suite import _ErrorHolder
            # print("Finding out the title of s")
            # print("s is", s)
            if isinstance(s, _ErrorHolder):
                pass
                # print("It is an error holdeR")
            # print(s.__dict__.keys())
            # print(s.description)

            """ The next line is in case s is an instance of unittest.suite._ErrorHolder which have no title property.
            I think this happens when errors occur during the setUpClass run. """
            nice_title = s.title if hasattr(s, 'title') else s.id()
