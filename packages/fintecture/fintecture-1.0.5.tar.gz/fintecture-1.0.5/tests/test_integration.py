...
class TestIntegration(object):
    ...
    def setup_fintecture(self):
        ...
        fintecture.proxy = orig_attrs["proxy"]
    ...
    def test_hits_api_base(self):
        ...
        fintecture.api_base = "http://localhost:%s" % self.mock_server_port
        fintecture.Balance.retrieve()
    ...
    def test_hits_proxy_through_default_http_client(self):
        ...
        fintecture.proxy = "http://localhost:%s" % self.mock_server_port
        fintecture.Balance.retrieve()
        assert MockServerRequestHandler.num_requests == 1

        fintecture.proxy = "http://bad-url"

        with warnings.catch_warnings(record=True) as w:
            fintecture.Balance.retrieve()
            assert len(w) == 1
            assert "fintecture.proxy was updated after sending a request" in str(
                w[0].message
            )
    ...
    def test_hits_proxy_through_custom_client(self):
        ...
        fintecture.default_http_client = (
            fintecture.http_client.new_default_http_client(
    ...
    def test_passes_client_telemetry_when_enabled(self):
                ...
                try:
                    ...
                    if req_num == 1:
                        ...
                        assert not self.headers.get(
                            "X-Fintecture-Client-Telemetry"
                        )
                    elif req_num == 2:
                        assert self.headers.get("X-Fintecture-Client-Telemetry")
                    ...
                    # json so it can be logged as a FintectureError.
        ...
        self.setup_mock_server(MockServerRequestHandler)
        fintecture.api_base = "http://localhost:%s" % self.mock_server_port
        fintecture.enable_telemetry = True
    ...
    def test_uses_thread_local_client_telemetry(self):
        class MockServerRequestHandler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if self.headers.get("X-Fintecture-Client-Telemetry"):
                    telemetry = json.loads(
                        self.headers.get("X-Fintecture-Client-Telemetry")
                    )
        ...
        fintecture.api_base = "http://localhost:%s" % self.mock_server_port
        fintecture.enable_telemetry = True
        fintecture.default_http_client = fintecture.http_client.RequestsClient()
