...
def _table_rowify(slug,counter,moment,contents,now):
    if isinstance(contents,list):
        contents = _olify(contents)
    style = ''
    if now < moment:
        style = 'style="background-color: lightyellow"'
    return f'<tr {style}><td>{slug}</td><td style="text-align: center">{counter}</td><td>{moment}</td><td>{contents}</td></tr>'
...
def tabulate(yamlfile, chronologically, format):
    ...
    with webuntis.Session(
        ...
        if format == 'html':
            ...
            print('<!DOCTYPE html><head></head><body><table><thead>'
            ...
            for (moment,slug,contents) in moment_slug_contents_tuples:
                ...
                print(_table_rowify(slug,slug_counters[slug],moment,contents,now))
