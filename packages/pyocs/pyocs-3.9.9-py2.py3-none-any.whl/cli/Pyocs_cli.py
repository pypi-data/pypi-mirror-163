    ...
    if cps:
        ...
        if cps_sw_msg_list:
            for ocs_number in ocs_list:
                        ...
                        first_sign = True
                        burn_place_hold_type, burn_type = get_burn_info_with_upload_cps(ocs_number)
...
def ocs_get_link(boardcode):
    ...
    board_code_List = database.get_value_of_key("pyocs_factory_code","code", boardcode[0])[0]

    if len(board_code_List) == 0:
        print("找不到对应的工厂代码，请确认条码输入是否有误!!")
        return
    else:
        split_str = board_code_List.split(',')
