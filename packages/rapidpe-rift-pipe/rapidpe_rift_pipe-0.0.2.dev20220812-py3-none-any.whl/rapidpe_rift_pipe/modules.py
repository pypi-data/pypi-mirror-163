...
#    string_of_dict = json.dumps(dict(cfg.items(section)))
    ...
    for opt,arg in list(dictin.items()):
#        print opt,arg
...
#    string_of_dict = json.dumps(dict(cfg.items(section)))
    ...
    for opt,arg in dictin.items():
        dictin[opt] = correct_list_string_formatting_if_list_string(arg)
...
def convert_cfg_section_to_cmd_line(cfg,cfg_section):
    ...
    cmd_line = ""
    for name,arg in list(cmds_dict.items()):
...
def correct_list_string_formatting_if_list_string(list_string_or_other):
    if not isinstance(list_string_or_other, str):
        return list_string_or_other

    if list_string_or_other[0] != "[":
        return list_string_or_other

    if list_string_or_other[-1] != "]":
        return list_string_or_other

    return correct_list_string_formatting(list_string_or_other)
...
def convert_injections_txt_to_objects(inj_file):
    ...
    inj = np.genfromtxt(inj_file,names=True)
...
class Injection:
    def __init__(self,e,ei):
        ...
        self.psi0 = e["psi0"][ei]
        self.phi0 = e["phi0"][ei]
        if "geocent_end_time" in e.dtype.names:
