...
class OWPythonScript(widget.OWBaseWidget, openclass=True):
    ...
    class Outputs:
        ...
        future_data = Output(
            "future_data", FutureTomwerScan, replaces=["out_future_data"]
        )
        cluster_config = Output(
            "cluster_config", SlurmClusterConfiguration, replaces=["out_cluster_config"]
        )

    signal_names = ("data", "future_data", "object", "configuration", "cluster_config")
    ...
    @Inputs.future_data  # noqa F811
    def set_object(self, data, id):
        self.handle_input(data, id, "future_data")
