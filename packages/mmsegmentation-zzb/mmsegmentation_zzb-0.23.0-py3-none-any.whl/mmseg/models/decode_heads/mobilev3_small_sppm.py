...
class MobilenetSmallASPPHead(BaseDecodeHead):
    def __init__(self,dilations=(2, 4),**kwargs):
        super(MobilenetSmallASPPHead, self).__init__(**kwargs)
        self.dilations = dilations
        self.last_conv = nn.Conv2d(in_channels=512,out_channels=96,kernel_size=1,stride=1)
        self.penultimate = nn.Conv2d(in_channels=192,out_channels=40,kernel_size=1,stride=1)
        self.bottom_third = nn.Conv2d(in_channels=80,out_channels=16,kernel_size=1,stride=1)

        self.last_liteaspp = LiteASPPHead(dilations, in_channels=576, channels=512, num_classes=21)
        self.penultimate_liteaspp = LiteASPPHead(dilations,in_channels=96,channels=96,num_classes=21)
        self.bottom_third_liteaspp = LiteASPPHead(dilations,in_channels=40,channels=40,num_classes=21)
