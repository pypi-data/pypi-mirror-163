...
from ..utils import MyInvertedResidualV3 as InvertedResidual


@BACKBONES.register_module()
class MyMobileNetV3Layer16(BaseModule):
    ...
    arch_settings = {
        'small': [[3, 16, 16, True, 'ReLU', 1],  # block0 layer1 os=2
                  [3, 72, 24, False, 'ReLU', 2],  # block1 layer2 os=8
                  [3, 88, 24, True, 'ReLU', 1],
                  [3, 96, 40, True, 'HSwish', 2],  # block2 layer4 os=16
                  [3, 240, 40, True, 'HSwish', 1],
                  [3, 240, 40, True, 'HSwish', 1],
                  # [5, 120, 48, True, 'HSwish', 1],  # block3 layer7 os=16
                  # [5, 144, 48, True, 'HSwish', 1],
                  [3, 288, 96, True, 'HSwish', 2],  # block4 layer9 os=32
                  [3, 288, 96, True, 'HSwish', 1],
                  [3, 288, 96, True, 'HSwish', 1],
                  [3, 576, 112, True, 'HSwish', 2],
                  [3, 576, 112, True, 'HSwish', 1],
                  [3, 576, 112, True, 'HSwish', 1]],
        'large': [[3, 16, 16, False, 'ReLU', 1],  # block0 layer1 os=2
                  [3, 64, 24, False, 'ReLU', 2],  # block1 layer2 os=4
                  [3, 72, 24, False, 'ReLU', 1],
                  [3, 72, 40, True, 'ReLU', 2],  # block2 layer4 os=8
                  [3, 120, 40, True, 'ReLU', 1],
                  [3, 120, 40, True, 'ReLU', 1],
                  ...
                  [3, 672, 160, True, 'HSwish', 2],  # block5 layer13 os=32
                  [3, 960, 160, True, 'HSwish', 1],
                  [3, 960, 160, True, 'HSwish', 1]]
    }  # yapf: disable
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MyMobileNetV3Layer16, self).__init__(init_cfg)
        ...
        for index in out_indices:
            if index not in range(0, len(self.arch_settings[arch]) + 2):#insure output index in arch_setting[arch] lenth
                raise ValueError(
    ...
    def _make_layer(self):
        ...
        layer_setting = self.arch_settings[self.arch]#get every setting of arch(small or large)
        for i, params in enumerate(layer_setting):#enumerate mean listing index and data
            (kernel_size, mid_channels, out_channels, with_se, act,
             stride) = params

            if self.arch == 'large' and i >= 9 or self.arch == 'small' and \
                    i >= 8:
        ...
        #     self.layer7.depthwise_conv.conv.stride = (1, 1)
        #     self.layer10.depthwise_conv.conv.stride = (1, 1)
    ...
    def train(self, mode=True):
        super(MyMobileNetV3Layer16, self).train(mode)
        self._freeze_stages()
