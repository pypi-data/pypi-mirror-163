...
path = str(Path(__file__).resolve().parent) + '\config.ini'
config = configparser.ConfigParser()
config.read(path, encoding='utf-8-sig')
private_key = open(os.path.join(config.get("xj_payment", 'APP_PRIVATE_KEY_FILE'))).read()
public_key = open(os.path.join(config.get("xj_payment", 'ALIPAY_PUBLIC_KEY_FILE'))).read()
app_id = config.get("xj_payment", 'ALIPAY_APP_ID')
print("<app_id>", app_id)
