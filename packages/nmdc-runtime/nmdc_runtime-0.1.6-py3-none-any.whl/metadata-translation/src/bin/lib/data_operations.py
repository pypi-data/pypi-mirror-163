...
def make_dataframe(
    ...
    if "tsv" == file_type.lower() or "csv" == file_type.lower():
        if None != file_archive:
            df = pds.read_csv(
                file_archive.open(file_name), sep=delimiter, nrows=nrows, comment="#"
            )
        else:
        ...
        if None != file_archive:
            df = pds.read_excel(
                file_archive.open(file_name), sheet_name=sheet_name, nrows=nrows
            )
        else:
        ...
        if None != file_archive:
            ...
            df = pds.concat(
                pds.read_excel(
                    file_name, sheet_name=None, ignore_index=True, nrows=nrows
                )
            )
...
def make_nmdc_dict_list(
    ...
    def map_slot_to_entity(slot_map, record):
        ...
        if type(()) != type(slot_value):
            ...
            if pds.isnull(record[slot_value]):
                return None
        ...
        if type(()) == type(slot_value):
            ...
            if slot_name in ["part_of", "has_input", "has_output"]:
                ...
                referenced_entity = [
                    nmdc_class(**{"id": id_val}) for id_val in id_values
                ]
    ...
    if add_attribute:
        for af in attribute_fields:
            if type({}) == type(af):
                af = list(af.keys())[0]  # needed for attributes given as a dict
            if not hasattr(nmdc_class, str(af)):
                setattr(nmdc_class, str(af), None)
            ### TODO ! throw a a warning ###

        if len(attribute_map) > 0:
            for af in attribute_map.values():
                if not hasattr(nmdc_class, af):
                    setattr(nmdc_class, af, None)
        ...
        for af in attribute_fields:
            if type({}) == type(af):
                ...
                if "part_of" == slot_name:
                    obj.part_of = map_slot_to_entity(af, record)
                if "has_input" == slot_name:
                    obj.has_input = map_slot_to_entity(af, record)
                if "has_output" == slot_name:
                    obj.has_output = map_slot_to_entity(af, record)

        dict_obj = json.loads(
            jsonasobj.as_json(obj)
        )  # in order to not save empty values you need to convert to json
...
def make_gold_to_mixs_list(source_list, dataframe, table_name):
    ...
    for item in source_list:
        ...
        if type({}) != type(item):
            ...
            target_item = get_mapped_term(
                dataframe,
                database="gold",
                table_name=table_name,
                source_field=item,
                target_field="mixs_term",
            )
...
def get_mapped_term(dataframe, database, table_name, source_field, target_field):
    return_val = dataframe[
        (dataframe["database"].str.lower() == database.lower())
        & (dataframe["table"].str.lower() == table_name.lower())
        & (dataframe["field"].str.lower() == source_field.lower())
    ]

    if len(return_val) > 0:
        return return_val[target_field].values[
            0
        ]  # if more than one match is found, only the first is returned
    else:
        return ""
...
def make_study_dataframe(study_table, contact_table, proposals_table, result_cols=[]):
    ...
    temp1_df = pds.merge(
        study_table.copy(), contact_table_splice, how="left", on="contact_id"
    )
...
def make_project_dataframe(
    ...
    project_table = project_table[
        project_table["omics_type"] != "Whole Genome Sequencing"
    ]
    #########################
...
def make_biosample_dataframe(
    biosample_table, project_biosample_table, project_table, result_cols=[]
):
    ...
    temp1_df = pds.merge(
        biosample_table, project_biosample_table_splice, how="inner", on="biosample_id"
    )
