# This file was automatically generated by SWIG (http://www.swig.org).
# Version 4.0.2
#
# Do not make changes to this file unless you know what you are doing--modify
# the SWIG interface file instead.

from sys import version_info as _swig_python_version_info
if _swig_python_version_info < (2, 7, 0):
    raise RuntimeError("Python 2.7 or later required")

# Import the low-level C/C++ module
if __package__ or "." in __name__:
    from . import _scenefilter
else:
    import _scenefilter

try:
    import builtins as __builtin__
except ImportError:
    import __builtin__

def _swig_repr(self):
    try:
        strthis = "proxy of " + self.this.__repr__()
    except __builtin__.Exception:
        strthis = ""
    return "<%s.%s; %s >" % (self.__class__.__module__, self.__class__.__name__, strthis,)


def _swig_setattr_nondynamic_instance_variable(set):
    def set_instance_attr(self, name, value):
        if name == "thisown":
            self.this.own(value)
        elif name == "this":
            set(self, name, value)
        elif hasattr(self, name) and isinstance(getattr(type(self), name), property):
            set(self, name, value)
        else:
            raise AttributeError("You cannot add instance attributes to %s" % self)
    return set_instance_attr


def _swig_setattr_nondynamic_class_variable(set):
    def set_class_attr(cls, name, value):
        if hasattr(cls, name) and not isinstance(getattr(cls, name), property):
            set(cls, name, value)
        else:
            raise AttributeError("You cannot add class attributes to %s" % cls)
    return set_class_attr


def _swig_add_metaclass(metaclass):
    """Class decorator for adding a metaclass to a SWIG wrapped class - a slimmed down version of six.add_metaclass"""
    def wrapper(cls):
        return metaclass(cls.__name__, cls.__bases__, cls.__dict__.copy())
    return wrapper


class _SwigNonDynamicMeta(type):
    """Meta class to enforce nondynamic attributes (no new attributes) for a class"""
    __setattr__ = _swig_setattr_nondynamic_class_variable(type.__setattr__)


import opencmiss.zinc.graphics
import opencmiss.zinc.field
import opencmiss.zinc.differentialoperator
import opencmiss.zinc.element
import opencmiss.zinc.node
import opencmiss.zinc.fieldassignment
import opencmiss.zinc.fieldcache
import opencmiss.zinc.fieldmodule
import opencmiss.zinc.scenecoordinatesystem
import opencmiss.zinc.timesequence
import opencmiss.zinc.optimisation
import opencmiss.zinc.fieldsmoothing
import opencmiss.zinc.fieldparameters
import opencmiss.zinc.region
import opencmiss.zinc.context
import opencmiss.zinc.font
import opencmiss.zinc.glyph
import opencmiss.zinc.material
import opencmiss.zinc.spectrum
import opencmiss.zinc.light
import opencmiss.zinc.logger
import opencmiss.zinc.sceneviewer
import opencmiss.zinc.scene
import opencmiss.zinc.selection
import opencmiss.zinc.timekeeper
import opencmiss.zinc.timenotifier
import opencmiss.zinc.scenepicker
import opencmiss.zinc.shader
import opencmiss.zinc.streamscene
import opencmiss.zinc.stream
import opencmiss.zinc.tessellation
import opencmiss.zinc.streamregion
import opencmiss.zinc.streamimage
class Scenefilter(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        _scenefilter.Scenefilter_swiginit(self, _scenefilter.new_Scenefilter(*args))
    __swig_destroy__ = _scenefilter.delete_Scenefilter

    def isValid(self) -> "bool":
        return _scenefilter.Scenefilter_isValid(self)

    def getId(self) -> "cmzn_scenefilter_id":
        return _scenefilter.Scenefilter_getId(self)

    def isManaged(self) -> "bool":
        return _scenefilter.Scenefilter_isManaged(self)

    def setManaged(self, value: "bool") -> "int":
        return _scenefilter.Scenefilter_setManaged(self, value)

    def evaluateGraphics(self, graphics: "Graphics") -> "bool":
        return _scenefilter.Scenefilter_evaluateGraphics(self, graphics)

    def isInverse(self) -> "bool":
        return _scenefilter.Scenefilter_isInverse(self)

    def setInverse(self, value: "bool") -> "int":
        return _scenefilter.Scenefilter_setInverse(self, value)

    def getName(self) -> "char *":
        return _scenefilter.Scenefilter_getName(self)

    def setName(self, name: "char const *") -> "int":
        return _scenefilter.Scenefilter_setName(self, name)

    def castOperator(self) -> "OpenCMISS::Zinc::ScenefilterOperator":
        return _scenefilter.Scenefilter_castOperator(self)

    def __eq__(self, other: "Scenefilter") -> "bool":
        return _scenefilter.Scenefilter___eq__(self, other)

# Register Scenefilter in _scenefilter:
_scenefilter.Scenefilter_swigregister(Scenefilter)


def __eq__(*args) -> "bool":
    return _scenefilter.__eq__(*args)
class ScenefilterOperator(Scenefilter):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        _scenefilter.ScenefilterOperator_swiginit(self, _scenefilter.new_ScenefilterOperator(*args))

    def appendOperand(self, operand: "Scenefilter") -> "int":
        return _scenefilter.ScenefilterOperator_appendOperand(self, operand)

    def getFirstOperand(self) -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.ScenefilterOperator_getFirstOperand(self)

    def getNextOperand(self, refOperand: "Scenefilter") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.ScenefilterOperator_getNextOperand(self, refOperand)

    def isOperandActive(self, operand: "Scenefilter") -> "bool":
        return _scenefilter.ScenefilterOperator_isOperandActive(self, operand)

    def setOperandActive(self, operand: "Scenefilter", isActive: "bool") -> "int":
        return _scenefilter.ScenefilterOperator_setOperandActive(self, operand, isActive)

    def insertOperandBefore(self, operand: "Scenefilter", refOperand: "Scenefilter") -> "int":
        return _scenefilter.ScenefilterOperator_insertOperandBefore(self, operand, refOperand)

    def removeOperand(self, operand: "Scenefilter") -> "int":
        return _scenefilter.ScenefilterOperator_removeOperand(self, operand)
    __swig_destroy__ = _scenefilter.delete_ScenefilterOperator

# Register ScenefilterOperator in _scenefilter:
_scenefilter.ScenefilterOperator_swigregister(ScenefilterOperator)

class Scenefiltermodule(object):
    thisown = property(lambda x: x.this.own(), lambda x, v: x.this.own(v), doc="The membership flag")
    __repr__ = _swig_repr

    def __init__(self, *args):
        _scenefilter.Scenefiltermodule_swiginit(self, _scenefilter.new_Scenefiltermodule(*args))
    __swig_destroy__ = _scenefilter.delete_Scenefiltermodule

    def isValid(self) -> "bool":
        return _scenefilter.Scenefiltermodule_isValid(self)

    def getId(self) -> "cmzn_scenefiltermodule_id":
        return _scenefilter.Scenefiltermodule_getId(self)

    def createScenefilterVisibilityFlags(self) -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_createScenefilterVisibilityFlags(self)

    def createScenefilterFieldDomainType(self, domainType: "OpenCMISS::Zinc::Field::DomainType") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_createScenefilterFieldDomainType(self, domainType)

    def createScenefilterGraphicsName(self, matchName: "char const *") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_createScenefilterGraphicsName(self, matchName)

    def createScenefilterGraphicsType(self, graphicsType: "OpenCMISS::Zinc::Graphics::Type") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_createScenefilterGraphicsType(self, graphicsType)

    def createScenefilterRegion(self, matchRegion: "Region") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_createScenefilterRegion(self, matchRegion)

    def createScenefilterOperatorAnd(self) -> "OpenCMISS::Zinc::ScenefilterOperator":
        return _scenefilter.Scenefiltermodule_createScenefilterOperatorAnd(self)

    def createScenefilterOperatorOr(self) -> "OpenCMISS::Zinc::ScenefilterOperator":
        return _scenefilter.Scenefiltermodule_createScenefilterOperatorOr(self)

    def findScenefilterByName(self, name: "char const *") -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_findScenefilterByName(self, name)

    def beginChange(self) -> "int":
        return _scenefilter.Scenefiltermodule_beginChange(self)

    def endChange(self) -> "int":
        return _scenefilter.Scenefiltermodule_endChange(self)

    def getDefaultScenefilter(self) -> "OpenCMISS::Zinc::Scenefilter":
        return _scenefilter.Scenefiltermodule_getDefaultScenefilter(self)

    def setDefaultScenefilter(self, filter: "Scenefilter") -> "int":
        return _scenefilter.Scenefiltermodule_setDefaultScenefilter(self, filter)

# Register Scenefiltermodule in _scenefilter:
_scenefilter.Scenefiltermodule_swigregister(Scenefiltermodule)



