        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if isfunction(after_exception):
                    after_exception(self, current_times=current_times, remark=remark, *args, **kwargs)
                ...
                if max_retry == -1:
                    ...
                    if isfunction(retry_error_callback):
                        # 重试结束，执行回调函数
                        return retry_error_callback(self, retry_times=max_retry, remark=remark, *args, **kwargs)
                    else:
                        # 没有指定回调函数时，弹出错误
                        raise e
                else:
