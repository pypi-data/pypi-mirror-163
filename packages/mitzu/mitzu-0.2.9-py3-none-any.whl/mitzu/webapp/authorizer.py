...
class JWTMitzuAuthorizer(MitzuAuthorizer):
    ...
    def handle_code_redirect(self):
        req_args = flask.request.args.to_dict()
        ...
        print(f"Oauth Token URL: {self.oauth_token_url}")
        resp = requests.post(self.oauth_token_url, params=payload, headers=headers)
        if resp.status_code != 200:
            print(f"Failed token resp: {resp.status_code}, {resp.content}")
        ...
        print(f"Setting cookie resp: {cookie_val}")
        return final_resp
    ...
    def setup_authorizer(self):
        ...
        def authorize_request():
            ...
            if code is not None:
                ...
                print("Unauthorized (missing jwt_token cookie)")
                ...
                try:
                    ...
                    if decoded_token is None:
                        print("Unauthorized (Invalid jwt token)")
                        resp = flask.redirect(code=301, location=self.unauthorized_url)
                    else:
                        print("Authorization finished (caching)")
