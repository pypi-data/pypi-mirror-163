...
class JWTMitzuAuthorizer(MitzuAuthorizer):
    ...
    def handle_code_redirect(self):
        ...
        logging.debug(f"Oauth Token URL: {self.oauth_token_url}")
        resp = requests.post(self.oauth_token_url, params=payload, headers=headers)
        if resp.status_code != 200:
            logging.debug(f"Failed token resp: {resp.status_code}, {resp.content}")
        ...
        logging.debug(f"Setting cookie resp: {cookie_val}")
        return final_resp
    ...
    def setup_authorizer(self):
        ...
        def authorize_request():
            ...
            code = get_oauth_code()
            logging.debug(f"URL: {flask.request.url}")
            logging.debug(f"Headers: {flask.request.headers}")
            logging.debug(f"Args: {flask.request.values}")
            resp: flask.Response
            if code is not None:
                logging.debug(f"Redirected with code= {code}")
                resp = self.handle_code_redirect()
            elif flask.request.url == self.unauthorized_url:
                logging.debug(f"Unauthorized URL: {self.unauthorized_url}")
                resp = None
            elif (
                self.signed_out_url is not None
                and flask.request.url == self.signed_out_url
            ):
                logging.debug(f"Signed out URL: {self.signed_out_url}")
                location = (
                ...
                try:
                    ...
                    if decoded_token is None:
                        logging.debug("Unauthorized (Invalid jwt token)")
                        resp = flask.redirect(code=301, location=self.unauthorized_url)
                    else:
                        logging.debug("Authorization finished (caching)")
                        self.jwt_encoded.set_value(jwt_encoded)
                        self.jwt_token.set_value(decoded_token)
                        resp = None
                except Exception as exc:
                    logging.debug(f"Authorization error: {exc}")
                    resp = flask.redirect(code=301, location=self.unauthorized_url)
