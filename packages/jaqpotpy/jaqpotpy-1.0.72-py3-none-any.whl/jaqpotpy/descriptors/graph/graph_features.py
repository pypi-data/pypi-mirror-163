...
from jaqpotpy.descriptors.graph.mol_graphs import ConvMol, WeaveMol
import logging
...
def one_of_k_encoding(x, allowable_set):
    ...
    -------
    >>> import jaqpotpy as jt
    >>> jt.descriptors.graph.graph_features.one_of_k_encoding("a", ["a", "b", "c"])
    [True, False, False]
    Raises
    ------
...
def one_of_k_encoding_unk(x, allowable_set):
    ...
    Examples
    --------
    >>> jt.descriptors.graph.graph_features.one_of_k_encoding_unk("s", ["a", "b", "c"])
    [False, False, True]
    """
    if x not in allowable_set:
        x = allowable_set[-1]
...
def get_intervals(l):
    ...
    Examples
    --------
    >>> jt.descriptors.graph.graph_features.get_intervals([[1], [1, 2], [1, 2, 3]])
    [1, 3, 12]
    >>> jt.descriptors.graph.graph_features.get_intervals([[1], [], [1, 2], [1, 2, 3]])
    [1, 1, 3, 12]
    """
...
def safe_index(l, e):
    ...
    Examples
    --------
    >>> jt.descriptors.graph.graph_features.safe_index([1, 2, 3], 1)
    0
    >>> jt.descriptors.graph.graph_features.safe_index([1, 2, 3], 7)
    3
    """
    try:
        return l.index(e)
...
def get_feature_list(atom):
    ...
    >>> atom = mol.GetAtoms()[0]
    >>> features = jt.descriptors.graph.graph_features.get_feature_list(atom)
    >>> type(features)
    <class 'list'>
    >>> len(features)
    6
...
def atom_features(atom,
    ...
    >>> atom = mol.GetAtoms()[0]
    >>> features = jt.descriptors.graph.graph_features.atom_features(atom)
    >>> type(features)
    <class 'numpy.ndarray'>
    >>> features.shape
    (75,)
...
class ConvMolFeaturizer(MolecularFeaturizer):
    ...
    >>> import jaqpotpy as jt
    >>> smiles = ["C", "CCC"]
    >>> featurizer=jt.descriptors.ConvMolFeaturizer(per_atom_fragmentation=False)
...
class WeaveFeaturizer(MolecularFeaturizer):
    ...
    >>> import jaqpotpy as jt
    >>> mols = ["CCC"]
    >>> featurizer = jt.descriptors.WeaveFeaturizer()
