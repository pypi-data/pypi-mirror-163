...
@jit
def _julia(c, xbound, ybound, update_func, width=5, height=5,
           dpi=100, maxiter=100, horizon=2.0**40, log_smooth=True):

    """
        function for producing Julia array
    """
...
def julia_series(c_vals, xbound, ybound, update_func, width=5, height=5, 
                 dpi=100, maxiter=100, horizon=2.0**40, log_smooth=True):

    """
        produces a series of Julia arrays (one for each c_val in c_vals),
    ...
    for c in c_vals:
        res = _julia(c, xbound, ybound, update_func, width=width, height=height,
                     dpi=dpi, maxiter=maxiter, horizon=horizon, log_smooth=log_smooth)
        res = fractal_result(*res)
        series.append(res)

    return series


def julia(c, xbound, ybound, update_func, width=5, height=5,
          dpi=100, maxiter=100, horizon=2.0**40, log_smooth=True):
