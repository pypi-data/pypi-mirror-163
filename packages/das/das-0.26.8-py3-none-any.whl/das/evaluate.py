...
from . import predict, data, utils, models, io, predict_oom
...
def evaluate_probabilities(x,
    ...
    y_pred = predict_oom.predict_probabilities(x, model=model, params=params, verbose=verbose, prepend_data_padding=False)
...
def evaluate(savename: str, custom_objects: Optional[Dict[str, Callable]] = None, full_output: bool = True, verbose: int = 1):
    ...
    if len(d['test']['x']) < params['nb_hist']:
        ...
        labels_test = predict_oom.labels_from_probabilities(y_test)
        labels_pred = predict_oom.labels_from_probabilities(y_pred)
