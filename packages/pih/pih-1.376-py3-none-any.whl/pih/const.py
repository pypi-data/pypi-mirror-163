...
LOCAL_COMMAND_LIST_DICT: dict = {
    ...
    #
    "Policy_genarate_password":                 CommandItem("Policy", "generatePassword.py", "Genarate password"),
    "Policy_genarate_login":                    CommandItem("Policy", "generateLogin.py", "Genarate login"),
    #
    "Combine_telephone_by_tab_number":          CommandItem("Combine", "telephoneByTabNumber.py", "Get user telephone by tab number"),
    "Combine_telephone_migration":              CommandItem("Combine", "orionToAdTelephoneMigration.py", "Migration telephone from Orion to AD"),
}
...
class CONST:
    ...
    class AD:
        ...
        DOMAIN: str = "fmv"
        DOMAIN_MIRROR: str = "pih"
        DOMAIN_MAIN: str = DOMAIN
        DOMAIN_FULL: str = "fmv.lan"
        USER_HOME_FOLDER_DISK: str = "U:"
        DN_ACTIVE_UNIT: str = f"OU=Users,OU=Unit,DC={DOMAIN},DC=lan"
        DN_INACTIVE_UNIT: str = f"OU=deadUsers,OU=Unit,DC={DOMAIN},DC=lan"
...
class PATH_IT:
    ...
    NAME: str = "5. IT"
    NEW_EMPLORERS_NAME: str = "New employers"
    ROOT: str = os.path.join(PATH_SHARE.PATH, NAME)
    NEW_EMPLORERS: str = os.path.join(ROOT, NEW_EMPLORERS_NAME)

    def get_NEW_EMPLORERS(self, path: str) -> str:
        return os.path.join(PATH_IT.NEW_EMPLORERS, path)
