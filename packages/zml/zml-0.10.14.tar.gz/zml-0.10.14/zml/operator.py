...
class Processor(Operator):
    ...
    def subtree_handler_for_loop(self, node):
        # prepare
        variable_descriptor = node.tokens.for_loop.variable.value
        if not self.lazy:
            iterator = node.eval_combined_accessor(node.tokens)
    ...
    def subtree_handler_if_statement(self, node):
        # prepare
        condition = node.eval_combined_accessor(node.tokens)
        if not self.lazy:
            if condition:
                # execute
                self.subtree_handler_default(node)
...
class Renderer(Operator):
    ...
    def subtree_handler_for_loop(self, node):
        ...
        if not self.lazy:
            variable_descriptor = node.tokens.for_loop.variable.value
            iterator = node.eval_combined_accessor(node.tokens.for_loop.iterator.combined_accessor.list)
            if isinstance(iterator, Model):
                iterator = iterator.fields.values()
            for item in iterator:
                node.set_var(variable_descriptor, item)
    ...
    def subtree_handler_if_statement(self, node):
        ...
        out = ''
        if not self.lazy:
            condition = node.eval_combined_accessor(node.tokens.if_statement)
            if condition:
                # execute
                out += self.subtree_handler_default(node)
        else:
