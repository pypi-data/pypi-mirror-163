...
class APIRouter(Blueprint):
    ...
    def route(
        ...
        **options: Any
        ...
        def decorator(func: Callable) -> Callable:
            ...
            pydantic_model_no_body = self.generate_endpoint_pydantic(func.__name__+"Schema_no_Body", paired_params, with_body=False)
            pydantic_model = self.generate_endpoint_pydantic(func.__name__+"Schema", paired_params, with_body=True)
            ...
            def create_modified_func():
                ...
                def modified_func(**paths):
                    try:
                        ...
                        if req.method == "GET":
                            valid_kwargs = self.get_kwargs(paths, req, paired_params, pydantic_model_no_body)
                        else:
    ...
    def generate_endpoint_pydantic(self, name: str, paired_params: Dict[str, ParamSignature], with_body: bool = True):
        if with_body:        
            params = {
                key: (pp._type, pp.param_object) 
                for key, pp in paired_params.items()
            }
        else:
            params = {
                key: (pp._type, pp.param_object.copy().disable_constraint())
                for key, pp in paired_params.items()
            }
                            ...
                            if b:
                                if BaseModel.__subclasscheck__(b):
                                    if total_body == 1:
                                        ...
                                        kwargs[k] = b(**request.json.get(k, None))
                                else:
                                    kwargs[k] = request.json.get(k, None)
                            else:
                                kwargs[k] = request.json.get(k, None)
