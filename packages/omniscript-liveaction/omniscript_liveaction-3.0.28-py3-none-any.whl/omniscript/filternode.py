...
class AddressNode(FilterNode):
    ...
    def _load(self, props):
        ...
        self.address_1 = OmniAddress('addr1', _address_1, _address_type).address
        self.address_2 = OmniAddress('addr2', _address_2, _address_type).address

    def _store(self, props):
        if not isinstance(props, dict):
            return
        _type = (self.address_1.address_type if self.address_1
                 else self.address_2.type if self.address_2 else 0)
        props[_json_type] = _type
        props[AddressNode._json_address_1] = (self.address_1.format(False)
                                              if self.address_1 else "")
        props[AddressNode._json_address_2] = (self.address_2.format(False)
                                              if self.address_2 else "")
...
class PortNode(FilterNode):
    ...
    def _load(self, props):
        ...
        for k,v in props.items():
            ...
            if a == PortNode._tag_port_1:
                self.port_1 = OmniPort(v)
            elif a == PortNode._tag_port_2:
                self.port_2 = OmniPort(v)
    ...
    def _store(self, props):
        if not isinstance(props, dict):
            return
        _type = (self.port_1.port_type if self.port_1
                 else self.port_2.port_type if self.port_2 else 0)
        props[_json_type] = _type
        if self.port_1:
            props[PortNode._json_port_1] = str(self.port_1)
        if self.port_2:
            props[PortNode._json_port_2] = str(self.port_2)
        props[_json_accept_1_to_2] = self.option_accept_1_to_2
        props[_json_accept_2_to_1] = self.option_accept_2_to_1
        super(PortNode, self)._store(props)

    def to_string(self, pad, operation=""):
        criteria = (''.ljust(pad * FilterNode.pad_depth)
            + operation
