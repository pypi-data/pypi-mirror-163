...
from .invariant import (CAPTURE_STATUS_CAPTURING, DECODE_PLAIN_TEXT, DECODE_HTML,
    DECODE_TAG_STREAM, ENGINE_FORMAT_JSON, ENGINE_FORMAT_HTML, ENGINE_FORMAT_PLAIN,
    ENGINE_FORMAT_TAG_STREAM, ENGINE_OP_GET, ENGINE_OP_POST, ENGINE_OP_DELETE)
...
class Capture(object):
    ...
    media_info = None
    """The :class:`MediaInfo <omniscript.mediainfo.MediaInfo>` object
    of the capture.
    """
    ...
    def get_packet(self, number):
        """Returns a :class:`Packet <omniscript.packet.Packet>`
        object.
        ...
        """
        data = self.get_packet_data(number)
        return Packet(number, data)
    ...
    def get_packet_data(self, number):
        ...
        resp = self._engine._issue_command(command, ENGINE_OP_GET)
        data = bytearray(resp['data'])
        return data

    def get_packet_decode(self, number, format=DECODE_PLAIN_TEXT):
        """Returns a string of the packet decode.
        ...
        if format == DECODE_PLAIN_TEXT:
            decode = ENGINE_FORMAT_PLAIN
        elif format == DECODE_HTML:
            decode = ENGINE_FORMAT_HTML
        elif format == DECODE_TAG_STREAM:
            decode = ENGINE_FORMAT_TAG_STREAM
        else:
            raise OmniError('Unrecognized format parameter.')

        command = f'captures/{self.id.format()}/packets/{number}/'
        resp = self._engine._issue_command(command, ENGINE_OP_GET, decode)
        if format == DECODE_TAG_STREAM:
            return resp.encode()
        return resp
    ...
    def refresh(self):
        ...
        """
        props = self._engine._issue_command(f'captures/{self.id.format()}/')
    ...
    def start(self):
        ...
        props = self._engine._issue_command(command, ENGINE_OP_POST)
    ...
    def stop(self):
        ...
        if self.id is None:
            ...
            return False
        command = f'running-captures/{self.id.format()}/'
        props = self._engine._issue_command(command, ENGINE_OP_DELETE)
        return (props is not None)
