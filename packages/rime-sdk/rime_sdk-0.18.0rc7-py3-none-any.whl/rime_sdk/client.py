...
from rime_sdk.job import BaseJob, ContinuousTestJob, ImageBuilderJob, Job
...
class Client:
    ...
    IMAGE_MAP = {
        ImageType.DEFAULT: BaseImageType.BASE_IMAGE_TYPE_DEFAULT,
    ...
    def _check_expiration(self) -> None:
        ...
        req = GetLimitStatusRequest(
            ...
            limit_name=LicenseLimitName.LICENSE_LIMIT_NAME_EXPIRATION,
        )
        ...
        if limit_status == LimitStatus.Status.WARN:
            logger.warning(f"Expiration date approaching: {expiration_date}.")
        elif limit_status == LimitStatus.Status.ERROR:
            raise ValueError(f"Expiration date has already passed: {expiration_date}.")
    ...
    def _check_stress_test_limit(self) -> None:
        req = GetLimitStatusRequest(
            customer_name=self._backend.customer_name,
            limit_name=LicenseLimitName.LICENSE_LIMIT_NAME_STRESS_TEST_RUNS,
        )
        with self._backend.get_feature_flag_stub() as feature_flag_client:
            with self._backend.GRPCErrorHandler():
                feature_flag_response = feature_flag_client.GetLimitStatus(req)
        ...
        if limit_status == LimitStatus.Status.WARN:
            ...
            diff = limit_value - curr_value
            logger.warning(
                f"Approaching limit of {limit_value} stress test runs. "
                f"You have {diff} runs left."
            )
        elif limit_status == LimitStatus.Status.ERROR:
            raise ValueError(f"Reached stress test run limit: {limit_value}.")
        elif limit_status == LimitStatus.Status.OK:
            pass
        else:
            raise ValueError("Unexpected status value.")
    ...
    def create_managed_image(
        ...
        image_type: ImageType = ImageType.DEFAULT,
    ) -> RIMEImageBuilder:
        ...
        Args:
            ...
            image_type: ImageType
                An enum refering to the base image type. Valid options are default, all,
                tabular, images, and nlp.
        ...
        .. code-block:: python
           ...
           requirements, image_type=image_type)
        ...
        image_type_proto = self.IMAGE_MAP[image_type]
        req = CreateImageRequest(
            name=name, pip_requirements=requirements, image_type=image_type_proto
        )
        with self._backend.GRPCErrorHandler():
    ...
    def upload_local_image_dataset_file(
        ...
        for index, data_dict in enumerate(data_dicts):
            ...
            uploaded_image_path = self.upload_dataset_file(
                image_path, upload_path=upload_path
            )
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            return (
                data_dicts,
                self.upload_dataset_file(temp_path, upload_path=upload_path),
            )
