...
from NEMO_billing.invoices.invoice_data_processor import get_rate_with_currency
...
class DatabaseRates(Rates):
    ...
    def load_rates(self):
        ...
        self.expand_rates_table = BillingRatesCustomization.get("rates_expand_table") == "enabled"
    ...
    def get_tool_rate(self, tool: Tool) -> str:
        ...
        html_rate = "</div></div>"  # close of previous divs from the tool control page
        html_rate += f'<div class="tool-problems"><div class="media"><a onclick="toggle_details(this)" class="pointer collapsed" data-toggle="collapse" data-target="#rates_details"><span class="glyphicon glyphicon-list-alt pull-left notification-icon primary-highlight"></span><span class="glyphicon pull-left chevron glyphicon-chevron-{"right" if not self.expand_rates_table else "down"}"></span></a>'
        html_rate += f'<div class="media-body"><span class="media-heading">Rates</span><div id="rates_details" class="collapse {"in" if self.expand_rates_table else ""}"><table class="table table-bordered table-hover thead-light" style="width: auto !important; margin-bottom: 0">'
        if rate_categories:
        ...
        html_rate += "</tr></table></div></div>"
        # html_rate += '</div></div>' # commented out intentionally (those will be closed by the tool control page)
        return html_rate
