...
def send_email(params, message):
    ...
    with open(here / 'logs/noted_email.txt', 'w') as f:
        ...
        f.write('NOTED\n')
    # Send email
    cmd = os.popen('sendmail -vt < ' + here / 'logs/noted_email.txt')
    return
...
def main():
    ...
    """Main function."""
    # Logging
    logging.basicConfig(level = logging.NOTSET, filename = here / str('logs/transfer_broker_' + app_timestamp + '.log'), filemode = 'w', format = '%(asctime)s %(name)s - %(levelname)s - %(threadName)s: %(message)s')
    ...
    params.read(here / 'params/params.ini')
    # Argument parser
