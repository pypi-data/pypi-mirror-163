...
class Plugin:
    ...
    type_name: str
    plugin_middleware: Type[Middleware] = None
    env_options: Dict

    def __init__(self, app: App, alias: str, middleware_options: Dict = None):
        """Init."""
        self._prepare(app, alias)

        self._register_instance()
        self._register_middleware(middleware_options)
        self._register_hooks(app)

    def _prepare(self, app: App, alias: alias):
        """Prepare plugin."""
        self.alias = str(alias).lower()
        self.app = app
        self.spec = load_spec()
        self.env_options = fn.alias_plugin_options(
            alias=self.alias,
    ...
    def _register_middleware(self, middleware_options: Dict = None) -> None:
        """Register middleware."""

        if self.plugin_middleware:
            if middleware_options:
                ...
                self.app.add_middleware(self.plugin_middleware)

    def _register_hooks(self, app: App) -> None:
        """Register hooks."""

        app.add_event_handler(
            'startup',
            partial(self.prepare, app=app, spec=self.spec),
        )

        app.add_event_handler(
            'shutdown',
            partial(self.release, app=app, spec=self.spec),
        )
