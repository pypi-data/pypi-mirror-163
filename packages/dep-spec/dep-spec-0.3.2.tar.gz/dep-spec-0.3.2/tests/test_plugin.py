...
class Mongo(Plugin):
    ...
    plugin_middleware = CustomMiddleware
...
async def test_plugin_create(env_plugin_options, mock_app, spec, mock_client):
    ...
    plugin = Mongo(app=mock_app, alias='default')

    assert plugin.alias == 'default'
    assert plugin.fqdn == 'mongo.default'
    assert plugin.env_options

    assert isinstance(plugin.spec, Spec)

    await plugin.prepare(mock_app, spec=spec)
    await plugin.release(mock_app, spec=spec)

    assert 'pool_size' in plugin.env_options
    assert 'uri' in plugin.env_options

    assert plugin.env_options['uri'] == 'any'
    assert plugin.env_options['pool_size'] == '1'
