...
class Module(metaclass=_ModuleMeta):
    ...
    def inject(self, app: App, **module_kwargs) -> None:
        ...
        try:
            ...
            if app.spec.runtime.environment.dont_care_secrets():
                log_extra.update({
            ...
            log.error(
                f'Inject module options: {_module_exc}',
                extra=log_extra,
                exc_info=True,
            )
    ...
    def lifespan(self, scope, receive, send, app: App):
        ...
        async def _hook():
            ...
            if message['type'] == 'lifespan.startup':
                ...
                try:
                ...
                except AsyncIOErrors as _async_exc:
                    log.error(
                        f'Prepare hook: {_async_exc}',
                        extra={
                            'alias': self.alias,
                            'module_settings': self.module_settings,
                            'middleware_settings': self.middleware_settings,
                    ...
                    raise exc_type.ModuleException(_async_exc)
                except Exception as _any_exc:
                    log.error(
                        f'Prepare hook: {_any_exc}',
                        extra={
                            'alias': self.alias,
                            'module_settings': self.module_settings,
                            'middleware_settings': self.middleware_settings,
