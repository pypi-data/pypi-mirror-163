...
def run_in_demo_projects(ctx, py_cmd, cov=False, bare=False, after=None, start=None):
    """
    ...
    if bare:
        print("Run `{}` in {} demo projects:".format(py_cmd, len(demo_projects)), end=" ", flush=True)
    for dpname in demo_projects:
        if bare:
            print(dpname, end=" ", flush=True)
        else:
            print("-" * 80)
        ...
        with cd(pth):
            ...
            if result.exited:
                raise Exception("Subprocess exited with {}".format(result.exited))
    if bare:
        print("DONE")
        return "\n".join(outputs)


@task(name='install', help=dict(
    list="Don't install anything, just list the requirements to `stdout`.",
    batch="Don't ask for confirmations."))
def install(ctx, list=False, batch=False):
    """
    ...
    output = run_in_demo_projects(ctx, cmd, bare=True)
...
@task(name='prep')
def prep(ctx, cov=False, after=None):
    """
    ...
    if cmd:
        ...
        run_in_demo_projects(ctx, cmd, bare=True, after=after)
