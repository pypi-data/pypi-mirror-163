...
class CVisConv(IVisStage):
    ...
    def __call__(self) -> None:
        ...
        if self.strategy in [1, 2]:
            ...
            self._visualize_escaped_reentered_configuration()
    ...
    def _visualize_escaped_reentered_configuration(self) -> None:
        r"""
        """
        for folder in self.folders:
            print(f'Visualizing re-entered and re-escaped in folder: {folder}...')
            for file in (self.parent_directory / folder).iterdir():
                if str(file.stem).startswith("spin_reesc") or (file.stem).startswith("spin_reen"):
                    print(f'   Visualize: {file.stem}...')
                    l_latt = CLattice(source='STM', path=file)
                    if self.spinvisualization_technique == 'oop':
                        l_visu = CVisualPyVista(lattice=l_latt, cam=self.spincam)
                    elif self.spinvisualization_technique == 'ipoop':
                        l_visu = CVisualPyVista(lattice=l_latt, cam=self.spincam, heatmap=True, heatmap_saturation=1.0)
                        ...
                        raise ValueError('Selected spin visualization technique is not coded yet!')
                    pngpath = self.parent_directory / folder / (str(file.stem)+'.png')
                    l_visu(outpath=pngpath, resolution=self.resolution)
                    l_visu.plotter.close()
                    print('...done')
