...
from Pxucz.initial.set_variables import (
    ...
    INITIAL_LOADER_TEXT,
    INITIAL_LOADER_CLOSE, PXUCZ_ASPECT_X, PXUCZ_ASPECT_Y,
)
from Pxucz.utils import global_variables


def create_window(window_width: int, window_height: int, window_name: str, fullscreen: bool):
...
def set_window_aspect_ratio(window):
    global_variables.set_var(
        name=INITIAL_LOADER_TEXT, value=f"WINDOW_ASPECT = ({global_variables.get_var(PXUCZ_ASPECT_X)}, {global_variables.get_var(PXUCZ_ASPECT_Y)})"
    )
    glfw.set_window_aspect_ratio(window=window, numer=global_variables.get_var(PXUCZ_ASPECT_X), denom=global_variables.get_var(PXUCZ_ASPECT_Y))
