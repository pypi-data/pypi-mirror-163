...
class ThreadListService:
    ...
    def list(params):
        ...
        # 分页数据
        page_set = ThreadListSerializer(Paginator(thread_set, size).get_page(page), many=True)
        final_res = page_set.data

        for it in final_res:
            it.update(it.pop('statistic')) if 'statistic' in it else ''
            it.update(it.pop('thread_extends')) if 'thread_extends' in it else ''

        return {'data': final_res, 'size': int(size), 'page': int(page), 'count': count}, None
