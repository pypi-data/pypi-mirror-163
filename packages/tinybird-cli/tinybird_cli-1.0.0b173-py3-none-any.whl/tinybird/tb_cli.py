...
async def get_config_and_hosts(ctx):
...
@click.pass_context
@coro
async def create_workspace(ctx: Context, workspace_name: str, user_token: str, fork: bool):
    ...
    try:
        ...
        created_workspace = await user_client.create_workspace(workspace_name)
...
async def add_members_to_workspace(ctx: Context, members_emails: str, user_token: str):
    ...
    if workspace is None:
        raise click.ClickException(FeedbackManager.error_unknown_resource(resource=config['d']))
...
@click.argument('connection_id', required=True)
@click.argument('datasource_name', required=True)
...
@click.argument('datasource_name', required=True)
@click.argument('workspace_name_or_id', required=True)
...
async def datasource_share(ctx: Context, datasource_name: str, workspace_name_or_id: str, user_token: str, yes: bool):
    ...
    if not user_token:
        user_token = click.prompt(
            f"\nIn order to share the workspace we need your user token. Copy it from {ui_host}/tokens and paste it here",
            hide_input=True)
