...
class ElasticsearchInitUtil:

    def __init__(
            self, elasticsearch: Elasticsearch, alias_name: str,
        ...
        self._policy_name = f'{alias_name}-ilm-policy'
    ...
    def initialize(self):
        ...
        try:
            ...
            if not indices.exists_alias(name=self._alias_name):
    ...
    def _create_index_template(self):
            ...
            r'settings': {
                r'index': {
                    r'lifecycle': {
                        ...
                        r'rollover_alias': self._alias_name,
                    },
                    r'refresh_interval': r'5s',
                    r'sort': {
                        r'field': r'datetime',
                        r'order': r'desc',
                    }
        ...
        self._elasticsearch.indices.put_index_template(
            ...
            template=template,
            index_patterns=[f'{self._alias_name}-*'],
        )

        self._elasticsearch.indices.create(
            index=f'<{self._alias_name}-{{now/d}}-000001>',
            aliases={self._alias_name: {r'is_write_index': True}},
        )
...
class ElasticsearchSink:
    ...
    def __init__(
        ...
        ElasticsearchInitUtil(
            self._elasticsearch, index,
            rollover_max_age, rollover_max_primary_shard_size, delete_min_age
        ).initialize()
