...
class MysqlOperator(object):
    ...
    def write_to_mysql(
        ...
        self.logger.debug(f"Chunk size is {chunk_size}.")
        with self.Session.begin() as session:
            i = 0
            for chunk in chunks:
                i += 1
                upsert(values=chunk, session=session, table_name=table_name)
                self.logger.debug(f"Chunk {i} uploaded")
    ...
    def delete(
        ...
        with self.Session.begin() as session:
            session.execute(sql)
            self.logger.debug(f"The below sql statement is executed \n {sql}")
    ...
    def select_all(
        ...
        self.logger.debug(f"The below sql statement is executed \n {sql}")
        self.logger.debug(f"Number of records retrieved = {str(len(result))}")
        return result
    ...
    def get_row_count(
        ...
        with self.Session.begin() as session:
            self.logger.debug(f"The below sql statement will be executed: \n {sql}")
