...
class SAPAbapODataConnector(HTTPConnector):
    ...
    def menu_action_select_entity(self, key:str=None):
        ...
        if self.endpoint != None and self.menu_entity != None and self.menu_request != None:
            ...
            self.menu_result = self.menu_request.select(add_params=self.menu_request_params, entity_key=key)
            ...
            if self.menu_result != None:
                ...
                print(f"Endpoint {self.endpoint} entity {self.menu_entity} selected - {count} records")
                if count == 0:
                    self.menu_result == None
            else:
            ...
            if entities == None or len(entities) == 0:
                ...
                while valid == True:
                    ...
                    if action.is_cancel_entered() == True:
                        ...
                        self.menu_request = SAPAbapODataRequest(self, self.menu_entity, self.endpoint)
                        if self.menu_request.set_metainfo(self.metadata_util) == True:
                            print(f"Entity {self.menu_entity} prepared for request")
                            valid = False
                        else:
                            self.menu_request = None
    ...
    def menu_find_endpoints(self):
        ...
        if endpoints == None or len(endpoints) == 0:
            ...
            valid = True
            while valid == True:
                menu = MenuSelection("Select OData Endpoint", entries, False, True) 
                ...
                if action.is_cancel_entered() == True:
                    ...
                    if self.set_endpoint(selected_endpoint) == True:
                        ...
                        print(f"Errors occured while selecting endpoint {selected_endpoint}\n{last_error}")
    ...
    def menu(self):  
        ...
        self.info("enter connector menu")
        ...
        while valid == True:
            ...
            if self.endpoint != None:
                if self.metadata_util != None and self.endpoint != None:
                    entries[SAPAbapODataConnector.MENU_METADATA_INFO] = f"Display metainfo for endpoint {self.endpoint}"

                entries[SAPAbapODataConnector.MENU_FIND_ENTITIES] = f"Choose entity in OData endpoint {self.endpoint}"

                    
                if self.menu_request != None:
                    entries[SAPAbapODataConnector.MENU_ENTITY_BY_KEY] = f"Extract single record by entity key (entity {self.menu_entity}, endpoint {self.endpoint})"
                    entries[SAPAbapODataConnector.MENU_REQUEST_CONFIGURE] = f"Configure data extraction request (entity {self.menu_entity}, endpoint {self.endpoint})"
                    entries[SAPAbapODataConnector.MENU_ENTITY_SELECT] = f"Extract data (entity {self.menu_entity}, endpoint {self.endpoint})"
            ...
            if self.menu_result != None:
                    ...
                    if result_count > 0:    
                        entries[SAPAbapODataConnector.MENU_RESULT_RAW] = f"Display current as raw data (entity {self.menu_entity}, endpoint {self.endpoint})"
                        entries[SAPAbapODataConnector.MENU_RESULT_PANDAS] = f"Display current result as pandas dataframe (entity {self.menu_entity}, endpoint {self.endpoint})"
                    entries[SAPAbapODataConnector.MENU_RESULT_MENU] = f"Enter current result menu (entity {self.menu_entity}, endpoint {self.endpoint})"
            ...
            if action.is_quit_entered():
                ...
                try:
                    ...
                    if selected == SAPAbapODataConnector.MENU_FIND_ENDPOINTS:
                        ...
                        self.menu_action_metadata_info()
                        ...
                        key = UserInput("Enter Entity Key").get_input(empty_allowed=True)
    ...
    def set_endpoint(self, endpoint:str) -> bool:
        ...
        if metadata_util.parse_metadata_xml(metadata_xml) == False:
...
class SAPAbapODataRequest(HTTPRequest):
    ...
    def parse_record(self, record:dict, detected_type=None) -> dict:
        try:
            ...
            self.error(f"parsing OData result failed: {type(exc)} - {exc}\n{record}")
            return None
    ...
    def select(self, max_rows:int=1000, odataFilter:str=None, entity_key:str=None, add_params:dict=None) -> Result:
        ...
        try:
            ...
            if not "d" in json.keys():
                self.error(f"invalid answer - no valid odata result json - {text}") 
                return None
            json_data = json["d"]
            if "results" in json_data.keys():
                ...
                self.error(f"invalid answer - errors occured - {type(exc)} - {exc}\n{text}") 
                return None
        ...
        if json_result == None:
            ...
            if type(json_result) != list:
                ...
                for record in json_result:
                    result_rec, odata_type = self.parse_record(record, odata_type)
                    if result_rec != None:
                        result.append(result_rec)
                    else:
