		...
		for line in prot:
			...
			if line.startswith(">"):
				if len(current_seq) > 0:
					kmer_set = self.unique_kmer_simple_key(current_seq)
...
def sql_query(genomes, proteins, hmms, db_name, output, threads, verbose, do_stdev, style, in_mem, make_db, qdb_name, do_comp):
	...
	threads = threads, do_sd = do_stdev, output_style = style, do_comp = do_comp, build_db_from_queries = make_db, 
	qdb_name = qdb_name, verbose = verbose, hmm_path = hmm_path)
...
def on_disk_work_one(query_grouping):
	curs = database.cursor()
	for q in query_grouping:
		results = []
		qname = _qnames[q]
...
class db_db_remake:
	...
	def open(self):
		...
		for group in groupings:
			...
			qgak_queue.put((group_id, next_set,))
			group_id += 1
	...
	def load_in_mem(self):
		...
		if self.style == "matrix":
			...
			for result in pool.imap_unordered(two_work, range(0, self.threads)):
	...
	def db_on_disk(self):				
		...
		if self.style == "matrix":
			result_files = []
			for result in pool.imap_unordered(on_disk_work_two, range(0, self.threads)):
				result_files.append(result)
...
def miga_merge_opts():
	...
	Give one genome in nt, aa, or aa+hmm format and a database to create or add to. It'll add the genome as efficiently as possible.
	''')
	...
	parser.add_argument('--target',   dest = 'database', default = None, help =  'Path to the target database. The file supplied will be added')	
