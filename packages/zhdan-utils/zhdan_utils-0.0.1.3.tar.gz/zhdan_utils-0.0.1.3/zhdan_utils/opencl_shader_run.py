...
def run_kernel(name, shape, *, inputs = None, outputs_number = None, ctx=[], queue = [], prg = [], source_code = None, file_name = 'compute_shaders.cl'):
    """
    Runs OpenCL kernel
    ...
    if inputs is None or outputs_number is None:
        raise(Exception(f"\'{inputs = }\' or \'{outputs_number = }\' is None"))
    if len(ctx)==0:
