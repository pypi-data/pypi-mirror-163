...
def dpsgd_cnn(
    ...
    class DPSGDClient(Client):
        ...
        def train(self):
            ...
            self.collect_metric([torch.linalg.norm(g).item() for i, g in enumerate(grads) if i not in self.bn_running_param_indices], "norms")

            for i, layer in enumerate(grads):
                if i not in self.bn_running_param_indices:
                    grads[i] /= max(1, torch.linalg.norm(layer) / C)
                    grads[i] += stdev * C * torch.randn(layer.shape)
    ...
    rayleaf.run_experiment(
        ...
        output_dir= f"output/norms/dpsgd-{dataset}-stdev{stdev}-C{C}-{curr_time}/",
        model = model,
