...
def load_params(cfg_filedir):
  # cfg file used initially is = 'noisyspeech.cfg'

  params = dict()
  cfg = CP.ConfigParser()
  cfg.read(cfg_filedir)
  params['cfg'] = cfg._sections['noisy_speech']
  cfg = params['cfg']
  params['fs'] = int(cfg['sampling_rate'])
  params['audioformat'] = cfg['audioformat']
  params['audio_length'] = float(cfg['audio_length'])
  params['target_level_lower'] =  float(cfg['target_level_lower'])
  params['target_level_upper'] =  float(cfg['target_level_upper'])
