...
def _bounding_box_from_xline_range(xline_range: Mapping[int, Tuple[int, int]]) -> "Seismic3dRect":
...
class Seismic3dDef(Seismic3dExtent):
    ...
    def get_majors(self) -> Mapping[int, List[int]]:
        ...
        return dict((minor, list(majors)) for minor, majors in mapping.items())
    ...
    def get_major_count(self) -> Mapping[int, int]:
        ...
        mapping = {}
        for xline, inlines in self.get_majors():
            mapping[xline] = len(inlines)
        return mapping

    def get_minor_count(self) -> Mapping[int, int]:
        mapping = {}
        for major in self.lines:
            # Unlike get_major_count, we can fetch one line at a time and save memory.
            mapping[major] = len(self.get_minor(major))
        return mapping
