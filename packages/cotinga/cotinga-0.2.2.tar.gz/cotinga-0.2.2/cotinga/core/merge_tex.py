...
def list_levels(classname, pupils, db):
    ...
    # TODO: handle the pupils that have got to the last level, the next line
    # raises an IndexError for them since there's no next level...
    levels = [all_levels[all_levels.index(L) + 1] for L in levels]
    return levels
...
def check_templates_fmt(dirpath, levels):
    ...
    if any(True if (Path(dirpath) / f'{L}.tex').is_file() else False
           for L in levels):
        singletons = True
    if any(True if (Path(dirpath) / f'{L}0.tex').is_file() else False
           for L in levels):
        pairs = True
...
def missing_templates(dirpath, levels, fmt):
    ...
    if fmt == 'singletons':
        return [L for L in levels if not (dirpath / f'{L}.tex').is_file()]
    else:  # fmt == 'pairs'
        return set([L for L in levels
                    if not (dirpath / f'{L}0.tex').is_file()]
                   + [L for L in levels
                      if not (dirpath / f'{L}1.tex').is_file()])
...
def incorrect_templates(dirpath, levels, fmt):
    ...
    if fmt == 'singletons':
        return set([L for L in levels
                    if '<FULLNAME>' not in (dirpath / f'{L}.tex').read_text()])
    else:  # fmt == 'pairs'
        return set([L for L in levels
                    if '<FULLNAME>' not in (dirpath / f'{L}0.tex').read_text()]
                   + [L for L in levels
                      if '<FULLNAME>'
                      not in (dirpath / f'{L}1.tex').read_text()])
...
class MergingOperation(GObject.Object):
    ...
    def merge_tests(self, dirpath, testname, classname, pupils, levels, fmt,
        ...
        for i, couple in enumerate(zip(pupils, levels)):
            ...
            if fmt == 'singletons':
                template_path = dirpath / f'{level}.tex'
            else:  # fmt == 'pairs'
                parity = counters[level] % 2
                template_path = dirpath / f'{level}{parity}.tex'
                counters[level] += 1
