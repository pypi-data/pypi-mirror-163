...
class Service:
    ...
    async def run_listener(self):
        async with await create_udp_socket(local_host=self.service_address, local_port=self.service_port) as udp:
            logger.debug('Создан UDP сокет {host}:{port}', host=self.service_address, port=self.service_port)
    ...
    async def run_sender(self):
        ...
        while True:
            ...
            if self.socket is None:
                ...
                logger.debug('Создаем сокет для отправки на {host}:{port}', host=self.auv_address, port=self.auv_port)
            ...
            if self.connected:
                ...
                async with await create_connected_udp_socket(remote_host=self.auv_address, remote_port=self.auv_port) as udp:
                    await udp.send(crc_package)
                    logger.debug('Отправка пакета на аппарат: {}', buffer)
                self.counter += 1
            else:
