...
class DataFileProperty:
    ...
    def _daily_load_and_save(self, file_pro: FileProperty) -> bool:
        ...
        if self.data_type == 'kline':
            exchan = LoadDataKline(self.exchange, csv_path=file_pro.file_path_csv())
        else:
            exchan = LoadTradeKline(self.exchange, csv_path=file_pro.file_path_csv())
        unix_start, unix_end = int(file_pro.start_date.timestamp() * 1000), int(file_pro.end_date.timestamp() * 1000)
        # 下载
        exchan.load_all(timeframe=file_pro.timeframe, unix_start=unix_start,
                        unix_end=unix_end)
        # 压缩
    ...
    def load(self, total=100):
        ...
        elif self.file_pro.freq == 'yearly':
            self.file_format = '%Y'
            self._merge(year_during, total)
