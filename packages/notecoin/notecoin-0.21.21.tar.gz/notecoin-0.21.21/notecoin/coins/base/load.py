...
from notecoin.base.database import TradeData
...
class LoadData:
    ...
    def write_data(self, data_list, cache=True):
        ...
        if cache and len(self.cache_data) < 30000:
            return
...
class LoadTradeKline(LoadData):
    def __init__(self, *args, **kwargs):
        ...
        self.table = TradeData(
            table_name=f"{self.exchange.name}-trade-{datetime.utcfromtimestamp(self.unix_start/1000).strftime('%Y%m%d')}")

    def load_close(self):
        self.table.to_csv_all(file_path=self.csv_path)

    def _write(self, data_list):
        self.table.insert(values=data_list)
        # self.csv_writer.writerows(data_list)
