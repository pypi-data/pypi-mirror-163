...
class ExpiredSessionTokenException(Exception):
    """Raise when a session token expires"""
	...
	def getMarketDataFromFile(urlForDetails):
		...
		while(isNotComplete):
			try:
				...
				if response.status_code == 403:
					raise ExpiredSessionTokenException("Your session token has expired, please renew.")
				time.sleep(pause)
		...
		while True:
			try:
				...
				fo.close()
				marketData = pd.read_csv(fileName, compression='gzip') #, header=None) #, error_bad_lines=False)
				if marketData.empty:
					time.sleep(pause)
					continue
				break
			except Exception as e:
				if fileResponse.status_code == 403:
					raise ExpiredSessionTokenException("Your session token has expired, please renew.")
				time.sleep(pause)
