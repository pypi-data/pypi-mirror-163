...
## Contents
 ...
 - `StateAwareMixin` with `@when` and `@toggle` decorators for stateful MQTT clients.
...
## `StateAwareMixin`, `@when` and `@toggle` decorators
...
Change handlers are should be decorated with either `@when` or `@toggle`.
