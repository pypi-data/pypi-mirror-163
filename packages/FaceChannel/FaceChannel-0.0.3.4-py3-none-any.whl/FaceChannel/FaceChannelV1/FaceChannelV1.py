...
class FaceChannelV1:
    ...
    def getDimensionalModel(self):
        ...
        dense = tf.keras.layers.Dense(200, activation="relu", name="denseLayer")(backbone)

        arousal_output = tf.keras.layers.Dense(units=1, activation='tanh', name='arousal_output')(dense)
        valence_output = tf.keras.layers.Dense(units=1, activation='tanh', name='valence_output')(dense)

        return tf.keras.Model(inputs=backbone.input, outputs=[arousal_output, valence_output])
    ...
    def getCategoricalModel(self, numberClasses):
        ...
        categoricalOutput = tf.keras.layers.Dense(units=numberClasses, activation='tanh', name='categoricalOutput')(dense)

        return tf.keras.Model(inputs=backbone.input, outputs=categoricalOutput)
    ...
    def buildFaceChannel(self):
        ...
        inputLayer = tf.keras.Input(shape=inputShape, name="Vision_Network_Input")

        # Conv1 and 2
        conv1 = tf.keras.layers.Conv2D(nch / 4, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv1")(
            inputLayer)
        bn1 = tf.keras.layers.BatchNormalization(axis=1)(conv1)
        actv1 = tf.keras.layers.Activation("relu")(bn1)

        conv2 = tf.keras.layers.Conv2D(nch / 4, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv2")(actv1)
        bn2 = tf.keras.layers.BatchNormalization(axis=1)(conv2)
        actv2 = tf.keras.layers.Activation("relu")(bn2)

        mp1 = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(actv2)
        drop1 = tf.keras.layers.Dropout(0.25)(mp1)

        # Conv 3 and 4
        conv3 = tf.keras.layers.Conv2D(nch / 2, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv3")(drop1)
        bn3 = tf.keras.layers.BatchNormalization(axis=1)(conv3)
        actv3 = tf.keras.layers.Activation("relu")(bn3)

        conv4 = tf.keras.layers.Conv2D(nch / 2, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv4")(actv3)
        bn4 = tf.keras.layers.BatchNormalization(axis=1)(conv4)
        actv4 = tf.keras.layers.Activation("relu")(bn4)

        mp2 = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(actv4)
        drop2 = tf.keras.layers.Dropout(0.25)(mp2)

        # Conv 5 and 6 and 7
        conv5 = tf.keras.layers.Conv2D(nch / 2, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv5")(drop2)
        bn5 = tf.keras.layers.BatchNormalization(axis=1)(conv5)
        actv5 = tf.keras.layers.Activation("relu")(bn5)

        conv6 = tf.keras.layers.Conv2D(nch / 2, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv6")(actv5)
        bn6 = tf.keras.layers.BatchNormalization(axis=1)(conv6)
        actv6 = tf.keras.layers.Activation("relu")(bn6)

        conv7 = tf.keras.layers.Conv2D(nch / 2, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv7")(actv6)
        bn7 = tf.keras.layers.BatchNormalization(axis=1)(conv7)
        actv7 = tf.keras.layers.Activation("relu")(bn7)

        mp3 = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(actv7)
        drop3 = tf.keras.layers.Dropout(0.25)(mp3)

        # Conv 8 and 9 and 10

        conv8 = tf.keras.layers.Conv2D(nch, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="Vision_conv8")(drop3)
        bn8 = tf.keras.layers.BatchNormalization(axis=1)(conv8)
        actv8 = tf.keras.layers.Activation("relu")(bn8)

        conv9 = tf.keras.layers.Conv2D(nch, (3, 3), padding="same", kernel_initializer="glorot_uniform", name="conv9")(actv8)
        bn9 = tf.keras.layers.BatchNormalization(axis=1)(conv9)
        actv9 = tf.keras.layers.Activation("relu")(bn9)

        conv10 = tf.keras.layers.Conv2D(nch, (3, 3), padding="same", kernel_initializer="glorot_uniform", activation="relu",
                        name="conv10")(actv9)

        conv10_inhibition = tf.keras.layers.Conv2D(nch, (3, 3), padding="same", kernel_initializer="glorot_uniform", activation="relu",
                                   name="conv10_inhibition")(actv9)

        v_conv_inhibitted = tf.keras.layers.Lambda(function=shuntingInhibition)([conv10, conv10_inhibition])

        mp4 = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(v_conv_inhibitted)
        drop4 = tf.keras.layers.Dropout(0.25)(mp4)
