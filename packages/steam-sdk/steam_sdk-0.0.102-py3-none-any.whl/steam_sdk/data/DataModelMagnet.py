...
class OptionMultipole(BaseModel):
    """
        Level 2: Class for FiQuS Multipole
    """
    run_type: str = None  # start_from_geometry, geometry, start_from_mesh, mesh, start_from_solution, solution
    launch_gui: bool = None
    reference_files: Reference = Reference()
    overwrite: bool = None
...
class SolveMultipole(BaseModel):
    """
        Level 2: Class for FiQuS Multipole
    """
...
class FiQuSMultipole(BaseModel):
    ...
    solve: SolveMultipole = SolveMultipole()
...
class Steps(BaseModel):
    ...
    gen_breps: Step = Step()
    ef_breps: Step = Step()
    pre_process: Step = Step()
    mesh: Step = Step()
    solve: Step = Step()
    post_process: Step = Step()
