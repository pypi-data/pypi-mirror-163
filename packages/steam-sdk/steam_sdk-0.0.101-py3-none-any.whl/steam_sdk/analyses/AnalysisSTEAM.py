...
class AnalysisSTEAM:
    ...
    def step_make_model(self, step, verbose: bool = False):
        ...
        ## Assuming the steam-models directory structure if 'steam-models' or model_library found in library path.
        # Else assuming library path directs straight to model
        if ("\steam-models\\" in str(self.library_path)) or ("/steam-models/" in str(self.library_path))\
                or ("\model_library\\" in str(self.library_path)) or ("/model_library/" in str(self.library_path)):
        ...
        if 'FiQuS' in step.software:
            local_model_folder = os.path.join(Path(self.settings.local_FiQuS_folder), step.simulation_name, str(step.simulation_number))
        else:
        ...
        BM = BuilderModel(file_model_data=file_model_data, case_model=case_model, software=software,
                          ...
                          relative_path_settings=relative_path_settings, local_model_path=local_model_folder)

        # Build simulation file
        if step.simulation_number:
            if 'FiQuS' in step.software:
                self.setup_sim_FiQuS(simulation_name=step.simulation_name, simulation_type=step.type, sim_number=step.simulation_number)
    ...
    def step_modify_model(self, step, verbose: bool = False):
        ...
        for v, value in enumerate(step.variable_value):
            ...
            if len_simulation_numbers > 0:
                ...
                if 'FiQuS' in step.software:
                    BM.buildFiQuS(local_folder=os.path.join(Path(self.settings.local_FiQuS_folder), step.simulation_name, str(simulation_number)))
                    self.setup_sim_FiQuS(simulation_name=step.simulation_name, simulation_type=step.type, sim_number=simulation_number)
                if 'LEDET' in step.software:
    ...
    def setup_sim_FiQuS(self, simulation_name, simulation_type, sim_number):
        """
        ...
        # Unpack
        output_folder = self.output_path
        local_FiQuS_folder = Path(self.settings.local_FiQuS_folder)

        # Add simulation number to the list
        self.list_sims.append(sim_number)

        # Make simulation folder
        local_model_folder = os.path.join(local_FiQuS_folder, simulation_name, str(sim_number))
        make_folder_if_not_existing(local_model_folder)

        # Copy simulation file
        file_name_temp = os.path.join(output_folder, simulation_name)
        yaml_temp = os.path.join(file_name_temp + '.yaml')
        file_name_local = os.path.join(local_model_folder, simulation_name)
        yaml_local = os.path.join(file_name_local + '.yaml')
        shutil.copyfile(yaml_temp, yaml_local)

        if simulation_type == 'MakeModel':
            geo_temp = os.path.join(file_name_temp + '.geom')
            set_temp = os.path.join(file_name_temp + '.set')
        ...
        print('Simulation files {} generated.'.format(file_name_temp))
        print('Simulation files {} copied.'.format(file_name_local))
    ...
    def run_sim(self, software: str, simulation_name: str, sim_number: int, simFileType: str = None, verbose: bool = False):
        ...
        """
        if software == 'FiQuS':
            local_model_folder_input = os.path.join(self.settings.local_FiQuS_folder, simulation_name, str(sim_number))
            relative_folder_output = os.path.join(simulation_name, str(sim_number))
            dFiQuS = DriverFiQuS(path_exe=self.settings.FiQuS_path, path_folder_FiQuS=self.settings.local_FiQuS_folder,
                                 path_folder_FiQuS_input=local_model_folder_input, verbose=verbose)
            dFiQuS.run_FiQuS(simulation_name, outputDirectory=relative_folder_output)
        elif software == 'LEDET':
