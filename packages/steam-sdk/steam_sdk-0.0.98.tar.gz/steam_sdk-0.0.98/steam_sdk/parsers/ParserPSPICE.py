        ...
        with open(full_path_file_name) as file:
            for row, line in enumerate(file):
                ...
                if '.TRAN' in line:
                    ...
                    line_split = line.rstrip('\n').split(' ')
                    self.circuit_data.Analysis.simulation_time.frequency_step = str(line_split[1])
                    self.circuit_data.Analysis.simulation_time.frequency_points = str(line_split[2])
                    self.circuit_data.Analysis.simulation_time.frequency_start = str(line_split[3])
                    self.circuit_data.Analysis.simulation_time.frequency_end = str(line_split[4])
                    continue
                elif '.STEP' in line:
    ...
    def write2pspice(self, full_path_file_name: str, verbose: bool = False):
        ...
        ** Writes a PSPICE netlist file **
        ...
        if analysis_type == 'transient':
            ...
            # Unpack inputs
            frequency_step = self.circuit_data.Analysis.simulation_time.frequency_step
            frequency_points = self.circuit_data.Analysis.simulation_time.frequency_points
            frequency_start = self.circuit_data.Analysis.simulation_time.frequency_start
            frequency_end = self.circuit_data.Analysis.simulation_time.frequency_end
            # Check inputs
            ...
            if 'Hz' not in frequency_start:
                ...
                print('No unit found in AC analysis starting frequency. Added unit Hz')
            if 'Hz' not in frequency_end:
                frequency_end = str(frequency_end)+'Hz'
                print('No unit found in AC analysis last frequency. Added unit Hz')
    ...
    def copy_additional_files(self, verbose: bool = False):
        ...
        for file_to_copy in self.circuit_data.GeneralParameters.additional_files:
            ...
            if os.path.isfile(os.path.join(file_to_copy)):
                ...
                print('File {} does not exist. Skipped.'.format(file_to_copy))
                continue
            if os.path.isdir(self.output_path):
                print('Dir {} exists'.format(self.output_path))
            else:
                print('Dir {} does not exist. Skipped.'.format(self.output_path))
                continue
...
def add_AC_analysis(frequency_step, frequency_points, frequency_start, frequency_end):
    ''' Format transient analysis row '''
    formatted_text = f'.AC {frequency_step} {str(frequency_points)} {str(frequency_start)} {str(frequency_end)}'
    return formatted_text
