...
def ValueFromDVPtr(cptr):
    ...
    elif name_view_cls=='cuFloatComplex':
        real = native.n_dvcomplex64_real(cptr)
        imag = native.n_dvcomplex64_imag(cptr)
        return complex(real, imag)
    elif name_view_cls=='cuDoubleComplex':
        real = native.n_dvcomplex128_real(cptr)
        imag = native.n_dvcomplex128_imag(cptr)
        return complex(real, imag)        
    else:
...
class DVComplex64(DeviceViewable):
    def __init__(self, value):
        self.m_cptr = check_cptr(native.n_dvcomplex64_create(value.real, value.imag))
    def value(self):
        real = native.n_dvcomplex64_real(self.m_cptr)
        imag = native.n_dvcomplex64_imag(self.m_cptr)
        return complex(real, imag)

class DVComplex128(DeviceViewable):
    def __init__(self, value):
        self.m_cptr = check_cptr(native.n_dvcomplex128_create(value.real, value.imag))
    def value(self):
        real = native.n_dvcomplex128_real(self.m_cptr)
        imag = native.n_dvcomplex128_imag(self.m_cptr)
        return complex(real, imag)
