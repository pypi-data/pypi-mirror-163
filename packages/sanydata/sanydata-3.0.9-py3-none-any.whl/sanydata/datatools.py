...
class DataTools(object):
    ...
    def __init__(self, *args):
        self.es = Elasticsearch(hosts='http://192.168.0.3:19200/')
...
class EsHandler(object):
    ...
    def __init__(self,
        ...
        self.model_middle_index = model_middle_index
        self.stub = stub
        self.dt = DataTools()
        self.es_client = None
        try:
    ...
    def bulk_update_model_result(self, wind_farm, turbine, data_time, data_type, data_doc, usage='all', file_path=None):
        """
        模型结果信息批量入库
        ...
        """
        es = self.es_client
        up_dict = {'_index': self.model_middle_index,
                   '_op_type': 'update',
                   "doc_as_upsert": True}
        ...
        if isinstance(data_doc, dict):
            for i in ['wind_farm', 'turbine', 'data_time', 'data_type', 'usage', 'file_path']:
                data_doc[i] = locals()[i]
                up_dict['_id'] = '@'.join([data_type, wind_farm, turbine, data_time])
    ...
    def get_model_middle_data(self, wind_farm, turbine=None, data_type=None,
                              start_time=None, end_time=None, columns=None,
                              usage='all', file_path=None):
        es = self.es_client
        # 机组号检查
        turbine = self.change_turbine(turbine)
        # 时间格式检查
        ...
        if isinstance(columns, list) and len(columns) > 0:
            source_includes = ['wind_farm', 'turbine', 'data_time', 'data_type'] + columns
            source_includes = list(set(source_includes))
        else:
            source_includes = None
        search_data = es.search(index='model_middle_result',
                                query=bd['query'],
