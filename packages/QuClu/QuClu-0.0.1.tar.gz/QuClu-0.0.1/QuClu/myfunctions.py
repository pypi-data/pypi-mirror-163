...
#in this case the partition will be distributed as a asymmetric Laplace. But the likelihood wrt to theta became VV (the within cluster 
# sum of distances quantile theta) we will use this find the optimal theta, by searching for the argmin
...
def alg_CU(data, k = 2,  eps = 1e-8, it_max = 100, B = 30): 
    ...
    #Initialization: since the algorithm is very dependent on the initialization of theta (that is common to all variables) we make the algorithm start at B=30 different values,
    #and keep the theta that gives the smallest sum of distances
    for j in range(B):
...
def alg_CS(data, k = 2,  eps = 1e-8, it_max = 100, B = 30,lam =0): 
    #initialization, and extraction of the number of observations and variables
    #to add the if
    lam = np.ones(data.shape[1])
    nobs = data.shape[0]
    p = data.shape[1]
    #creating zeros matrix that we will use later 
    qq = np.zeros((k,p))
    ...
    for j in range(B):
        ...
        for i in range(k):
            ...
            #now we calculate the sum( variable wise) of distances of each points from the quantile of each vluster and save this in the matrix QQ
    ...
    while ratio > eps and h < it_max :
        ...
        for i in range(k):
            if nk[i] > 0 : #for each non-empty cluster
                qq[i,] = np.apply_along_axis(np.quantile,0,data.values[cl==i],theta) #find new cluster quantiles
...
# CLUSTERING
    #we now have 2 steps, the clustering step, and then we update the parameter of each cluster
    
    #start with the value obtained after selecting the best initialization
    cl = cl_true
    theta = theta_true
    ratio = 5
    h = 0
...
def alg_VS(data, k = 2,  eps = 1e-8, it_max = 100, B = 30,lam =0): 
    #initialization, and extraction of the number of observations and variables
    #to add the if
    lam = np.ones(data.shape[1])
    nobs = data.shape[0]
