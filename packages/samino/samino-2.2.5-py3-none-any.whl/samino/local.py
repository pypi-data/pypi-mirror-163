...
class Local:
    ...
    def get_member_followers(self, userId: str = None, start: int = 0, size: int = 25):
        req = self.session.get(api(f"/x{self.comId}/s/user-profile/{userId}/member?start={start}&size={size}"), headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return UserProfileList(req.json()["userProfileList"]).UserProfileList
    ...
    def send_message(self, chatId: str, message: str = None, messageType: int = 0, file: BinaryIO = None, fileType: str = None, replyTo: str = None, mentionUserIds: list = None, stickerId: str = None, snippetLink: str = None, ytVideo: str = None, snippetImage: BinaryIO = None, embedId: str = None, embedType: int = None, embedLink: str = None, embedTitle: str = None, embedContent: str = None, embedImage: BinaryIO = None):
        ...
        if snippetLink and snippetImage:
            ...
            print(f"\n\n{data['extensions']['linkSnippetList'][0]['mediaUploadValue']}")
    ...
    def chat_settings(self, chatId: str, viewOnly: bool = False, doNotDisturb: bool = True, canInvite: bool = False, canTip: bool = None, pin: bool = None, coHosts: Union[str, list] = None):
        res = []
    ...
    def get_blog_comments(self, wikiId: str = None, blogId: str = None, quizId: str = None, sorting: str = 'newest', size: int = 25, start: int = 0):
        sorting = sorting.lower()
        if sorting == 'top': sorting = "vote"
        if sorting not in ["newest", "oldest", "vote"]: raise TypeError("حط تايب يا حمار")
        ...
        req = self.session.get(url, headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return CommentList(req.json()["commentList"]).CommentList
    ...
    def invite_by_host(self, chatId: str, userId: Union[str, list]):
        req = self.session.post(api(f"/x{self.comId}/s/chat/thread/{chatId}/member/{userId}/invite-av-chat"), headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return Json(req.json())
    ...
    def post_wiki(self, title: str, content: str, fansOnly: bool = False, icon: str = None, backgroundColor: str = None, keywords: Union[str, list] = None):
        data = {
    ...
    def edit_blog(self, title: str, content: str, blogId: str = None, wikiId: str = None, fansOnly: bool = False, backgroundColor: str = None, media: list = None):
        data = {
            "title": title,
            "content": content,
            "timestamp": int(timestamp() * 1000)
    ...
    def get_quizes(self, type: str = "recent", start: int = 0, size: int = 25):
        ...
        req = self.session.get(url, headers=self.headers, proxies=self.proxies)
        if req.status_code != 200:return CheckExceptions(req.json())
        else:return BlogList(req.json()["blogList"]).BlogList

    def get_quiz_questions(self, quizId: str):
        req = self.session.get(api(f"/x{self.comId}/s/blog/{quizId}?action=review"), headers=self.headers,proxies=self.proxies)
        if req.status_code != 200:return CheckExceptions(req.json())
        else:return QuizQuestionList(req.json()["blog"]["quizQuestionList"]).QuizQuestionList
    ...
    def play_quiz(self, quizId: str, questions: list, answers: list, mode: int = 0):
        ...
        req = self.session.post(api(f"/x{self.comId}/s/blog/{quizId}/quiz/result"),headers=headers.Headers(data=data).headers, proxies=self.proxies, data=data)
        if req.status_code != 200:return CheckExceptions(req.json())
        else:return req.status_code

    def get_quiz_rankings(self, quizId: str, start: int = 0, size: int = 25):
        req = self.session.get(api(f"/x{self.comId}/s/blog/{quizId}/quiz/result?start={start}&size={size}"),headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else:return QuizRankings(req.json()).QuizRankings

    def search_user(self, username: str, start: int = 0, size: int = 25):
        req = self.session.get(api(f"/x{self.comId}/s/user-profile?type=name&q={username}&start={start}&size={size}"), headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return UserProfileList(req.json()["userProfileList"]).UserProfileList

    def search_blog(self, words: str, start: int = 0, size: int = 25):
        req = self.session.get(api(f"/x{self.comId}/s/blog?type=keywords&q={words}&start={start}&size={size}"), headers=self.headers, proxies=self.proxies)
        if req.status_code != 200: return CheckExceptions(req.json())
        else: return BlogList(req.json()["blogList"]).BlogList