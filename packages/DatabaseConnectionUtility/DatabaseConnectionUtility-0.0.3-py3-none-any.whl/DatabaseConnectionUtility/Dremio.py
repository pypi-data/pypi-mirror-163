...
class Dremio:
    def getConnection(self, dbDetails):   
        logger.log(f'inside dremio1 getConnection','0')
        uid  = ""
        pwd  = ""
        url  = ""
        pool = None
        ...
        logger.log(f'dbDetails inMem1 class: {dbDetails}', '0' )
        try:
            ...
            if pool != None:
                logger.log(f'Connected to Dremio1 DB.','0')
        
        except Exception as e:
            logger.log(f'Issue in dremio1 connection.{e}','0')
            raise e
