...
class Client:
    ...
    def _call(self, called_method_name: str, *args, **kwargs):
        ...
        # The s3.download_fileobj is a special case where the data is written to the
        # Fileobj argument and not returned via an output argument.
        if self._service_name == "s3" and called_method_name == "download_fileobj":
            buffer: io.BytesIO = request["Fileobj"]
            file_obj_data: typing.Union[str, bytes, None] = response.pop(
                "Fileobj", None
            )
            if isinstance(file_obj_data, str):
                file_obj_data = file_obj_data.encode()
            if file_obj_data:
                buffer.write(file_obj_data)
