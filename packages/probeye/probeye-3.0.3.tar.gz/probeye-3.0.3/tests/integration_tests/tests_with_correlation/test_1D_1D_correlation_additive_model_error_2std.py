...
----------------------------------------------------------------------------------------
      ---> Additive model prediction error (individual std. dev. for sensors) <---
----------------------------------------------------------------------------------------
...
to estimate the bridge's bending stiffness 'EI'. Next to 'EI' there are four other
parameters to infer: the additive model error standard deviations 'sigma_1', 'sigma_2'
(one for each of the two sensors), the temporal correlation length 'l_corr_t' and the
spatial correlation length l_corr_x. Hence, five parameters in total, all of which are
inferred in this example using a maximum likelihood estimation.
"""
...
class TestProblem(unittest.TestCase):
    def test_1D_1D_correlation_additive_model_error_2std(
        self,
        n_steps: int = 200,
        ...
        # 'true' value of add. model error sd (sensor 1), and its uniform prior params
        sigma_1 = 1e-3
        low_sigma_1 = 0.0
        high_sigma_1 = 1e-2

        # 'true' value of add. model error sd (sensor 2), and its uniform prior params
        sigma_2 = 1e-3
        low_sigma_2 = 0.0
        high_sigma_2 = 1e-2
        ...
        problem = InverseProblem(
            "Simply supported beam with time-space correlation (1D-1D, AME, 2std)",
            log_level="DEBUG",
        )
        ...
        problem.add_parameter(
            name="L", tex="$L$", info="Length of the beam [m]", value=L
        )
        problem.add_parameter(
            name="sigma_1",
            domain="(0, +oo)",
            tex=r"$\sigma$",
            info="Standard deviation of zero-mean model error for sensor 1",
        ...
        problem.add_parameter(
            ...
            domain="(0, +oo)",
            tex=r"$\sigma$",
            info="Standard deviation of zero-mean model error for sensor 2",
        ...
        for exp_name, exp_dict in experiments_def.items():
            ...
            cov_compiler.add_measurement_space_points(
                ...
                standard_deviation=np.array([sigma_1, sigma_2]),
                group="space",
            )
...
class BeamModel(ForwardModelBase):
    def interface(self):
        ...
        self.output_sensors = [
            Sensor(name="y1", x=30.0, std_model="sigma_1"),
            Sensor(name="y2", x=35.0, std_model="sigma_2"),
        ]
