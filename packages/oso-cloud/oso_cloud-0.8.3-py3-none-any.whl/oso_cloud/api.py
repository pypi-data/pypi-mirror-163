...
class Variable:
    kind: str
    value: Optional[Union[TypedId, TypedVar]]
...
class API:
    ...
    def get_facts(self, predicate, *args):
        ...
        for i, arg in enumerate(args):
            if arg.kind == "TypedVariable":
                params[f"args.{i}.type"] = arg.value.type
            elif arg.kind == "TypedId":
                params[f"args.{i}.type"] = arg.value.type
                params[f"args.{i}.id"] = arg.value.id
        json = None
        result = self._do_get(
    ...
    def post_query(self, data):
        def parse_var(var_dict) -> Variable:
            kind = var_dict["kind"]
            if kind == "TypedId":
                return Variable(kind="TypedId", value=TypedId(**var_dict["value"]))
            if kind == "TypedVariable":
                return Variable(
                    kind="TypedVariable", value=TypedVar(**var_dict["value"])
                )
            if kind == "FreeVariable":
                return Variable(kind="FreeVariable", value=None)
        ...
        response = self._handle_result(result)
        # TODO: Better codegen for this nested stuff. Not really a problem
        # in the other languages, just python.
        return QueryResult(
            [
                VariableFact(
                    fact["predicate"], [parse_var(arg) for arg in fact["args"]]
                )
