...
class LebaiRobot:
    ...
    def run_scene(self, scene_id: int, execute_count: int = 1, clear: bool = True, timeout: int = 0) -> TaskStatus:
        """
        运行场景
        ...
        :return: 任务Id
        """
        taks_id = self.http_service.run_scene(scene_id, execute_count, clear)['id']
        t0 = time.time()
        while time.time() - t0 < timeout:
            task_info = self.http_service.get_task(taks_id)
