...
def train(network, epoch, train_loader, it, optimizer, lr, lr_loss, lr_scheduler):
  ...
  return network, it, optimizer, lr, lr_loss, lr_scheduler
...
def lr_range_finder(network, train_loader):
  ...
  for epoch in range(1, n_epochs+1):
    network, it, optimizer, lr, lr_loss, lr_scheduler = train(network, epoch, train_loader, it, optimizer, lr, lr_loss, lr_scheduler)
    if len(it)>1000:
      break
