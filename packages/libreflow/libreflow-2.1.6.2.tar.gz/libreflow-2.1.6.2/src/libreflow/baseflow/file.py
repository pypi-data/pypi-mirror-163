...
bpy.ops.lfs.playblast(do_render=True, filepath='%s', studio='%s', project='%s', sequence='s%04i', scene='%s', do_simplify=%s, do_reduce_textures=%s, target_texture_width=%s, resolution_percentage=%s, version='%s')""" % (
    ...
    def run(self, button):
        ...
        # Set last revision oid in playblast file
        playblast_file.ensure_last_revision_oid()
...
class PlayblastResolutionChoiceValue(flow.values.SessionValue):
    ...
    def choices(self):
        site = self.root().project().get_current_site()
        return site.ae_render_settings_templates.keys()
    ...
    def revert_to_default(self):
        ...
        if value in self.choices():
            ...
            site = self.root().project().get_current_site()
            keys = site.ae_render_settings_templates.keys()
            if keys:
                self.set(keys[0])
...
class PublishAndRenderAEPlayblast(PublishAndRenderPlayblast):
    ...
    render_blender_playblast = flow.Label('<h2>Playblast</h2>')
    resolution = flow.SessionParam(None, PlayblastResolutionChoiceValue)
    render_in_pool = flow.SessionParam(False, PresetSessionValue).ui(
        tooltip='Submit playblast rendering in a job pool',
        editor='bool'
    )
    ...
    def get_buttons(self):
        self.resolution.revert_to_default()
        return super(PublishAndRenderAEPlayblast, self).get_buttons()
    ...
    def _configure_and_render(self, revision_name):
        ...
        render_select_mode.resolution.set(self.resolution.get())
...
class SubmitRenderAEPlayblast(flow.Action):
    ...
    def run(self, button):
        ...
        render_folder = self._ensure_file('compositing_render')
        mov_file = self._ensure_file('compositing_movie.mov')
...
class SelectAEPlayblastRenderMode(flow.Action):
    ...
    def allow_context(self, context):
        ...
        return (
            ...
            and site.ae_render_settings_templates.len() > 0
            and site.ae_output_module_templates.has_key('default')
            and site.ae_output_module_audio.get() is not None
            and not self._file.is_empty()
        )
    ...
    def run(self, button):
        if button == 'Cancel':
            ...
            site = self.root().project().get_current_site()
            render_settings = site.ae_render_settings_templates.get_key(
                self.resolution.get()
            )
            output_module = site.ae_output_module_templates.get_key('default')
