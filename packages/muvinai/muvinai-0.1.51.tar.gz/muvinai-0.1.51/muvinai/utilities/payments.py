            ...
            try:
                for discount in [i for i in client["discounts"] if i["aplicaciones_restantes"] > 0]:
                    if "concepto" in discount.keys():
                        ...
                        concepto = "n/a"
                    prices["Descuento " + concepto] = round(- (plan_price * discount["porcentaje"]) / 100 - discount["monto_absoluto"], 2)
            except:
...
def process_payment(payment_data: dict, card_data: dict, _client: dict, _plan: dict, sdk: mercadopago.SDK) -> dict:
    ...
    if "poi" in _client.keys():
        ...
        subscription_sequence = str(_client["poi"]["payment_reference"])
    ...
    db = init_mongo()
    merchant = db.merchants.find_one(payment_data['merchant_id'])
    ...
    point_of_interaction["transaction_data"].update({
        ...
        "subscription_id": f"{merchant['keys']['application_id']}-{_client['documento']}"
    })
    point_of_interaction["type"] = "SUBSCRIPTIONS"

    if "seller_fee" in payment_data.keys():
        if 100 > payment_data["seller_fee"] > 0:
            merchant_cadena = db.merchants.find_one(ObjectId('6178652dcfb117ad58a2cd3d'))
            point_of_interaction["transaction_data"]['subscription_id'] = f"{merchant_cadena['keys']['application_id']}-{_client['documento']}"
...
def update_client(_client: dict, _payment_result: dict, _plan: dict, period: str, update_npd: bool = False):
    ...
    if _payment_result["status"] == "approved":
        ...
        if _payment_result["id"] > 400:  # es un pago de mercadopago
            if "poi" in _client.keys():
