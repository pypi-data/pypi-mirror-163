...
class SquareCase(MeshBase):
    ...
    def extract_horizontal_line_data(self, y_coordinate, result, error=1e-6):
        """
        ...
        for i in range(self._cell_num):
            if abs(self._cell_center[i, 1] - y) < error:
                data.append(i)
    ...
    def extract_vertical_line_data(self, x_coordinate, result, error=1e-6):
        """
        ...
        data = []
        for i in range(self._cell_num):
            if abs(self._cell_center[i, 0] - x) < error:
                data.append(i)
