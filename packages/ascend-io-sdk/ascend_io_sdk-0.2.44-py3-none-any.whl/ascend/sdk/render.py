...
def _inline_code_list_for(component: Component, resource_path_prefix: str, base_path: str) -> Optional[List[InlineCode]]:
  ...
  # for custom blob write
    ...
    if component.container:
      ...
      write_strategy_value = value_from_proto(details.get('write_strategy', UnionValue()))
    ...
    if component.container:
      if component.container.HasField("byte_function") and component.container.byte_function.container.executable.code.source.HasField("inline"):
        ...
        if code_interface:
          ...
          code = code_interface_value.get("bytes", dict()).get("code_fingerprint_strategy", UnionValue()).get("content_based_fingerprint", {}).get("code")
          if code:
            icl.append(
                InlineCode(
                    code=code,
                    resource_path=os.path.join(resource_path_prefix, f"{component.id}_custom_code.py"),  # noqa: E126
                    base_path=base_path,
                    attribute_path=('container', 'record_connection', 'details', 'code_interface', 'bytes', 'code_fingerprint_strategy',
