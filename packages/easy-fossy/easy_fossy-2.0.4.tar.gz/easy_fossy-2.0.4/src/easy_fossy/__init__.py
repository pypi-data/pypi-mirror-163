...
class easy_fossy:
    ...
    def __init__(self, config_file: str, server_to_use: str = "test"):
        ...
        self.token_expire_yyyy_mm_dd = self.today + \
            datetime.timedelta(days=self.config.getint('token_valdity_days'))

        self.bearer_token = self.config.get('bearer_token')
        if self.token_expire:
            token_expire_datetime = datetime.date.fromisoformat(
                self.token_expire)
        else:
        ...
        if(not self.bearer_token or not token_expire_datetime or (self.today > token_expire_datetime)):
    ...
    def get_token_by_uname_pwd(self) -> str:
        ...
        payload = {
            ...
            "token_name": str('created_viaapi_on_')+str(self.now.strftime(self.dt_format)),
    ...
    def get_job_info_by_upload_id(self, upload_id: int) -> Job:
        ...
        response = requests.request(
            "GET", self.url+str(f'jobs?upload={upload_id}'), data=payload, headers=headers)
    ...
    def generate_and_get_desired_report_for_uploadid(self, upload_id: int, report_format: ReportFormat):
        ...
        timer = 0
        response = requests.request(
            "GET", self.url+str('report/')+str(report_id), data=payload, headers=headers)
        while response.status_code != 200:

            print(f'waiting for {timewait} sec')
            time.sleep(timewait)
            response = requests.request(
                "GET", self.url+str(f'report/{report_id}'), data=payload, headers=headers)
            if timer > timeout:
                break
    ...
    def get_folder_info_by_id(self, folder_id: int) -> Folder:
        ...
        response = requests.request(
            "GET", self.url+str('folders/')+str(folder_id), data=payload, headers=headers)
    ...
    def change_folder_name_or_desc(self, folder_id: int, new_folder_name: str = '', new_folder_desc: str = ''):
        ...
        response = requests.request(
            "PATCH", self.url+str(f'folders/{folder_id}'), data=payload, headers=headers)
        match response.json():
            case {**info}:
                report_info = Info(**info)
                print(report_info.message)
    ...
    def create_folder_under_parent_folder_id(self, parent_folder_id: int, folder_name: str) -> Info:
    ...
    def delete_folder_by_id(self, folder_id: int):
        ...
        response = requests.request(
            "DELETE", self.url+str('folders/{folder_id}'), data=payload, headers=headers)
    ...
    def apply_action_to_folderid(self, actions: Action, folder_id: int, parent_folder_id: int) -> Info:
    ...
    def get_upload_summary_for_uploadid(self, upload_id: int) -> UploadSummary:
        ...
        response = requests.request(
            "GET", self.url+str(f'uploads/{upload_id}/summary'), data=payload, headers=headers)
        match response.json():
            case {**info}:
    ...
    def get_all_uploads_based_on(self, folder_id: int, is_recursive: bool, search_pattern_key: str, upload_status: ClearingStatus, assignee: str, since_yyyy_mm_dd: str, page: int, limit: int) -> List[Upload]:
        ...
        response = requests.request(
            "GET", self.url+str('uploads'), data=payload, headers=headers, params=querystring)
    ...
    def get_licenses_found_by_agents_for_uploadid(self, upload_id: int, agents: List[str], show_directories: bool) -> UploadLicenses | Info:
        ...
        response = requests.request(
            "GET", self.url+str(f'uploads/{upload_id}/licenses'), data=payload, headers=headers, params=querystring)
    ...
    def get_upload_id_by_local_package_upload(self, file_path: str, folder_id: int, upload_desc: str, visibility: Public) -> str:
        ...
        response = requests.post(
            self.url+str('uploads'), data=m, headers=headers)
    ...
    def get_upload_id_by_download_url_package_upload(self, file_download_url: str, file_name: str, folder_id: int, upload_desc: str, visibility: Public) -> str:
        ...
        m = MultipartEncoder(
            fields={'url': file_download_url, 'name': file_name})
        headers = {
            # "accept": "application/json",
            "folderId": str(folder_id),
            "groupName": self.group_name,
    ...
    def get_upload_id_by_giturl_package_upload(self, git_url: str, branch_name: str, upload_name: str, folder_id: int, upload_desc: str, visibility: Public) -> str:
    ...
    def trigger_analysis_for_upload_package(self, file_path: str, folder_id: int):
        ...
        uploads: List[Upload] = self.get_all_uploads_based_on(folder_id=folder_id, is_recursive=True,
                                                              search_pattern_key='', upload_status=ClearingStatus.Open, assignee='', since_yyyy_mm_dd='', page=1, limit=1000, group_name=self.group_name)
        file_name = file_path.split('/').pop()
    ...
    def trigger_analysis_for_url_upload_package(self, file_download_url: str, file_name: str, branch_name: str, folder_id: int):
    ...
    def trigger_analysis_for_git_upload_package(self, git_url: str, branch_name: str, folder_id: int):
        ...
        if size > 1:
            ...
            # no upload_id is there
            upload_id = self.get_upload_id_by_giturl_package_upload(git_url=git_url, branch_name=branch_name, upload_name=file_name,
                                                                    folder_id=1, upload_desc='', visibility=Public.public, group_name=self.group_name)
    ...
    def delete_uploads_by_upload_id(self, upload_id: int) -> Info:
        ...
        response = requests.request(
            "DELETE", self.url+str(f'uploads/{upload_id}'), data=payload, headers=headers)
    ...
    def get_all_license_based_on(self, is_active: str, license_kind: Kind, page: int, limit: int) -> List[License]:
        """
        ...
        response = requests.request(
            "GET", self.url+str('license'), data=payload, headers=headers, params=querystring)
    ...
    def get_all_license_short_names_based_on(self, is_active: str, license_kind: Kind, page: int, contains_key: str, limit: int) -> List[str]:
        """
        ...
        licenses = self.get_all_license_based_on(
            is_active=is_active, license_kind=license_kind, page=page, limit=limit, group_name=self.group_name)
        license_shortnames = [
    ...
    def get_license_by_short_name(self, short_name: str) -> LicenseShortnameGetResponse:
        ...
        response = requests.request(
            "GET", self.url+str(f'license/{short_name}'), data=payload, headers=headers)
        match response.json():
    ...
    def update_license_info_by_short_name(self, short_name: str, new_full_name: str, new_license_text: str, new_url: str, new_risk: int) -> Info:
        ...
        response = requests.request(
            "PATCH", self.url+str(f'license/{short_name}'), json=payload, headers=headers)
    ...
    def add_new_license(self, unique_short_name: str, new_full_name: str, new_license_text: str, new_url: str, new_risk: int, isCandidate: bool, merge_request: bool) -> Info:
