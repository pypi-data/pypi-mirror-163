...
class BaseEventRepository(BaseEventRepositoryAbstract):
    ...
    def get_first_event_to_send(self) -> Optional[BaseEventModel]:
        query: Query = self.db.query(self.MODEL) \
            ...
            .filter(self.MODEL.retry_at >= datetime.utcnow()) \
    ...
    def negative_ack(self, topic: str, event_to_send: BaseEvent) -> BaseEventModel:
        ...
        event_to_send.retry_at = int(time())
        event_to_send.retry_at = event_to_send.retry_at + (event_to_send.retry * 2)
    ...
    def _send_event(self, topic: str, event_to_send: BaseEvent) -> BaseEventModel:
        ...
        event.retry_at = None if event_to_send.retry_at is None else datetime.fromtimestamp(event_to_send.retry_at)
        event.sent_at = None
        self.db.add(event)
        return event
