...
class HCol(object):
	...
	def __cmp__(self, other):
		return isinstance(other, HCol) and cmp(self.name, other.name)
	...
	def add(self, name, val = None, unit = None):
		...
		if val == None:
			...
			if isinstance(val, datetime.datetime):
				try:
					val = HDateTime.make_from_dt(val, val.tzinfo.zone.split('/')[-1])
				except:
					val = HDateTime.make_from_dt(val)
				...
				self[name] = HStr(val.decode('utf-8'))
...
class HRow(HDict):
	...
	def get(self, col):
		...
		if isinstance(col, HCol):
			...
			#print('GRID.PY get by index:',ret)
			ret = self.get(col.name)
			#print('GRID.PY get by name :',ret)
		else:
			#print('GRID.PY get type',type(col),col)
			name = col
