...
import string
from cxhs.grid import HGridBuilder, HDictBuilder, HCol
from cxhs.val import (
    HMarker, HBool, HStr, HNum, HDate, HRef, HTime, HTimeZone, HDateTime, HUri, is_id_char
)
...
def _dict_sort(d):
	'''
	Custom sorter for tags in a row ('id' first, 'mod' last, all others in alphabetical
	order.
	'''
	current_keys = list(d.keys())
	keyorder = _list_sort(current_keys)
	return {k: d[k] for k in keyorder if k in d}

def _list_sort(ll):
	''' sort a List of Str (column names) or List of HCol
	Needs to return a List of the same type
	Currently does not return a List of HCol
	'''
	l = list(ll)
	# convert HCol to str
	if l and isinstance(l[0], HCol):
		l = [c.dis for c in l]
	# define columns fixed in order
	fixed = ['id', 'createdBy', 'mod']
	# remove fixed columns from temporary List "l"
	for tag in fixed:
		try:
			l.remove(tag)
		except:
	...
	first = []
	last = []
	if ll and isinstance(ll[0], HCol):
		if HCol(0,'id',{}) in ll:
			first.append('id')
		if HCol(0,'createdBy',{}) in ll:
			last.append('createdBy')
		if HCol(0,'mod',{}) in ll:
			last.append('mod')
	else:
		if 'id' in ll:
		...
		if 'mod' in ll:
			last.append('mod')
	keyorder = first + sorted(l) + last
	#print("IO.PY",first,sorted(l),last)
	return keyorder
...
class JsonWriter(object):
	...
	def writeGrid(self, grid):
		...
		for row in grid.rows:
			...
			rows.append( _dict_sort(json_row) )
	...
	def _writeDictTags(self, meta, meta_dict):
		for name, val in meta.items():
			#meta_dict[name] = val.__str__()
			if val != None:
				meta_dict[name] = val.to_json() #val.__str__()
...
class ZincWriter(object):
	...
	def writeGrid(self, grid):
		#self.out.write('ver:"%s"' % grid.ver)
		self.out.write('ver:"2.0"')
		#print("IO.PY grid.meta",list(grid.meta.keys()))
		self._writeMeta(grid.meta)
		self.out.write('\n')

		# cols
		control = grid.cols
		# put some prints in log
		#_list_sort(grid.cols)
		#control = _list_sort(grid.cols)

		for i, col in enumerate(control):
			if i > 0:
				self.out.write(",")
			self._writeCol(col)
	...
	def _writeRow(self, grid, row):
		# sort column names
		#print("IO.PY grid.cols type",type(grid.cols))
		#if grid.cols and isinstance(grid.cols, list):
		#	print('\telem type',type(grid.cols[0]))
		#print([(c.index,c.name) for c in grid.cols])
		control = grid.cols
		#control = _list_sort(grid.cols)

		for i, col in enumerate(control):
			val = row.get(col)
			#print("IO.PY col:%s val:%s" % (col.name,val))
			if i > 0:
				self.out.write(',')
			if val == None:
				if i == 0:
...
class ZincReader(object):
	...
	def __init__(self, s):
		...
		if not isinstance(s, str):
			try:
				...
				raise ValueError("Only str can be read via ZincReader")
		#
	...
	def _init_grid_read(self, grid_number = 0):
		self.stream = StringIO(self.grid_strings[grid_number])
		self.lineNum = 1
		self.version = None
		self.cur = chr(0)
		self.peek = chr(0)
	...
	def readGrid(self, grid_index=0):
		...
		# find cols passed in through requests
		numCols = 0
		while True:
			name = self._readId()
			self._skipSpace()
		...
		if date != None:
			...
			if self.cur != ' ':
				if not zutc:
					...
					tz = HTimeZone.UTC
			elif zutc and self.peek != None and not (self.peek >= 'A' and self.peek <= 'Z' ):
				tz = HTimeZone.UTC
			else:
				self._consume()
	...
	def _consumeNewLine(self):
		nextUp = (self.cur, self.peek)
		if self.cur != "\n":
			self._err("Expecting newline %s|%s" % nextUp)
		self._consume()
