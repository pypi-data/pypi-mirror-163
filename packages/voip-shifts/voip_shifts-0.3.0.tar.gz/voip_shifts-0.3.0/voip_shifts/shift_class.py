...
class Shifts():
    def __init__(self, employee_id:str, client_id:str, service_id:str, 
                ...
                active:bool, skip:bool, number:str, destination_phone:str) -> None:
    ...
    def shift_call(self, type_of_call) -> None:
        ...
        if Config.MODE == "prod":
            ...
            log.info(f"from: {self.number}")
            log.info(f"To: {self.destination_phone}")
        else:
            log.info(f"""./call-prog, -r {self.sip_server} -u {self.username} -p {self.password} -n \
                    {self.number} -t {self.destination_phone}@{self.sip_server} -d \
