...
class LicenseNotFoundError(ValueError):
    def __init__(self, name) -> None:
        super().__init__(f'License with spdx_id or full_name "{name}" is not available')


def MultipleLicensesWarning(name):
    warnings.warn(
        "Multiple licenses with spdx_id or full_name {} are available, returning a List".format(
            name
        ),
        RuntimeWarning,
    )
