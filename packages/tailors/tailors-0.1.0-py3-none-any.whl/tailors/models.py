...
class TailorsIO(abc.ABC):
    ...
    def encode(text: str,
        ...
        encoder = tokenizer.encode_plus if isinstance(text, str) else tokenizer.prepare_for_model
        encoded = encoder(
            text,
        ...
        if return_tensors:
            ...
            offset_mapping = offset_mapping.tolist() if offset_mapping is not None else None

        return input_ids, attention_mask, token_type_ids, offset_mapping

    @staticmethod
    def build_id_tag_mapping(tags: list):
        tags = hao.lists.uniquify(tags + ['O'])
        id2tag = {i: tag for i, tag in enumerate(tags)}
        tag2id = {tag: i for i, tag in enumerate(tags)}
        return id2tag, tag2id, len(tags)

    def build_special_tags(self):
        return {}, {}
    ...
    @abc.abstractmethod
    def for_inference(self, lines: List[str], bz: int = 32):
        raise NotImplementedError()

    @abc.abstractmethod
    def for_eval(self, lines: Iterator[str], bz: int = 32):
        raise NotImplementedError()
...
class Tailors(nn.Module, abc.ABC):
    ...
    def get_io(self):
        ...
        try:
            ...
            raise TailorsError(f"[io] expecting `{class_name}` in module: `{module_name}`")
        LOGGER.info(f"[io] {io_class}")
    ...
    def export_to_onnx(self, output_path):
        torch.onnx.export(
            ...
            dynamic_axes={
                'input_ids': {0: 'batch'},  # 第0维是batch dimension
                'attention_mask': {0: 'batch'},  # 第0维是batch dimension
                'token_type_id': {0: 'batch'},  # 第0维是batch dimension
                'tag': {0: 'batch'},
                'score': {0: 'batch'},
            },
            export_params=True,
