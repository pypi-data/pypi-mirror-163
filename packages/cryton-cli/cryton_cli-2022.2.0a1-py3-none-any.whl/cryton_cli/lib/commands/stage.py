...
@click.pass_obj
def stage_list(ctx: util.CliContext, less: bool, offset: int, limit: int, localize: bool, parent: int, 
    ...
    response = api.get_request(ctx.api_url, api.STAGE_LIST + appendix, custom_params=custom_params)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def stage_read(ctx: util.CliContext, stage_id: int, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.STAGE_READ, stage_id)
...
@click.argument('stage_id', type=click.INT, required=True)
@click.pass_obj
def stage_delete(ctx: util.CliContext, stage_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.delete_request(ctx.api_url, api.STAGE_DELETE, stage_id)

    util.echo_msg(response, 'Stage successfully deleted!', ctx.debug, ctx.parse)
...
@click.option('-i', '--inventory', type=click.Path(exists=True), help='Path to an inventory file containing variables.',
              multiple=True)
@click.pass_obj
def stage_validate(ctx: util.CliContext, file: str, inventory: tuple) -> None:
    """
    ...
    :return: None
    """
    stage_yaml = util.get_yaml_from_file(file, inventory)
    response = api.post_request(ctx.api_url, api.STAGE_VALIDATE, custom_dict=stage_yaml)

    util.echo_msg(response, 'Stage successfully validated!', ctx.debug, ctx.parse)
...
@click.option('-f', '--filter', 'parameter_filters', type=click.STRING, multiple=True,
              help='Filter results using returned parameters (for example `id=1`, `name=test`, etc.).')
@click.pass_obj
def stage_execution_list(ctx: util.CliContext, less: bool, offset: int, limit: int, localize: bool, parent: int, 
                         parameter_filters: Optional[List[str]]) -> None:
    """
    ...
    response = api.get_request(ctx.api_url, api.STAGE_EXECUTION_LIST + appendix, custom_params=custom_params)
...
@click.pass_obj
def stage_execution_delete(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.delete_request(ctx.api_url, api.STAGE_EXECUTION_DELETE, execution_id)

    util.echo_msg(response, 'Stage execution successfully deleted!', ctx.debug, ctx.parse)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def stage_execution_read(ctx: util.CliContext, execution_id: int, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.STAGE_EXECUTION_READ, execution_id)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def stage_execution_report(ctx: util.CliContext, execution_id: int, file: str, less: bool,
                           localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.STAGE_EXECUTION_REPORT, execution_id)

    util.get_report(response, file, 'stage-execution_{}'.format(execution_id), less, less, localize, ctx.debug, ctx.parse)


@stage_execution.command('kill')
@click.argument('execution_id', type=click.INT, required=True)
@click.pass_obj
def stage_execution_kill(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.STAGE_EXECUTION_KILL, execution_id)

    util.echo_msg(response, 'Stage execution successfully killed!', ctx.debug, ctx.parse)
...
@click.pass_obj
def stage_execution_re_execute(ctx: util.CliContext, execution_id: int, immediately: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.STAGE_EXECUTION_RE_EXECUTE, execution_id, {"immediately": immediately})

    util.echo_msg(response, 'Stage execution successfully re-executed!', ctx.debug, ctx.parse)
