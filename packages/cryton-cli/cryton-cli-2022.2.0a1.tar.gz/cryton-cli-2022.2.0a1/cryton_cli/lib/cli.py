...
@click.option('--parse', is_flag=True, help='Show non formatted json output (use only with the `--debug` flag).')
@click.version_option(message=f"%(prog)s, version %(version)s")
@click.pass_context
def cli(ctx: click.Context, host: str, port: int, secure: bool, debug: bool, parse: bool) -> None:
    """
    ...
    :return: None
    """
    ctx.obj = util.CliContext(host, port, secure, debug, parse)
