...
@click.pass_obj
def plan_list(ctx: util.CliContext, less: bool, offset: int, limit: int, localize: bool, 
    ...
    response = api.get_request(ctx.api_url, api.PLAN_LIST + appendix)

    to_print = ['id', 'name', 'owner', 'evidence_dir']
    util.echo_list(response, to_print, less, localize, ctx.debug, ctx.parse)


@plan.command('create')
@click.argument('template_id', type=click.INT, required=True)
@click.option('-i', '--inventory-file', type=click.Path(exists=True), multiple=True,
              help="Inventory file used to fill the template. Can be used multiple times.")
@click.pass_obj
def plan_create(ctx: util.CliContext, template_id: int, inventory_file: list) -> None:
    """
    ...
    :param inventory_file: Inventory file(s) used to fill the template
    :return: None
    """
    files = {}
    if inventory_file:
        for i in range(len(inventory_file)):
            files.update({f"{i}": open(inventory_file[i], "rb")})
    data = {'template_id': template_id}

    response = api.post_request(ctx.api_url, api.PLAN_CREATE, files=files, data=data)
    util.echo_msg(response, 'Plan Instance successfully created!', ctx.debug, ctx.parse)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def plan_read(ctx: util.CliContext, plan_id: int, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.PLAN_READ, plan_id)
...
@click.argument('plan_id', type=click.INT, required=True)
@click.pass_obj
def plan_delete(ctx: util.CliContext, plan_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.delete_request(ctx.api_url, api.PLAN_DELETE, plan_id)

    util.echo_msg(response, 'Plan successfully deleted!', ctx.debug, ctx.parse)
...
@click.option('-i', '--inventory', type=click.Path(exists=True), help='Path to an inventory file containing variables.',
              multiple=True)
@click.pass_obj
def plan_validate(ctx: util.CliContext, file: str, inventory: tuple) -> None:
    """
    ...
    :return: None
    """
    plan_yaml = util.get_yaml_from_file(file, inventory)
    response = api.post_request(ctx.api_url, api.PLAN_VALIDATE, custom_dict=plan_yaml)

    util.echo_msg(response, 'Plan successfully validated!', ctx.debug, ctx.parse)
...
@click.pass_obj
def plan_execute(ctx: util.CliContext, plan_id: int, worker_id: int, run_id: int) -> None:
    """
    ...
    response = api.post_request(ctx.api_url, api.PLAN_EXECUTE, plan_id, arguments)

    util.echo_msg(response, 'Plan successfully executed!', ctx.debug, ctx.parse)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def plan_get_plan(ctx: util.CliContext, plan_id: int, file: str, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.PLAN_GET_PLAN, plan_id)

    util.get_report(response, file, 'plan_{}'.format(plan_id), less, less, localize, ctx.debug, ctx.parse)
...
@click.option('-f', '--filter', 'parameter_filters', type=click.STRING, multiple=True,
              help='Filter results using returned parameters (for example `id=1`, `name=test`, etc.).')
@click.pass_obj
def plan_execution_list(ctx: util.CliContext, less: bool, offset: int, limit: int, localize: bool, parent: int, 
                        parameter_filters: Optional[List[str]]) -> None:
    """
    ...
    response = api.get_request(ctx.api_url, api.PLAN_EXECUTION_LIST + appendix, custom_params=custom_params)
...
@click.pass_obj
def plan_execution_delete(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.delete_request(ctx.api_url, api.PLAN_EXECUTION_DELETE, execution_id)

    util.echo_msg(response, 'Plan execution successfully deleted!', ctx.debug, ctx.parse)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def plan_execution_read(ctx: util.CliContext, execution_id: int, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.PLAN_EXECUTION_READ, execution_id)
...
@click.argument('execution_id', type=click.INT, required=True)
@click.pass_obj
def plan_execution_pause(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.PLAN_EXECUTION_PAUSE, execution_id)

    util.echo_msg(response, 'Plan execution successfully paused!', ctx.debug, ctx.parse)
...
@click.option('--localize', is_flag=True, help='Convert UTC datetime to local timezone.')
@click.pass_obj
def plan_execution_report(ctx: util.CliContext, execution_id: int, file: str, less: bool, localize: bool) -> None:
    """
    ...
    :return: None
    """
    response = api.get_request(ctx.api_url, api.PLAN_EXECUTION_REPORT, execution_id)

    util.get_report(response, file, 'plan-execution_{}'.format(execution_id), less, less, localize, ctx.debug, ctx.parse)


@plan_execution.command('resume')
@click.argument('execution_id', type=click.INT, required=True)
@click.pass_obj
def plan_execution_unpause(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.PLAN_EXECUTION_UNPAUSE, execution_id)

    util.echo_msg(response, 'Plan execution successfully resumed!', ctx.debug, ctx.parse)


@plan_execution.command('validate-modules')
@click.argument('execution_id', type=click.INT, required=True)
@click.pass_obj
def plan_execution_validate_modules(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.PLAN_EXECUTION_VALIDATE_MODULES, execution_id)

    util.echo_msg(response, 'Modules were validated!', ctx.debug, ctx.parse)


@plan_execution.command('kill')
@click.argument('execution_id', type=click.INT, required=True)
@click.pass_obj
def plan_execution_kill(ctx: util.CliContext, execution_id: int) -> None:
    """
    ...
    :return: None
    """
    response = api.post_request(ctx.api_url, api.PLAN_EXECUTION_KILL, execution_id)

    util.echo_msg(response, 'Plan execution successfully killed!', ctx.debug, ctx.parse)
