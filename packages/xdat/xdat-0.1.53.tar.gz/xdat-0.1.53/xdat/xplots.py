...
from . import xproblem, xpd, xsettings
...
def plot_multi(df, kind='scatter', plot_func=None, x=None, y=None, plot_on=None, group_on=None, color_on=None, label_on=None, style_on=None, figsize=(10,6), alpha=1.0, hdi_probs=(0.1, 0.25, 0.5, 0.8), color_dict=None, title='', x_axis_type=None, legend_loc='best', xlim=None, ylim=None, save_to=None, clear_folder=False, add_date=True, **kwargs):
    ...
    if isinstance(color_on, str):
        color_on = [color_on]

    if not style_on:
        style_on = []

    if isinstance(style_on, str):
        style_on = [style_on]
        ...
        for df_g, _, group_title in xpd.x_iter_groups(df_p, group_on):
            ...
            if plot_func:
                ...
                for hdi_prob in hdi_probs:
                    ...
                    plt.fill_between(df_hdi[x], df_hdi['low'], df_hdi['high'], **params)
                    label = None
