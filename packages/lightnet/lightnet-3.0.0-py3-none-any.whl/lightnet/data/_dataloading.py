...
from lightnet._imports import pd, bb
...
class Dataset(torchDataset):
    ...
    def input_dim(self):
        ...
        """
        return getattr(self, '_input_dim', self.__input_dim)
...
class BatchSampler(torchBatchSampler):
    ...
    def __set_input_dim(self):
        ...
        if self.new_input_dim is not None:
            log.info('Resizing network %s', self.new_input_dim)
...
def brambox_collate(batch):
    ...
    if isinstance(elem, pd.DataFrame):
        for i, df in enumerate(batch):
            df['batch_number'] = i
        if 'image' in elem.columns and elem.image.dtype == 'category':
            return bb.util.concat(batch, ignore_index=True, sort=False)
        else:
            return pd.concat(batch, ignore_index=True, sort=False)
    # Copied from torch.utils.data.dataloader.default_collate
    elif isinstance(elem, collections.abc.Mapping):
        return {key: brambox_collate([d[key] for d in batch]) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, '_fields'):  # namedtuple
        elem_type = type(elem)
        return elem_type(*(brambox_collate(samples) for samples in zip(*batch)))
