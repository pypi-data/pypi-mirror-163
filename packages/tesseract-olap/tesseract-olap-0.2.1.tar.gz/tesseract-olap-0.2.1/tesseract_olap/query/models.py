...
FilterCondition = Union[
    NumericConstraint,
    Tuple["FilterCondition", LogicOperator, "FilterCondition"],
]
...
class CutIntent:
    ...
    """
    level: str
    include_members: Set[str]
    exclude_members: Set[str]

    @classmethod
    def new(cls, level: str, incl: Array[str], excl: Array[str]):
        null_values = ("", ",")
        return cls(level=level,
                   include_members=set(incl).difference(null_values),
                   exclude_members=set(excl).difference(null_values))
