...
```
## Bigeye Credential File Formats
Bigeye CLI supports basic authentication and accepts a file in the below format:
```yaml
...
```
Bigeye CLI also supports browser authentication for Firefox, Chrome, and other Chromium based browsers.  Chrome supports
multiple google authenticated profiles and requres the user to identify the profile that is logged into Bigeye.  Other
browsers do not requre the browser_profile_user_name to be included.
```yaml
browser: Chrome
browser_profile_user_name: user@company.com, # required only if Chrome and profiles are being used.
base_url: "https://app.bigeye.com",
```
