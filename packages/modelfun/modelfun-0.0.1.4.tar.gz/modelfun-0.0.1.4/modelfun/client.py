...
import modelfun
from modelfun.embeddings import Embeddings
from modelfun.error import ModelfunError
from modelfun.generation import Generations, Generation, TokenLikelihood
from modelfun.tokenize import Tokens
from modelfun.classify import Classifications, Classification, Example as ClassifyExample, Confidence
from modelfun.extract import Entity, Example as ExtractExample, Extraction, Extractions
...
try:
    from modelfun.tokenizer import tokenizer
...
class Client:
    def __init__(
        ...
        self.api_url = modelfun.MODELFUN_API_URL
        self.num_workers = num_workers
        self.request_dict = request_dict
        if version is None:
            self.modelfun_version = modelfun.MODELFUN_VERSION
        else:
        ...
        if check_api_key:
            try:
                ...
                if not res['valid']:
                    raise ModelfunError('invalid api key')
            except ModelfunError as e:
                raise ModelfunError(
                    message=e.message,
    ...
    def check_api_key(self) -> Response:
        ...
        if self.modelfun_version != '':
            headers['modelfun-Version'] = self.modelfun_version

        url = urljoin(self.api_url, modelfun.CHECK_API_KEY_URL)
        if use_xhr_client:
    ...
    def generate(
        ...
        response = self.__request(json_body, modelfun.GENERATE_URL, model_name)
    ...
    def classify(
        ...
        response = self.__request(json_body, modelfun.CLASSIFY_URL, model_name)
        classifications = []
    ...
    def unstable_extract(
        ...
        '''
        Makes a request to the modelfun API to extract entities from a list of texts.
        Takes in a list of modelfun.extract.Example objects to specify the entities to extract.
        Returns an modelfun.extract.Extractions object containing extractions per text.
        '''
        ...
        response = self.__request(json_body, modelfun.EXTRACT_URL)
        extractions = []
    ...
    def tokenize(self, text: str) -> Tokens:
        if (use_go_tokenizer):
            ...
            response = self.__request(json_body, modelfun.TOKENIZE_URL)
