...
class OWPyRScript(OWWidget):
    name = "R Script"
    description = "Write a R script and run it on input data or models."
    icon = "icons/R.svg"
    priority = 3150
    ...
    class Inputs:
        data = Input("Data", Table, replaces=["in_data"],
                     default=True, multiple=True, auto_summary=False)
        ...
        object = Input("Object", object, replaces=["in_object"],
                       default=False, multiple=True, auto_summary=False)

    class Outputs:
        data = Output("Data", Table, replaces=["out_data"], auto_summary=False)
        learner = Output("Learner", Learner, replaces=["out_learner"], auto_summary=False)
        classifier = Output("Classifier", Model, replaces=["out_classifier"], auto_summary=False)
    ...
    def __init__(self):
        ...
        self.execute_button = gui.button(
            self.buttonsArea, self, 'Run', callback=self.commit)
    ...
    def commit(self):
        ...
        cmd_str = str(self.text.toPlainText())
        # hcmray
        pre_str = "from Orange.data.pandas_compat import table_from_frame,table_to_frame\n"
        pre_str = pre_str + "\n"
        pre_str = pre_str + "import pandas as pd\n"
        pre_str = pre_str + "from rpy2 import robjects as ro\n"
        pre_str = pre_str + "from rpy2.robjects import pandas2ri\n"
        pre_str = pre_str + "from rpy2.robjects.conversion import localconverter\n"
        pre_str = pre_str + "\n"
        #为了去掉时区，在rpy2包中的pandas2ri.py文件中，rpy2py_posixct(obj)函数后加上：.strftime('%Y-%m-%d')
        #直接修改输出的时间格式，使之不含时区，能正确由table_from_frame处理。
        pre_str = pre_str + "pandas2ri.activate()\n"
        pre_str = pre_str + "\n"
        pre_str = pre_str + \
            "if not(in_data is None):ro.globalenv['in_data'] = table_to_frame(in_data)\n"
        pre_str = pre_str + "\n"
        # pre_str = pre_str+"Main.in_object = in_object\n"
        ##
        #aft_str = "out_data = table_from_frame(r_out_data)\n"
        # aft_str = aft_str+"out_object = Main.out_object\n"
        # R_str = "out_data = in_data\n "
        R_conv = """with localconverter(ro.default_converter + pandas2ri.converter):
  out_data = ro.conversion.rpy2py(ro.r.out_data)\n"""
        R_conv = R_conv + "out_data = table_from_frame(out_data)\n"
        cmd_str = pre_str + 'ro.r("""\n' + cmd_str + '""")\n'+R_conv
        # 需要的处理加在这里前后
        lcls["_script"] = cmd_str
