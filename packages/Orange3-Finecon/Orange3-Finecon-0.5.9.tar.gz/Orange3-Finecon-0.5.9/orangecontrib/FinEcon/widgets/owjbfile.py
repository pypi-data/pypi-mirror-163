...
class OWJBFile(OWSaveBase):
    name = "NoteBook"
    description = "保存数据文件，并执行笔记本文件。"
    icon = "icons/框框.svg"
    priority = 9
    ...
    def __init__(self):
        ...
        gui.button(
            ...
            label=f"正在执行文件 {self.stored_name}" if subpro_stats else "执行",
            callback=self.exec_file)
        gui.button(
            self.buttonsArea, self,
            label=f"停止执行",
            callback=self.stop_exec_file)
        self.grid.addWidget(
            gui.checkBox(
    ...
    def exec_file(self):
        ipynb_file = self.filename + ".ipynb"
        ipynb_start = """{
 "cells": [
  {
   "cell_type": "code",
 ...
 "nbformat_minor": 2
        ...
        """
        if ~os.path.exists(os.path.split(ipynb_file)[0]):
            with open(ipynb_file, 'w') as f:
                f.write(ipynb_start)
            
        self.subpro_stats = subprocess.Popen(["jupyter", "notebook", ipynb_file])
        
    def stop_exec_file(self):
        if self.subpro_stats is not None:
            self.subpro_stats.kill()
            self.subpro_stats = None

if __name__ == "__main__":  # pragma: no cover
    WidgetPreview(OWJBFile).run(Table("iris"))
