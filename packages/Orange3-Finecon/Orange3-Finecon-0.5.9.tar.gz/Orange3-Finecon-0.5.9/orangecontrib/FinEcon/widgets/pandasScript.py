...
class OWPyPandasScript(OWWidget):
    name = "Pandas Script"
    description = "Write a Pandas script and run it on input data or models."
    ...
    def commit(self):
        ...
        pre_str = pre_str + "\n"
        pre_str = pre_str + "in_data = table_to_frame(in_data)\n"
        pre_str = pre_str + "in_pds = list()\n"
        pre_str = pre_str + "in_pds = (in_pds.append(table_to_frame(d)) for d in in_datas)\n"
        pre_str = pre_str + "in_datas = in_pds"
        aft_str = "out_data = table_from_frame(out_data)\n"
        cmd_str = pre_str + '\n' + cmd_str + '\n' + aft_str
        # 需要的处理加在这里前后
...
if __name__ == "__main__":  # pragma: no cover
    WidgetPreview(OWPyPandasScript).run()
