...
"""
    ...
    def add_constraint(self, ei:int, ej:int, constr: Tuple[Union[float, int, timedelta],Union[float, int, timedelta]]) -> None:
        ...
        - constr: a couple (min,max)
        """

        if not isinstance(constr,tuple):
            warnings.warn("error: constraint must be a tuple (=> constraint not added)")
            return
        

        if len(constr)!=2:
            warnings.warn("error: constraint must have 2 values (=> constraint not added)")
            return

        if ei==ej:
            warnings.warn("Impossible to add the constraint: event id must be different")
            return
    ...
    def __str__(self) -> str:
        ...
        for i in self.sequence:
            for j in [v for v in self.sequence if v>i]:
    ...
    def clean(self) -> None:
        for itempos in list(self.sequence.keys()): #HACK, use list to copy the keys
            if self.sequence[ itempos ] is None:
                del self.sequence[ itempos ]
        posmax = max(self.sequence) #max of the keys
        for p in list(self.tconst.keys()): #HACK, use list to copy the keys
    ...
    def minimize(self) -> None:
        ...
        mat=np.matrix( np.zeros( (max(self.sequence)+1,max(self.sequence)+1) ))
        for i in range(max(self.sequence)+1):
            for j in range(i+1,max(self.sequence)+1):
    ...
    def recognize(self, 
        ...
        if isinstance(sequence, pd.core.frame.DataFrame):
    ...
    def __interval_sim(self, 
        ...
        if isinstance(interval[0], pd._libs.tslibs.timedeltas.Timedelta):
            interval=(interval[0].total_seconds(),interval[1].total_seconds())
        if isinstance(pos,pd._libs.tslibs.timedeltas.Timedelta):
    ...
    def cmp(self, 
        ...
        if isinstance(sequence, pd.core.frame.DataFrame):
            if "labels" not in sequence.columns:
                #missing column
                return [[]]
            if isinstance(sequence.index, pd.core.indexes.datetimes.DatetimeIndex):
