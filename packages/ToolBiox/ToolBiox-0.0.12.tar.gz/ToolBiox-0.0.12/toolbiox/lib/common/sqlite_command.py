...
def sqlite_select(db_file, table_name, column_list=None, key_name=None, value_tuple=None):
...
def store_dict_to_db(input_dict, output_db):
    ...
    input_dict_info_list = [(i, input_dict[i]) for i in input_dict]
    build_database(input_dict_info_list, lambda x: {'table_record': (x[0], pickle_dump_obj(
        x[1]))}, {'table_record': ['id', 'codenstring']}, output_db, add_mode=True)
