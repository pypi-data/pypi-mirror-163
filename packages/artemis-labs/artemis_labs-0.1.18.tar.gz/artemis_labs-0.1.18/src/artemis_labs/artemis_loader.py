                    ...
                    if token.token_type == TokenType.NAMED_ARGUMENT:
                        ...
                        if token_content.startswith('"'):
                            for j in range(i + 1, len(tokens)):
                                old_token_value = tokens[i + 1].token_value
                                new_token_value = (old_token_value[0], old_token_value[1] + " " + tokens[j+1].token_value)
                                tokens[i + 1].token_value = new_token_value
                                if '"' in tokens[j + 1].token_value:
                                    final_token_value_content = new_token_value[1].strip()
                                    if final_token_value_content.startswith('"'):
                                        final_token_value_content = final_token_value_content[1:]
                                    if final_token_value_content.endswith('"'):
                                        final_token_value_content = final_token_value_content[:-1]
                                    final_token_value = (new_token_value[0], final_token_value_content)
                                    tokens[i + 1].token_value = final_token_value
                                    break
        else:
        ...
        for i, line in enumerate(lines):
            ...
            if token_chain.is_valid():
                ...
                if '@input' == token_chain.get_command() and next_line != None:
                    ...
                    # append fodder
                    pass_1.append(line)
                    pass_1.append(f"{token_chain.get_indentation()}app.createInput({i - 4 + 1}, 'Variable {lhs}', '')")
                ...
                if '@output' == token_chain.get_command() and next_line != None:
                    ...
                    pass_1.append(f"{token_chain.get_indentation()}app.createOutput({i - 4 + 1}, '{lhs}', {lhs}, '{componentType}', '', {token_chain.get_named_args()})")
                ...
                if '@doc' == token_chain.get_command():
                    ...
                    pass_1.append(f"{token_chain.get_indentation()}app.createOutput({i - 4 + 1}, '', '{markdown_text}', '{token_chain.get_component_type()}', '')")
                    continue
