    ...
    function createOutputComponentDOM(in_componentType, outputValue, parentElementSelector, startTop=false) {
        ...
        if (componentType == "card") {
            ...
            $(component.getId()).on('contextmenu', function(e) {
                ...
                // Get DOM and aspect
                let componentDOM = qs("#" + this.id + " .card-body-preview");
    ...
    function createOutputComponent(outputLine, outputName, outputValue, outputComment, outputComponentType) {
        ...
        if (outputComponentType == "markdown") {
            ...
            for (let i = 0; i < links.length; i++) {
                let link = links[i];
                console.log(link);
                if (link.href.substring(0, 3) != 'www') {
                    $(link).click((e)=>{
                        let openFileMessage = {};
            ...
            for (let i = 0; i < paragraphs.length; i++) {
                ...
                for (let j = 0; j < chunks.length; j++) {
                    if (j % 2 == 0) {
                        ...
                        cleanedBucket.push("<latex-js hyphenate=\"false\" class=\"latex-equation\" style=\"display: inline-block;\">" + '$' + chunks[j] + '$' + "</latex-js>");
                    }
                }
