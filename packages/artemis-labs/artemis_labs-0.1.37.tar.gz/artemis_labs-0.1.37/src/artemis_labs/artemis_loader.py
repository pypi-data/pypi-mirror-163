    ...
    def transform_script(lines, init_offset=10):
        ...
        for i, line in enumerate(lines):
            ...
            if 'app = artemis' in line:
                ...
                pass_1.append(f"app.createOutput(0, 0, '', '{attr_output}', 'markdown', '')")
                continue
            ...
            if token_chain.is_valid():
                ...
                if token_chain.get_command() == '@enddoc'  and i < len(lines) - 1 and "'''" in lines[i + 1] and doc_accum_active:
                    ...
                    pass_1.append(f"{token_chain.get_indentation()}app.createOutput({doc_accum_line_start - 1}, {i - init_offset + 2}, '', '{doc_accum}', 'markdown', '')")
                    skip_counter = 1
                    doc_accum_active = False
                    doc_accum_line_start = -1
                    doc_accum = ''
            ...
            if token_chain.is_valid() and not in_block_quote:
                ...
                if '@input' == token_chain.get_command() and next_line != None:
                    ...
                    # append fodder
                    pass_1.append(line)
                    pass_1.append(f"{token_chain.get_indentation()}app.createInput({i - init_offset + 1}, {i - init_offset + 2}, 'Variable {lhs}', '')")
                ...
                if '@output' == token_chain.get_command() and next_line != None:
                    ...
                    pass_1.append(f"{token_chain.get_indentation()}app.createOutput({i - init_offset + 1}, {i - init_offset + 2}, '{lhs}', {lhs}, '{componentType}', '', {token_chain.get_named_args()})")
                ...
                if '@doc' == token_chain.get_command():
                    ...
                    # Store MD
                    pass_1.append(f"{token_chain.get_indentation()}app.createOutput({i - init_offset + 1}, {i - init_offset + 1}, '', '{markdown_text}', '{token_chain.get_component_type()}', '')")
                    continue
