...
try:
    ...
    SEABORN = False
    print('WARNING: seaborn not installed. Install seaborn to check out the summary.')    
...
def get_markers_minor_type2(file, target_cells = [], pnsh12 = PNSH12,
                            rem_common = False, comb_mkrs = True, verbose = False):
...
def load_markers_all(file, target_cells = [], pnsh12 = '111111', comb_mkrs = True,  verbose = False):
        ...
        if c in major_type_lst:
            ...
            for c2 in cell_type_lst:
                ...
                if pnsh12[2] == '1':
                    ...
                    mkrs_sec = get_markers_from_df(df, [c2], pnsh12 =pnsh12_n, verbose = verbose)
...
def select_variable_genes(X, log_transformed = False, N_genes = 2000):
    ...
    lc = int(len(odr)*0.1)
    uc = int(len(odr)*0.9)
    xsm_sel = xsm[odr[lc:uc]]
    min_xsm = xsm_sel.min()
    max_xsm = xsm_sel.max()
    Ns = 10
    dx = (max_xsm - min_xsm)/Ns
    
    rng = np.arange(min_xsm, max_xsm+(Ns/2), dx)
    nss = []
    for n in range(Ns):
        lc = rng[n]
        uc = rng[n+1]
        bx = (xsm >= lc) & (xsm < uc)
        nss.append(np.sum(bx))
    N_to_sel = min(nss)
    if N_to_sel < 400:
        N_to_sel = 400
    ...
    for n in range(Ns):
        ...
        uc = rng[n+1]
        bx = (xsm >= lc) & (xsm < uc)
        if np.sum(bx) <= N_to_sel:
            bg[bx] = True
            # print('%i/%i,' % (np.sum(bx), N_to_sel))
        else:
            ilst = list(idx[bx])
            lst_sel = np.random.choice(ilst, N_to_sel, replace = False)
            bg[lst_sel] = True
            # print('%i/%i,' % (len(lst_sel), N_to_sel))
            
    Xs_sel = Xs.loc[:,bg]    
        
    ## Fit
    m_sel = Xs_sel.mean(axis = 0)
    s_sel = Xs_sel.var(axis = 0)

    lm_sel = np.log10(m_sel + 0.000001)
    ls_sel = np.log10(s_sel + 0.000001)
...
def apply_knn1(nei_indices, cell_type, score, pmaj = 0.25, nlog_pval_th = 2):
    ...
    for k in range( len(cell_type) ):
        ...
        if len(set(list(cts))) <= 1:
            ...
            #'''
            b = cts == majority
            sm = score.loc[ilst[b],:].sum(axis=0)
            score2.iloc[k,:] = list( sm )
            #'''
...
def get_markers_all(mkr_file, target_lst, pnsh12, level = 1, comb_mkrs = True):
...
def HiCAT( X_cell_by_gene, marker_file, log_transformed = False,
               ...
               model = 'gmm', N_gmm_components = 10, 
               Target_FPR = 0.05, pval_th = 0.05, pval_th_subset = 1, 
               pmaj = 0.8, pth_fit_pnt = 0.4, pth_min = 0.25, min_logP_diff = 1, 
               use_minor = True, minor_wgt = 0, use_union = False,
               use_markers_for_pca = False, comb_mkrs = False, 
               knn_type = 1, knn_pmaj = 0.4, N_max_to_ave = 1,
               thresholding_minor = True, thresholding_subset = False, 
               pct_cutoff_minor = True, pct_cutoff_subset = False ):
    ...
    if len(target_cell_types) > 0:
        ...
        if len(target_tissues) > 0:
            ...
            print('No target tissues/celltypes were provided.')
            return
    ...
    if verbose: print('HiCAT running ..')
    else: print('HiCAT running ..', end = '', flush = True)
    ...
    X_pca = pd.DataFrame(X_pca, index = Xx.index, columns = list(np.arange(int(N_components_pca))))
    ...
    if True:
        ...
        if not use_union:            
            ...
            df_res_subset_all, df_GSA_score_subset_all = \
                GSA_cell_subtyping( Xs, X_pca, mkr_lst_subset, mkr_lst_subset_neg, verbose = verbose )
        ...
        for tc in target_cell_lst.keys():
            ...
            if (len(list(mkr_lst.keys())) <= 1):
                ...
                if (np.sum(b_cur) < N_neighbors_minor):
                    ...
                    if use_union:
                        df_res_cur, df_GSA_score_cur = GSA_cell_subtyping( X_cur, X_pca_cur, mkr_lst, mkr_lst_neg, 
                                                               verbose = print_report )
                    else: 
                    ...
                    if X_cur.shape[1] < 1000:
                        if not log_transformed:
                            Xx_cur = np.log2(1 + X_normalize(X_cur))
                            Xx_cur = X_scale(Xx_cur, max_val = 10)
                        else:
                            Xx_cur = X_cur
                            Xx_cur = X_scale(Xx_cur, max_val = 10)
                    else:
                        #'''
                        gene_sel2 = select_variable_genes(X_cur, log_transformed = log_transformed, N_genes = 1000)
                        if not log_transformed:
                            Xx_cur = np.log2(1 + X_normalize(X_cur.loc[:,gene_sel2]))
                            Xx_cur = X_scale(Xx_cur, max_val = 10)
                        else:
                            Xx_cur = X_cur.loc[:,gene_sel2]
                            Xx_cur = X_scale(Xx_cur, max_val = 10)
                        '''
                        if not log_transformed:
                            Xx_cur = np.log2(1 + X_normalize(X_cur))
                            gene_sel2 = select_variable_genes(Xx_cur, log_transformed = log_transformed, N_genes = 1000)
                            Xx_cur = X_scale(Xx_cur.loc[:,gene_sel2], max_val = 10)
                        else:
                            Xx_cur = X_cur
                            gene_sel2 = select_variable_genes(Xx_cur, log_transformed = log_transformed, N_genes = 1000)
                            Xx_cur = X_scale(Xx_cur.loc[:,gene_sel2], max_val = 10)
                    ...
                    df_res_cur, df_score_cur, df_GSA_score_cur = \
                        run_gsa_and_clf(X_pca_cur, X_cur, cobj, y_clust_cur, mkr_lst, mkr_lst_neg, 
                                         ...
                                         pct_cutoff = pct_cutoff_minor, print_report = verbose, 
                                         df_GSA_score = df_pass)
                ...
                if (len(list(mkr_lst.keys())) <= 1) | (np.sum(b_cur) < 10):
                    ...
                    df_res_cur, df_score_cur, df_GSA_score_cur = \
                        run_gsa_and_clf(X_pca_cur, X_cur, cobj, y_clust_cur, mkr_lst, mkr_lst_neg, 
                                         ...
                                         pct_cutoff = pct_cutoff_subset, print_report = verbose,
                                         df_GSA_score = df_pass)
                        ...
                        if df_GSA_score_cur is not None:
                            ...
                            for cx in list(cols):
                                ...
                                if minor_type in minor_type_lst_all:
                                    ...
                                    print('WARNING: %s (%s) not in ' % (minor_type, c), minor_type_lst_all) 
...
def get_markers_all(mkr_file, target_lst, pnsh12, genes = None, level = 1, 
                    rem_cmn = False, comb_mkrs = True ):
...
def plot_dot_s(adata, target_lst, type_level, mkr_file, title = None, rend = None, level = 1, 
              ...
              to_exclude = [], Npt = 15, comb_mkrs = False, minNcells = 20, vg_rot = 0,
...
def plot_marker_expression_profile( df_pred, X, mkr_file, pnsh12 = '100000', 
