...
class BeagleRoboid(Roboid):
    ...
    def _create_model(self):
        ...
        dict[Beagle.ACCELEROMETER] = self._accelerometer_device = self._add_device(Beagle.ACCELEROMETER, "Accelerometer", DeviceType.SENSOR, DataType.FLOAT, 3, -16, 16, 0)
        dict[Beagle.GYROSCOPE] = self._gyroscope_device = self._add_device(Beagle.GYROSCOPE, "Gyroscope", DeviceType.SENSOR, DataType.FLOAT, 3, -2000, 2000, 0)
        dict[Beagle.MAGNETOMETER] = self._magnetometer_device = self._add_device(Beagle.MAGNETOMETER, "Magnetometer", DeviceType.SENSOR, DataType.FLOAT, 3, -2500, 2500, 0)
    ...
    def _decode_sensory_packet(self, packet):
        ...
        if packet[0] == 0x10: # basic
            ...
            value = packet[3] & 0xff
            value |= (packet[4] & 0xff) << 8
            self._timestamp_imu_device._put(value)
            ...
            while index < length + 2:
                ...
                if id == 1: # gyroscope
                    ...
                    self._gyroscope_device._put_at(0, gyroscope_range * value / 32768.0)
                    value = packet[index+2] & 0xff
                    value |= (packet[index+3] & 0xff) << 8
                    if value > 0x7fff: value -= 0x10000
                    self._gyroscope_device._put_at(1, gyroscope_range * value / 32768.0)
                    value = packet[index+4] & 0xff
                    value |= (packet[index+5] & 0xff) << 8
                    if value > 0x7fff: value -= 0x10000
                    self._gyroscope_device._put_at(2, gyroscope_range * value / 32768.0)
                    index += 6
                elif id == 2: # accelerometer
                    value = packet[index] & 0xff
                    value |= (packet[index+1] & 0x0f) << 8
                    if value > 0x7ff: value -= 0x1000
                    acc_x = -accelerometer_range * value / 2048.0
                    self._accelerometer_device._put_at(0, acc_x)
                    value = packet[index+2] & 0xff
                    value |= (packet[index+3] & 0x0f) << 8
                    if value > 0x7ff: value -= 0x1000
                    acc_y = -accelerometer_range * value / 2048.0
                    self._accelerometer_device._put_at(1, acc_y)
                    value = packet[index+4] & 0xff
                    value |= (packet[index+5] & 0x0f) << 8
                    if value > 0x7ff: value -= 0x1000
                    acc_z = -accelerometer_range * value / 2048.0
                    self._accelerometer_device._put_at(2, acc_z)
                    index += 7 # temperature
                    ...
                    self._magnetometer_device._put_at(2, mag_z)
