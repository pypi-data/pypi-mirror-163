# Releasing the HEA Server Data Adapters Microservice
...
1. For numbered releases, remove `.dev` from the version number in setup.py, tag it in git to indicate a release,
   ...
   `x.x.x` with the actual version number being released. Name the tag `heaserver-data-adapters-x.x.x`, replacing
