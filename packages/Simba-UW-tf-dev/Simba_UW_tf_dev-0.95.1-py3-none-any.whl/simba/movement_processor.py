    ...
    def process_movement(self):
        ...
        for file_path in self.files_found:
            ...
            for animal_name, animal_bps in self.bp_dict.items():
                results[video_name][animal_name] = {}
                ...
                movement = self.euclidean_distance(animal_df[animal_bps[0]], animal_df[animal_bps[0] + '_shifted'], animal_df[animal_bps[1]], animal_df[animal_bps[1] + '_shifted'], self.px_per_mm)
                results[video_name][animal_name]['Distance (cm)'] = round((movement.sum() / 10), 4)
                velocity_lst = []
                for df in np.array_split(movement, self.fps):
                    velocity_lst.append(df.sum())
                results[video_name][animal_name]['Velocity (cm/s)'] = round((mean(velocity_lst) / 10), 4)
