        ...
        if len(isOnlyEngNum.strip()) == 0:
            ...
            if len(multi_noun_list_score) == 0:
                if not in_dict(cleaning_multi_noun_result_score, multi_noun):
                    check_capitalize_score = 1000 * len(re.findall('[A-Z]+', multi_noun)) / len(multi_noun.replace(" "))
...
if __name__ == '__main__':
    ...
    # _, a = extract_multi_noun(
    #     "The Bank shall be governed by at least three Directors (hereinafter the \"Board of Directors\" or \"BOD\") who are physical persons elected by the Shareholders. The BOD members shall constitute of at least two Independent members with qualifications required by NBC.")
    # print('---')
    # print(a)
