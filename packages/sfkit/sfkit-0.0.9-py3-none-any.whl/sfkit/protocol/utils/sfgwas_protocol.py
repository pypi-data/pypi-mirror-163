...
def install_sfgwas() -> None:
    ...
    commands = """sudo apt-get update -y
                    ...
                    sudo unzip plink2_linux_avx2_20220603.zip -d /usr/local
    ...
    doc_ref = firestore.Client().collection("studies").document("token")
    doc_ref_dict: dict = doc_ref.get().to_dict()  # type: ignore
    username = doc_ref_dict.keys()[0]  # type: ignore
    token = doc_ref_dict[username]
...
def update_configGlobal(doc_ref_dict: dict, configuration: str) -> None:
    ...
    for i, participant in enumerate(doc_ref_dict["participants"]):
        assert doc_ref_dict["personal_parameters"][participant]["NUM_INDS"]["value"] == data["num_inds"][i]
...
def build_sfgwas(configuration: str) -> None:
    ...
    for line in fileinput.input(files="sfgwas-private/main_test.go", inplace=True):
        if "var defaultConfigPath = " in line:
            ...
            print("func SimonTestGwas(t *testing.T) {")
        else:
            print(line, end="")
