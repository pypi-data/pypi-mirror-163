...
class FormativeCheck(CheckData):
    ...
    def regular(self, rule, **kwargs):
        ...
        # 用户姓名
        if rule == 'is_user_name':
            return self.is_user_name
        # 整数
        elif rule == 'is_int':
            return self.is_int
        # 密码
        elif rule == 'is_user_password':
            return self.is_user_password
        # URL
        elif rule == 'is_url':
            return self.is_url
        # 数字
        elif rule == 'is_int_or_float':
            return self.is_int_or_float
        # 时间
        elif rule == 'is_datetime_string':
            return self.is_datetime_string
        # 银行卡
        elif rule == 'is_bank_number':
            return self.is_bank_number
        # 邮箱
        elif rule == 'is_mailbox':
            return self.is_mailbox
        # JSON
        elif rule == 'is_json':
            # noinspection PyBroadException
            try:
                # 不能用ast，因为ast的处理能力太强，各种格式都不保存，无法触发tey
        ...
        # 枚举
            ...
            return is_true
        # 整形(浮点)列表
        elif rule == 'is_int_float_list':
            parameter = json.loads(self.parameter)
            is_true = True

            for key in parameter:
                super().__init__(key)
                if not self.is_int_or_float:
                    is_true = False

            return is_true
        # URL列表
        elif rule == 'is_url_list':
            parameter = json.loads(self.parameter)
            is_true = True
    ...
    def regular_logic(self, rule, choices):
        if self.regular(rule, choices=choices) is not False:
            if rule in ["is_int_float_list", "is_url_list", "is_json"]:
                return json.loads(self.parameter)
            elif rule == "is_int":
