...
class RedisStorage(StorageInterface):
    ...
    def __init__(
        self,
        file_storage_base_path: Path = Path("./buffycache"),
        redis_connection_params: Dict = {"host": "localhost", "port": 6379},
    ):
        self.db = redis.Redis(**redis_connection_params)
        self.files_base = (
            file_storage_base_path
            if isinstance(file_storage_base_path, Path)
            else Path(file_storage_base_path)
        )
    ...
    def create_request(self, request_in: Request_in) -> Request:
        request: Request = Request.from_request_in(request_in=request_in)
    ...
    def get_response_content_hash(self):
        pass
