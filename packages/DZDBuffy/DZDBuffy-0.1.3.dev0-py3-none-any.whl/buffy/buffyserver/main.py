...
def _start_api_service():
    ...
    storage = storage_module_mapping[config.STORAGE_MODULE](**config.STORAGE_CONFIG)
...
def start_buffy_server(
    ...
    log.info(f"Storage config: '{config.STORAGE_CONFIG}'")
    log.info(f"Uvicorn config: '{config.UVICORN_PARAMS}'")
