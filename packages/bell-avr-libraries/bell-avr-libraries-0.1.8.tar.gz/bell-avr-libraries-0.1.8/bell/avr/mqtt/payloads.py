...
class _AvrAutonomousCallable(Protocol):
    """
    Class used only for type-hinting MQTT callbacks from the `avr/autonomous` topic
    """
...
class AvrGameBuildingStatePayload(TypedDict):
    """
...
class _AvrGameBuildingStateCallable(Protocol):
    """
    Class used only for type-hinting MQTT callbacks from the `avr/game/building/state` topic
    """
    def __call__(self, payload: AvrGameBuildingStatePayload) -> Any:
        ...
