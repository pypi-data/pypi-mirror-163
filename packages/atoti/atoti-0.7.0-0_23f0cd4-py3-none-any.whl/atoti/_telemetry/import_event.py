...
from atoti_core import get_env_flag, keyword_only_dataclass, running_in_ipython
...
def _get_environment() -> Optional[str]:
    for env_var, env_name in _ENV_VAR_TO_ENV_NAME.items():
    ...
    if running_in_ipython():
        ...
        ipython_shell = str(get_ipython())
        return next(
            (
                env_name
                for shell_name, env_name in _IPYTHON_SHELL_TO_ENV_NAME.items()
                if shell_name in ipython_shell
            ),
            "IPython",
        )
    return None
