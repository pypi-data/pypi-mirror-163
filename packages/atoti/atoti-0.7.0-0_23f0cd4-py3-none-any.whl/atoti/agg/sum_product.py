...
@overload
def sum_product(
    *factors: Union[MeasureConvertible, MeasureDescription], scope: Scope
) -> MeasureDescription:
    ...
...
@doc(scope=SCOPE_DOC)
def sum_product(
    *factors: Union[Column, MeasureConvertible, MeasureDescription],
    ...
    return agg(
        SumProductEncapsulationMeasure(
            _factors=[
                measure._measure_description
                if isinstance(measure, MeasureConvertible)
                else measure
                for measure in measures
            ]
        ),
        plugin_key="SUM_PRODUCT",
