...
class Quantity(np.ndarray):
    ...
    def __new__(cls, data, unit=None, derived_units=None, dtype=None,
        ...
        result = np.array(data, dtype=dtype, copy=copy, order=order,
    ...
    def __getattr__(self, name):
        if self.dtype.names is None or name not in self.dtype.names:
            raise AttributeError("'" + self.__class__.__name__ + "' object has"
                                 " no attribute '" + name + "'")
        return self[name]
    ...
    @classmethod
    def empty(cls, shape, unit=None, derived_units=None, dtype=None,
              order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(empty(shape, dtype, order), dtype=dtype, unit=unit,
                   derived_units=derived_units, copy=False, **keywords)

    @classmethod
    def ones(cls, shape, unit=None, derived_units=None, dtype=None,
             order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(np.ones(shape, dtype, order), dtype=dtype, unit=unit,
                   derived_units=derived_units, copy=False, **keywords)

    @classmethod
    def zeros(cls, shape, unit=None, derived_units=None, dtype=None,
              order=None, **keywords):
        if dtype is None:
            dtype = cls.default_dtype
        return cls(np.zeros(shape, dtype, order), dtype=dtype, unit=unit,
                   derived_units=derived_units, copy=False, **keywords)
