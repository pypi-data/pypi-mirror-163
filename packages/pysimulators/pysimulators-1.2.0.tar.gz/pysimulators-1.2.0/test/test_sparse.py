...
from pyoperators import (
    CompositionOperator, DiagonalOperator, Rotation2dOperator,
    Rotation3dOperator)
...
def test_fsc2():
    ...
    def func1(itype, ftype, vtype, block_size):
        ...
        assert_same(out, exp)
        out = input_fsr_ * mat
        assert_same(out, FSRMatrix(mat.shape[::-1], data=mat.data) * 
                    input_fsr_)
...
def test_fsr2():
    ...
    def func2(itype, ftype, shapein):
        ...
        if shapein is None:
            return
        op2 = SparseOperator(mat, shapein=(2, 2) + shapein,
                             shapeout=(3, 2) + shapein)
        assert_same(op2.todense(), todense)
    for itype in INT_TYPES:
        for ftype in FLOAT_TYPES:
            for shapein in (), (3,):
...
def test_block():
    ...
    def get_mat(itype, ftype, inner_block_shape, indices):
        ...
        mat = FSRBlockMatrix(shape, inner_block_shape, len(indices),
                             dtype=ftype, dtype_index=itype)
