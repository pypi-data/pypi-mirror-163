...
from pysimulators import (
    create_fitsheader, LayoutGridSquares, Quantity, SceneGrid)
...
def test_spatial_integration():
    ...
    def func(center, ncolmax, itype, ftype, expected):
        ...
        proj = plane.get_integration_operator(
            plane.topixel(detectors), ncolmax=ncolmax, dtype=ftype,
            dtype_index=itype)
...
def test_spatial_integration2():
    ...
    def func(spacing, angle):
        ...
        grid = LayoutGridSquares(shape_grid, spacing, origin=origin,
                                 angle=angle, filling_factor=filling_factor)
        ...
        assert_same(np.sum(proj.matrix.data.value),
                    len(grid) * spacing**2 * filling_factor, rtol=100)
