...
try:
    ...
    # this is the parent process
    def datagrabber(valuesN, valuesL, dimensions, calculationtype, calculation2type, equilibrium_file, potential_energy,
                    NumberOfEigenvalues):
        q = Queue()
        p1 = Process(target=datamuncher, args=(q,))
        p1.start()
        time.sleep(1)
        holder = GUI.main_window(valuesN, valuesL, dimensions, calculationtype, calculation2type, equilibrium_file,
                                 potential_energy, NumberOfEigenvalues)
