...
class Database:
    ...
    def load_data(db) -> dict:
        folder = os.path.join(db.clud_path, ".clustery")

        if not os.path.exists(folder):
            os.makedirs(os.path.join(db.clud_path, ".clustery"))

        data_path = os.path.join(folder, f"{db.name}.clud.temp")

        data = {}

        if os.path.exists(os.path.join(folder, f"{db.name}.clud")):
            pyAesCrypt.decryptFile(os.path.join(folder, f"{db.name}.clud"), data_path, db.enc_key)
    ...
    def save_data(db, data:dict):
        ...
        pyAesCrypt.encryptFile(data_path, os.path.join(folder, f"{db.name}.clud"), db.enc_key)
        os.remove(data_path)
