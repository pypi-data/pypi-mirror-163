...
def visualize(predictions, contracts, output_dir):
    outputs = []
    for idx, contract in enumerate(contracts):
        ...
        for color_idx, pred in enumerate(predictions[12*idx:12*(idx+1)]):
            ...
            for start, end in zip(pred['answer_start'], pred['answer_end']):
                ents.append({
                    'start': start,
                    'end': end,
                    'label': pred['question']
    ...
    for output in outputs:
        filename = output.pop('contract', None)
        html_results.append({filename: displacy.render(output, manual=True, style="ent", options=options)})
...
def main():
    ...
    help_msg = 'contract-reviewer -b <alex-apostolo/legal-bert-base-cuad> -l <akdeniz27/roberta-large-cuad> -o <output_dir> ...<files>\ncontract-reviewer -m <alex-apostolo/legal-bert-base-cuad> -o <output_dir> ...<files>'
    try:
        opts, args = getopt.getopt(argv[1:],"hb:l:m:o:",["base-model=","large-model=", "model=", "output="])
    ...
    visualize(predictions, _read_contracts(paths), output_dir)