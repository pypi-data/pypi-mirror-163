...
class Label(Boite):
    ...
    def imageTexte(self):
        ...
        self.height = max(self.height, r.height + 2)
        if r.width > self.width - 4 and self.width > 0:
            r.width = self.width - 4
            text_bitmap = text_bitmap.subsurface(r)
        else:
            self.width = max(r.width, self.width)
    ...
    def draw(self):
        # fonction qui permet l'affichage de texte à  l'ecran
        super().draw()
        Boite.ecran.blit(self.image, [self.Parent.left + self.left + 2, self.Parent.top + self.top + 2])
