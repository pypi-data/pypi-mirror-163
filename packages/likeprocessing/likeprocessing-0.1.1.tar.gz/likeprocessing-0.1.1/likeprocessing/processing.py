...
def run(globales):
    global __key_pressed, click, click_down, click_up, keys, __quitter, var_globales, frameCount, clock, secondes
    ...
    while True:
        ...
        if globales.get('draw'):
            if __no_loop is False or (__no_loop is True and frameCount == 0):
                draw_background()
                globales['draw']()
                reset()
        if __quitter:
