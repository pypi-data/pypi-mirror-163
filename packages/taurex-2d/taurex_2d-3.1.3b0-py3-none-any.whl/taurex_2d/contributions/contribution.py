...
def contribute_tau_1D(startK, endK, density_offset, sigma, density, path, nlayers,
                   ngrid, layer, tau):
    """
...
def contribute_tau_2D(startK, endK, density_offset, sigma, density, angle, z, path, nlayers,
                   ngrid, layer, tau):
    """
    ...
    for k in range(startK, endK):
        ...
        for wn in range(ngrid):
            tau[layer, wn] += sigma[_angle, _z, wn]*_path*_density

def contribute_tau(startK, endK, density_offset, sigma, density, path, nlayers,
                   ngrid, layer, tau):
    """
    Interface to call either 1D or 2D function to perform a cross-section integration for tau to avoid numba compatibility of types issues.
    ...
    if (density.ndim > 1):
        ...
        contribute_tau_2D(startK, endK, density_offset,
                       sigma, density, angle, z, path_length, nlayers, ngrid, layer, tau)
    else:
        contribute_tau_1D(startK, endK, density_offset,
                       sigma, density, path, nlayers, ngrid, layer, tau)


class SimpleContribution(Fittable, Logger, Writeable):
    """
