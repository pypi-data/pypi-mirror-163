...
def terminatorIndex(profile):
    return int(len(profile)/2)

def slice2D(profile, idx):
    if (profile.ndim == 2):
        return profile[idx]
    else:
        return profile

def terminator2D(profile):
    """
    Returns terminator slice of a simple 2D array

    Parameters
    ----------
    profile: 2D profile (alpha, z)
    """
    if (profile.ndim == 2):
        return profile[int(len(profile)/2)]
    else:
        return profile

def terminator2D3(profile):
    """
    Returns terminator slice of a 3D array when running a 2D model

    Parameters
    ----------
    profile: 3D profile (alpha, z, wngrid) for example
    """
    if (profile.ndim == 3):
        return profile[int(len(profile)/2)]
    else:
