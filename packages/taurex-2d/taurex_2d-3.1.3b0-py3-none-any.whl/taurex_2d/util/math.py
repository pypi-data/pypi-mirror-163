...
def forward_error(array1, array2, err):
    return ((norm(array2 - array1)) / norm(array1)) < err

def error(array1, array2):
    return norm((array2 - array1)/array1)
...
def interp_log_only(y1,y2,x,x1,x2):
    return y1*np.log(x2*(x1 - x)*(np.exp(y1)-np.exp(y2))/(x*(x2 - x1)))

def interp_lin_only(x11,x12,P,Pmin,Pmax):
    return (x11*(Pmax - Pmin) - (P - Pmin)*(x11 - x12))/(Pmax - Pmin)

def interp1d(xx, yy, xkind='linear', ykind='linear'):
    logx = xx
    if xkind == 'log':
        logx = np.log(xx)
    if ykind == 'log':
        lin_interp = sp.interpolate.interp1d(logx, np.log(yy))
        log_interp = lambda x: np.exp(lin_interp(x))
        if xkind == 'log':
            log_interp = lambda x: np.exp(lin_interp(np.log(x)))
        return log_interp
    lin_interp = sp.interpolate.interp1d(logx, yy, kind=ykind)
    return lin_interp
