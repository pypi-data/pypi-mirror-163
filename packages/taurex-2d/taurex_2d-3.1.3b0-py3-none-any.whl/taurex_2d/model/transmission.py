...
class TransmissionModel(SimpleForwardModel):
    ...
    def compute_path_length(self, dz):
        ...
        z = self.altitudeProfile[:-1]
        self.debug('Computing path_length: \n z=%s \n dz=%s', z, dz)

        # use midlayers for the position of rays
        if self.midlayers:
            z = self.altitudeProfile
            for layer in range(0, total_layers):
                p = (planet_radius+ z[layer]+dz[layer]/2)**2
                k = np.zeros(shape=(self.nLayers-layer))
                k[0] = np.sqrt((planet_radius + z[layer+1])**2 - p)

                k[1:] = np.sqrt((planet_radius+ z[layer+2:])**2 - p)
                k[1:] -= np.sqrt((planet_radius+
                                z[layer+1:self.nLayers])**2 - p)
                dl.append(k*2.0)
            return dl
    ...
    def path_integral(self, wngrid, return_contrib):
        ...
        if self.midlayers:
            dz = np.diff(self.altitudeProfile)
        else:
            dz = np.gradient(self.altitude_layers)
