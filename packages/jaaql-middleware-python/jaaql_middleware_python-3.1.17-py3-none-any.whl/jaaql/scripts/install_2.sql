...
DECLARE
    username text;
BEGIN
    SELECT split_part(public_credentials, ':', 1) as username into username FROM jaaql__user WHERE application = the_application;
    EXECUTE 'DROP ROLE ' || quote_ident(username);
    DELETE FROM jaaql__authorization_configuration WHERE role = username;
    UPDATE jaaql__user SET deleted = current_timestamp WHERE application = the_application;
    UPDATE jaaql__credentials_node SET deleted = current_timestamp WHERE role = username;
