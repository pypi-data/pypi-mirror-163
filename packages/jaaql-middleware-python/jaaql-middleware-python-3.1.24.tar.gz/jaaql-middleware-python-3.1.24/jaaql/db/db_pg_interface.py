...
class DBPGInterface(DBInterface):
    ...
    def __init__(self, config, host: str, port: int, db_name: str, username: str, password: str, is_jaaql_user: bool,
        ...
        try:
            ...
            if self.is_host_pool:
                if db_name.lower() not in HOST_POOL:
                    HOST_POOL[db_name.lower()] = ConnectionPool(conn_str, min_size=PGCONN__min_conns,
                                                                              max_size=PGCONN__max_conns_jaaql_user, max_lifetime=60 * 30)
                    HOST_USER[db_name.lower()] = self.username
                self.pg_pool = HOST_POOL[db_name.lower()]
            else:
    ...
    def get_conn(self):
        try:
            ...
            if HOST_USER[self.db_name.lower()] != self.username and self.is_host_pool:
    ...
    def put_conn(self, conn):
        if self.is_host_pool and HOST_USER[self.db_name.lower()] != self.username:
            with conn.cursor() as cursor:
    ...
    def execute_query(self, conn, query, parameters=None):
        while True:
            try:
                with conn.cursor() as cursor:
                    do_prepare = False
                    if HOST_USER[self.db_name.lower()] != self.username and self.username != USERNAME__jaaql:
                        do_prepare = True
