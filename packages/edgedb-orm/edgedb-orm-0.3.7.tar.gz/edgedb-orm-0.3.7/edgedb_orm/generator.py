...
import edgedb
from pydantic import BaseModel, parse_raw_as, root_validator, ValidationError
...
class DBConfig(BaseModel):
    ...
    def check_raw_or_vars(cls, values):
        if values.get("host", None) is None:
            if values.get("host_var", None) is None:
                raise ValidationError("Must give either a host or a host_var")
        if values.get("password", None) is None:
            if values.get("password_var", None) is None:
                raise ValidationError("Must give either a password or a password_var")
        return values

    @property
    def password_val(self) -> str:
        return self.password or os.environ[self.password_var]

    @property
    def host_val(self) -> str:
        return self.host or os.environ[self.host_var]

    @property
    def password_variable(self) -> str:
        return (
            f'os.environ["{self.password_var}"]'
            if self.password_var
            else f'"{self.password}"'
        )

    @property
    def host_variable(self) -> str:
        return f'os.environ["{self.host_var}"]' if self.host_var else f'"{self.host}"'
...
def build_from_str_validator_str(node_config: T.Optional[NodeConfig]) -> str:
    ...
    for field_name, props_config in node_config.basemodel_properties.items():
        if props_config.validate_as_basemodel is False:
            continue
        key = field_name
        if field_name in node_config.appendix_properties:
            key += "_"
        field_name_strs.append(f'"{key}"')
...
client = create_async_client(
    ...
    host={db_config.host_variable},
    password={db_config.password_variable},
    port={db_config.port}
)
    """
...
async def build_enums_from_config(db_config: DBConfig, include_strawberry: bool) -> str:
    client = edgedb.create_async_client(
        ...
        host=db_config.host_val,
        password=db_config.password_val,
        port=db_config.port,
    )
