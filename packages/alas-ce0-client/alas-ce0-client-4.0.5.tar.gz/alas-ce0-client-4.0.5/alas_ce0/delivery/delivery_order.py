...
def get_status_info(delivery_order):
    ...
    if delivery_order['status'] == DeliveryOrderStatusType.VirtuallyReceived.value:
        events_info = filter(
            lambda x: x['status'] == delivery_order['status'],
            delivery_order['events_info']
        )
    ...
    elif DeliveryOrderStatusType.Delivered.value in delivery_order['statuses']:
        events_info = filter(
    ...
    elif delivery_order['status'] == DeliveryOrderStatusType.DeliveryRejected.value:
        events_info = filter(
            lambda x: x['status'] == delivery_order['status'],
            delivery_order['events_info']
        )
        status = 'Entrega Rechazada'
        description = _get_schedule_rejected_comments(events_info[len(events_info) - 1]['info']) if \
            _get_schedule_rejected_comments(events_info[len(events_info) - 1]['info']) is not None \
            else "Entrega Rechazada"
