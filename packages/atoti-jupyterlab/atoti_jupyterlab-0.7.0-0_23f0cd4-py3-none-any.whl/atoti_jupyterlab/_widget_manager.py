...
from atoti_core import BaseSessionBound, keyword_only_dataclass, running_in_ipython

from ._comm_targets import WIDGET_COMM_TARGET_NAME
from ._mime_types import WIDGET_MIME_TYPE

if TYPE_CHECKING:
    from ipykernel.ipkernel import (  # pylint: disable=undeclared-dependency
        IPythonKernel,
    )
...
class WidgetManager:
    ...
    def __init__(self) -> None:
        ...
        from IPython import get_ipython  # pylint: disable=undeclared-dependency
    ...
    def display_widget(self, session: BaseSessionBound, name: Optional[str]) -> None:
        ...
        from ipykernel.comm import Comm  # pylint: disable=undeclared-dependency
        from IPython.display import (  # pylint: disable=undeclared-dependency
            publish_display_data,
        )

        data: Dict[str, Any] = {
            "text/plain": f"""Open the notebook in JupyterLab with the atoti extension enabled to {"see" if self._cell and self._cell.has_built_widget else "build"} this widget."""
        }
