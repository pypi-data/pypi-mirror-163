        ...
        if m: # store logline if start matches
            ...
            if m: # store the correlation
                ...
                if pat not in self.items:
                    ...
                    assert isinstance(self.items[pat], LogLine), f"Conflict found for {pat}"
...
correlations = [
    Correlator('Websocket calls',
        'sendWS -> (\S+-\d+)',
        'sendWS -> onSuccess -> (\S+-\d+)'
        ),
    Correlator("Downloads",
...
def run():
    ...
    parser.add_argument('--extra', default=False, type=bool, help="show extra log lines (non jsapp)", action=argparse.BooleanOptionalAction)
    parser.add_argument('-s', default=False, type=bool, help="show summary", action=argparse.BooleanOptionalAction)
    parser.add_argument('-n', default=False, type=bool, help="don't show log", action=argparse.BooleanOptionalAction)
    ...
    for line in gzip.open(args.logfile, 'rt', encoding='utf-8', errors='replace'):
        if finished:
            break
        # iso date is MUCH faster to parse, hence using JSapp ts
        ...
        if m:
            ...
            if end is not None:
                if end < ts:
                    finished = True
                    continue
            entry = LogLine(prefix=line[:m.end()+1].rstrip(), timestamp=ts, text=line[m.end()+1:].rstrip())
