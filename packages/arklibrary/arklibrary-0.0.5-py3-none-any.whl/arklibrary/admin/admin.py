...
from gamedriver import Driver
from gamedriver.blueprints import *


def initialize_admin(driver=None, password=None, player_id=None, map_name=None):
    if Admin.admin:
        return Admin.admin
    if password is None or player_id is None:
        with open("config.json", 'r') as r:
            data = json.load(r)["gamedriver"]
            if password is None:
                password = data["admin_password"]
            if player_id is None:
                player_id = data["admin_player_id"]
            if map_name is None:
                map_name = data["map_name"]

    return Admin(driver=driver, password=password, player_id=player_id, map_name=map_name)


class Admin:
    admin = None

    def __init__(self, driver=None, password=None, player_id=None, map_name=None):
        # ctrl-x to clear console
        # ctrl-c to copy console
        self.__password = password
        self.player_id = player_id
        self.map_name = map_name
        if self.__password is None or player_id is None:
            with open("config.json", 'r') as r:
                data = json.load(r)["gamedriver"]
                if self.__password is None:
                    self.__password = data["admin_password"]
                if self.player_id is None:
                    self.player_id = data["admin_player_id"]
                if self.map_name is None:
                    self.map_name = data["map"]
        Admin.admin = self

        assert self.__password is not None, "Admins must supply a password"
        if driver is None:
            driver = Driver()
        self.driver = driver
        self.command_list = []
