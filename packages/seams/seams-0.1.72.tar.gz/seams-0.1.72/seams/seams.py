...
class Seams(object):
    ...
    def create_vertex(self, 
        ...
        body = {}
        if attributes:
            body = self.__properties_formatter_name(vertex_label, vertex_name, attributes)
        try:
    ...
    def attach_edges(self, 
        ...
        try:
            response = self.__http_request(self.bearer, 'POST', '{}/tenant/{}/edge/attach'.format(self.URL, tenant_id), json=body)
            return response.text
        except:
            raise SeamsAPIException(response.text)
    ...
    def __properties_formatter_name(self, 
                               vertex_label,
                               vertex_name, 
                               args):
        '''
        Private helper function that formats a list of key value pairs for properties on a vertex, this one includes 
        vertex name for the create_vertex method
        '''
        for item in args:
            if item != 'status':
                if isinstance(args[item], list):
                    args[item] = json.dumps(args[item])
        return '{{"vertexLabel": "{}", "vertexName": "{}", "properties": {}}}'.format(vertex_label, vertex_name, json.dumps(args))
