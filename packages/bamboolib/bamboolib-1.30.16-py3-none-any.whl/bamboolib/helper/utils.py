...
def collapsible_notification(title, body, collapsed=True, type_="info"):
    """
    ...
    output = widgets.Accordion(children=[notification(body, type=type_)])
    output.set_title(0, title)
    output.add_class("bamboolib-collapsible-notification")
    output.add_class(f"bamboolib-collapsible-notification-{type_}")
...
def maybe_message_segment(message, details={}):
    ...
    analytics.track(user_id, message, details)
