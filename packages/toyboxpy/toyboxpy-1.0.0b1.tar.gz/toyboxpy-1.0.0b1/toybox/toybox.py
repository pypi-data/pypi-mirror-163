...
class Toybox:
    ...
    def installDependency(self, dep, lua_includes, makefiles, include_files, names_of_dep_with_extensions, force_install=False):
        ...
        if (force_install is False) and should_copy:
            ...
            if should_copy and force_install:
        ...
        if makefile_path is not None:
            makefiles.append(os.path.join(dep.subFolder(), makefile_path))

        include_file_path = Toybox.findIncludeFileIn(dep_folder, dep.repo_name)
        if include_file_path is not None:
            include_files.append(os.path.join(dep.subFolder(), include_file_path))
            names_of_dep_with_extensions.append(dep.repo_name)

        box_file = Boxfile(dep_folder)
        for child_dep in box_file.dependencies:
            self.installDependency(child_dep, lua_includes, makefiles, include_files, names_of_dep_with_extensions, force_install)
    ...
    def update(self):
        ...
        try:
            ...
            makefiles = []
            include_files = []
            names_of_dep_with_extensions = []

            box_file = Boxfile(Toybox.boxfileFolder())
            for dep in box_file.dependencies:
                self.installDependency(dep, lua_includes, makefiles, include_files, names_of_dep_with_extensions)

            if os.path.exists(Toybox.toyboxesFolder()):
                Toybox.generateReadMeFile()

                if len(lua_includes) != 0:
                    Toybox.generateLuaIncludeFileWith(lua_includes)

                if len(makefiles) != 0:
                    Toybox.generateMakefileWith(makefiles)

                if len(include_files) != 0:
                    Toybox.generateIncludeFileWith(include_files, names_of_dep_with_extensions)
    ...
    def printUsage(cls):
        Toybox.printVersion()
        print('Usage:')
        print('    toybox.py help                      - Show a help message.')
        print('    toybox.py version                   - Get the Toybox version.')
        print('    toybox.py info                      - Describe your dependency set.')
        print('    toybox.py add <dependency>          - Add a new dependency.')
        print('    toybox.py remove <dependency>       - Remove a dependency.')
        print('    toybox.py update                    - Update all the dependencies.')
        print('    toybox.py update <dependency>       - Update a single dependency.')
    ...
    def generateReadMeFile(cls):
        with open(os.path.join(Toybox.toyboxesFolder(), 'README.md'), 'w') as out_file:
            ...
            out_file.write('\n')
            out_file.write('Please [install](https://github.com/DidierMalenfant/toybox.py#installing) **toybox.py** in order to modify or update the content of this folder.\n')

    @classmethod
    def generateLuaIncludeFileWith(cls, lua_includes):
        with open(os.path.join(Toybox.toyboxesFolder(), 'toyboxes.lua'), 'w') as out_file:
            out_file.write('--\n')
            out_file.write('--  toyboxes.lua - include file auto-generated by toybox.py\n')
            out_file.write('--\n')
            out_file.write('\n')

            for lua_include in lua_includes:
                out_file.write('import \'' + lua_include + '\'\n')

            out_file.close()

    @classmethod
    def generateMakefileWith(cls, makefiles):
        with open(os.path.join(Toybox.toyboxesFolder(), 'toyboxes.mk'), 'w') as out_file:
            out_file.write('--\n')
            out_file.write('--  toyboxes.mk - include file auto-generated by toybox.py\n')
            out_file.write('--\n')
            out_file.write('\n')
            ...
            for makefile in makefiles:
                out_file.write('include $(_RELATIVE_DIR)/' + makefile + '\n')

            out_file.close()

    @classmethod
    def generateIncludeFileWith(cls, include_files, names_of_dep_with_extensions):
        with open(os.path.join(Toybox.toyboxesFolder(), 'toyboxes.h'), 'w') as out_file:
            out_file.write('--\n')
            out_file.write('--  toyboxes.h - include file auto-generated by toybox.py\n')
            out_file.write('--\n')
            out_file.write('\n')

            for include in include_files:
                out_file.write('include "' + include + '"\n')

            out_file.write('\n')

            prefix = ''
            out_file.write('#define REGISTER_TOYBOXES(pd)')
            for name in names_of_dep_with_extensions:
                out_file.write(prefix + '   register_' + name + '(pd);')
                prefix = ' \\\n                             '
