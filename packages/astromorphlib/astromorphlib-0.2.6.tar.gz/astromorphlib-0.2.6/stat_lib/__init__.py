...
def mass_decals(gal, tab_gal, size_image_phy, outputfile='properties.dat'):
    ...
    home = 'https://www.legacysurvey.org/viewer/fits-cutout?'
    path = 'fits_images/{}_{}_{}kpc.fits'.format(gal[:-1], 'g', size_image_phy)
    if not os.path.exists(path):
        print (f"\nDowloading {path} ...")
    ...
    path = 'fits_images/{}_{}_{}kpc.fits'.format(gal[:-1], 'r', size_image_phy)
    if not os.path.exists(path):
        print (f"\nDowloading {path} ...")
        part0 = 'ra={}&dec={}&layer=dr8-south&pixscale=0.27&bands=r&size={}'.format(
             tab_gal['ra'][idg], tab_gal['dec'][idg], int(size_image))
    ...
    imagn = fits.open('fits_images/{}_{}_{}kpc.fits'.format(gal[:-1], 'g',
                      size_image_phy))
    imag = imagn[0].data

    imaxy = pfunc.callf('fits_images/{}_{}_{}kpc.fits'.format(gal[:-1], 'g',
                      size_image_phy))[0]


    imarn = fits.open('fits_images/{}_{}_{}kpc.fits'.format(gal[:-1], 'r',
                      size_image_phy))
    imar = imarn[0].data
