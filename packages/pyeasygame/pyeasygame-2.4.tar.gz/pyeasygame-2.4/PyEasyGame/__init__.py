...
def pickup_item(image,xy,condition,pickup_value,rect1=(0,0,0,0),rect2=(0,0,0,0)):
    mx,my = pygame.mouse.get_pos()
    mouse = pygame.mouse.get_pressed()
    pir = list(xy)
...
def draw_image(image, xy, image_value=0,camera=True,draw_img=True):
...
def draw_rect(xy,width,height,color,draw_rect=True,rect_value=0,radius_boolean=False, radius_value=0):
...
def player(image,xy,move_left='a',move_right='d',move_up='w',move_down='s',jump=False,speed=3,jump_height=10, collide_sides=False, sticky_keys=True, camera_follower=False):
    image_player = None
