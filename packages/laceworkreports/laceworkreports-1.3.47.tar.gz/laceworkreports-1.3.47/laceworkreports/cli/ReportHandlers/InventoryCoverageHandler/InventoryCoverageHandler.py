...
def html(
    ...
    report_type: InventoryReportType = typer.Option(
        InventoryReportType.AWS.value, help="Inventory report type"
    ),
    ...
    template_path: str = typer.Option(
        Path(__file__).resolve().parent.joinpath("inventory_coverage.html.j2"),
    ...
    # report details
    report_title = "Inventory Coverage"
    db_table = "inventory"
    ...
    for lwAccount in lwAccounts:
        ...
        logging.info("Syncing inventory")
        ...
        # # iterate through each csp type
        # for csp in ["aws", "gcp", "az" ]:

        reportHelper.get_account_inventory(
            client=lw,
