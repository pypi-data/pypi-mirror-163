...
class _ODFVPipelineBuilder(_PipelineBuilder):
    ...
    def pandas_udf_wrapper(self, *args):
        ...
        SPARK_TYPE_NAME_TO_PANDAS_TYPES = {
            LongType.typeName(): "int64",
            DoubleType.typeName(): "float64",
            StringType.typeName(): "object",
            BooleanType.typeName(): "bool",
            ArrayType.typeName(): "object",
            StructType.typeName(): "object",
        }
        ...
        def _convert_output_array(arr: Union[List, np.array], field: StructField) -> List:
            assert isinstance(field.dataType, ArrayType), f"Unexpected field data type {field.dataType}"
            ...
            elif element_type in (FloatType(), DoubleType()):
                if isinstance(arr, list):
                    arr = np.array(arr)
            ...
            elif element_type == StringType() or isinstance(element_type, StructType):
                # If the array element type is an object type (e.g. a string or dict/struct) and the array is not a list
                # (it may be a numpy array), then convert it to a list.
                if isinstance(arr, list):
                    return arr
                else:
                    return arr.tolist()
        ...
        for field in self._output_schema:
            ...
            assert SPARK_TYPE_NAME_TO_PANDAS_TYPES[field.dataType.typeName()] == output_df[field.name].dtype.name, (
                f"DataFrame field '{field.name}' returned from '{self._name}' has type '{output_df[field.name].dtype.name}', "
                f"but is expected to have Pandas dtype '{SPARK_TYPE_NAME_TO_PANDAS_TYPES[field.dataType.typeName()]}' (corresponding to Spark type {field.dataType.__class__.__name__}). "
