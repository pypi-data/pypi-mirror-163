...
class FeatureView(FeatureDefinition):
    ...
    def get_historical_features(
        ...
        """
        Returns a Tecton :class:`tecton.DataFrame` of historical values for this feature view.
        ...
        if conf.get_bool("ALPHA_ATHENA_COMPUTE_ENABLED") and utils.is_live_workspace(self.workspace):
            if not utils.is_live_workspace(self.workspace):
...
class CompatFeatureView(FeatureView):
    ...
    def preview(self, limit=10, time_range: Optional[pendulum.Period] = None, use_materialized_data: bool = True):
        ...
        :param time_range: Time range to collect features from. Will default to recent data (past 2 days).
        :param use_materialized_data: Use materialized data if materialization is enabled.
    ...
    def _assert_writes_to_offline_feature_store(self):
        if not self._writes_to_offline_feature_store:
            raise core_errors.FV_NEEDS_TO_BE_MATERIALIZED(self.name)
    ...
    def get_feature_dataframe(
        ...
        """
        Deprecated.
        Returns a Tecton :class:`tecton.DataFrame` that contains the output Feature Transformation of the Feature View.
        ...
        :param spine_time_key: Name of the time column in spine.
            If unspecified, will default to the time column of the spine if there is only one present.
        :param use_materialized_data: Use materialized data if materialization is enabled
        :param save: set to True to persist DataFrame as a Dataset object
        :param save_as: name to save the DataFrame as. Not applicable when save=False.
            If unspecified and save=True, a name will be generated.
        :return: A Tecton :class:`tecton.DataFrame`.
        """
        logger.warning(
    ...
    def get_features(
        ...
        :param entities: Filter feature data to a set of entity IDs.
            If specified, this DataFrame should only contain join key columns.
        :param start_time: The interval start time from when we want to retrieve features.
        :param end_time:  The interval end time until when we want to retrieve features.
        :param from_source: Whether feature values should be recomputed from the original data source.
    ...
    def run(
        ...
        **mock_inputs: Union[pandas.DataFrame, DataFrame],
        ...
        :param feature_start_time: Start time for the feature. mock_inputs and linked DataSources will be filtered in
            ...
            If not set, default to feature_end_time minus materialization schedule interval.

        :param feature_end_time: End time for input data (both data sources, and mock_inputs). mock_inputs and linked
            DataSources will be filtered with respect to providing neccessity inputs for this feature time.
            The output values with timestamps later than this will be dropped.
            If not set, default to datetime now, at the time of a run.
