...
SELECT
    {{ all_spine_join_keys }}
    {%- if spine_contains_non_join_keys %},_TT_SPINE_TABLE.*{%- endif -%}{%- if feature_set_items|length > 0 %},{%- endif -%}
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                {# Use the closest recent tile start time as the feature timestamp for aggregation features #}
                {%- if item.aggregation.slide_interval.ToSeconds() > 0 -%}
                    DATE_ADD('SECOND', -MOD(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }}), _TT_SPINE_TABLE.{{ spine_timestamp_key }}) AS "{{ final_select_name(item.name + separator + item.timestamp_key, false, true) }}"
                {%- else %}
