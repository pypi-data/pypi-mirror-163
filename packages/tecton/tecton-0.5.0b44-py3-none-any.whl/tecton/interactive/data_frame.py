...
class DataFrame(object):
    ...
    def _explain(self, extended: bool = False):
        """Prints the query plan"""
        if self._spark_df:
            self._spark_df.explain(extended=extended)
        elif self._querytree:
            print(self._querytree.pretty_print(verbose=extended))
    ...
    @classmethod
    def _from_id(cls, x):
        """Creates a Tecton DataFrame from a subtree of prior querytree labeled by an id in ._explain()."""
