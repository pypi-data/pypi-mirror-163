...
class _SciLineEditQt(QtWidgets.QLineEdit):
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        ...
        if (key == Qt.Key_Return) or (key == Qt.Key_Enter):
            ...
            self._toggle_auto_prefix_mode()
        elif (key == Qt.Key_Backspace) or event.matches(QtGui.QKeySequence.Paste):
            super().keyPressEvent(event)
            self._handle_selection()
        elif event.matches(QtGui.QKeySequence.SelectAll) or event.matches(QtGui.QKeySequence.Copy):
            super().keyPressEvent(event)
            self._handle_selection()
...
class NSFSciEdit(QtWidgets.QWidget):
    """ Custom Qt Widget to show scientific numbers in a edit with a descriptiv label. """
    value_changed_event = Signal(float)
