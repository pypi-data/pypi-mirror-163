...
@app.command()
def play_game(
        task: Task = typer.Option(default="simulate", help="Whether judge should test a game or replay existing game using game logs",),
        ...
        visualize: bool = typer.Option(
            default=False,
            help="Whether to print out board states and moves when running simulator. By default true when running for either play or replay tasks"),
    ...
    if task != Task.SIMULATE:
        ...
        game_simulator = Simulator(visualize=visualize, logging_level=log_level)
    ...
    try:
        if task == Task.SIMULATE:
            ...
            log_serializer.dump(player_moves=player_moves, winner=winner, error_message=None)
        ...
        if task == task.SIMULATE:
            log_serializer.dump(player_moves=player_moves, error_message=ex.message)
        ...
        logger.error(ex)
        if task == Task.SIMULATE:
            log_serializer.dump(player_moves=player_moves, error_message=str(ex))
