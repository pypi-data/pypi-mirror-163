...
class Sf:
    ...
    def authenticate(self, sfUsername: Optional[str]=None, 
        ...
        self.bulk = bulk
        # SFDC Metadata API
        sfMeta = SfdcSession(session_id=self.session_id, instance=".".join(self.instance.split(".")[:2]))
        self.sfMeta = sfMeta
...
#                     return pd.DataFrame(columns=results)
    ...
    async def async_query(query, excludedFields: Optional[List]= []) -> pd.DataFrame:
