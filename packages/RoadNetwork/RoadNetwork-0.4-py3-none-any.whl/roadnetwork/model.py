...
utilities.get_subclasses(Abstract_weight)
utilities.get_subclasses(Abstract_static_weight)
utilities.get_subclasses(Abstract_dynamic_weight)
utilities.get_list_function_name(Weight_Shortest_Distance)

utilities=Utilities()
utilities.get_list_function_name(Cars) , utilities.get_class_attribute(class_object)

class_object = Cars(0,0,0,0)
functions_list = utilities.get_list_function(Weight_Shortest_Distance)
for function_name in functions_list : print(function_name,"\n"), utilities.show_information(function_name) , print("\n ############################################ \n")
    ...
    def run(self,dt,time,flow_function,proportion_types,length,t_data=1,t_plot=1,random=False,density=False,show=False,label=False,video=False) :
        ...
        for i , flow in enumerate(entry_flow) :  
            ...
            if show and (i % n_show == 0) or (i == Nmax)  :
                ...
                plt.title(title) 
                if density : figure = simulate_network.draw_density(label)
                else : figure = simulate_network.draw_car(label)            
                display(plt.gcf())
                sleep(0.5)
                plt.clf()
                plt.close() 
