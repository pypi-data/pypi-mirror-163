...
def handle_save(data: DataFrame, name, fmt='json', orient='list'):
...
def get_hash_list(collection_creator, fmt, use_downloaded):
...
@click.option('--rpc', '-r', default=get_rpc(), callback=rpc_handler, prompt='Desired RPC URL', expose_value=True)
@click.option('--use-downloaded', '-dl', is_flag=True)
def get_owners(collection_creator, fmt, rpc, use_downloaded):
    mint_ids: List[List[str]] = asyncio.run(download_collection_data(collection_creator=collection_creator,
                                                                     rpc=get_rpc(),
                                                                     include_owner=True,
                                                                     include_token_acct=False,
    ...
    handle_save(data=grouped, name=f'owners_{collection_creator}', fmt=fmt, orient='records')
