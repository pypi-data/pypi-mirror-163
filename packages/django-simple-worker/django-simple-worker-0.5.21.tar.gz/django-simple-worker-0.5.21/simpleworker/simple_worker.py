...
class SimpleWorker:
    ...
    def should_skip_due_to_exclusivity(self, task: SimpleTask) -> bool:
        ...
        currently_processing_with_same_name = """
              ...
              AND name = %s
                      """
        currently_processing = list(Task.objects.raw(currently_processing_with_same_name, [task.get_name()]))
    ...
    def handle_task(self, task: Task):
        ...
        try:
            ...
            if self.should_skip_due_to_exclusivity(simple_task):
                logger.info(f'Task {task_name} id {task_id} is exclusive and task with the same name is already running. Will retry after 60 seconds.')
