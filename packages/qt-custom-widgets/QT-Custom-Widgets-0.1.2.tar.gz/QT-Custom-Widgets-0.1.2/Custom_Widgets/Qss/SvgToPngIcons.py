...
class NewIconsGenerator():
    ...
    def generateNewIcons(self, progress_callback):
        ...
        if settings.value("ICONS-COLOR") is None:
            ...
            oldIconsDestinationFolder = os.path.abspath(
                os.path.join(os.getcwd(), 'QSS/' + variablesFile['icons-color'].replace("#", "")))
        else:
            oldIconsDestinationFolder = os.path.abspath(
                os.path.join(os.getcwd(), 'QSS/' + settings.value("ICONS-COLOR").replace("#", "")))
        ...
        if not settings.value("ICONS-COLOR") == normal_color and color["icons-color"] is not None:
            ...
            for name in list_of_files:
                ...
                if os.path.exists(os.path.join(oldIconsFolder, name_2)) and not os.path.exists(
                        os.path.join(oldIconsDestinationFolder, name_2)):
            ...
            for name in list_of_files:
                ...
                with codecs.open(name, encoding='utf-8', errors='ignore') as f:
                    ...
                    if os.path.exists(filename):
                        ...
                        if settings.value("ICONS-COLOR") is not None and settings.value("ICONS-COLOR").replace("#",
                                                                                                               "") == iconsFolderName:
                            continue
                    try:
                        # Convert each SVG icon to png
