...
def build_nexus_from_edf(filename):
    ...
    entry.attrs['default'] = 'data'
    entry.attrs['version'] = '1.0'
    entry.title = nx.NXfield(value=header['Comment'])
...
def build_rxnexus_from_edf(fileList, directBeamFile, outputFile,
                           offset=0, length=20, chi=0):
    fileObj = fabio.open(fileList[0])
    header = fileObj.header
    root = nx.NXroot()
    root.attrs['default'] = 'raw_data'
    root.attrs['NX_class'] = b'NXroot'
    entry = nx.NXentry()
    entry.title = nx.NXfield(value=header['Comment'])
    entry.run = nx.NXfield(value=header['run'])
    ...
    collimator = nx.NXcollimator(length=nx.NXfield(1200, units='mm'),
                                 ...
                                 s1hl=nx.NXfield(float(header['s1hl']), attrs={'units': 'mm'}),
                                 s1hr=nx.NXfield(float(header['s1hr']), attrs={'units': 'mm'}),
                                 s2bot=nx.NXfield(float(header['s2bot']), attrs={'units': 'mm'}),
                                 s2top=nx.NXfield(float(header['s2top']), attrs={'units': 'mm'}),
                                 s2hl=nx.NXfield(float(header['s2hl']), attrs={'units': 'mm'}),
                                 s2hr=nx.NXfield(float(header['s2hr']), attrs={'units': 'mm'}))
    # instrument/detector
    dist = float(header['SampleDistance']) * 1000
...
#
    sample = nx.NXsample(sample_name=header['Comment'],
                         length=nx.NXfield(float(length), attrs={'units': 'mm'}),
                         x_position=nx.NXfield(float(header['x']), attrs={'units': 'mm'}),
                         y_position=nx.NXfield(float(header['z']), attrs={'units': 'mm'}),
                         om=nx.NXfield(np.zeros(len(fileList))), attrs={'units': 'deg'},
                         phi=nx.NXfield(float(header['phi']), attrs={'units': 'deg'}),
                         rx=nx.NXfield(float(header['rx']), attrs={'units': 'deg'}),
                         ry=nx.NXfield(float(header['ry']), attrs={'units': 'deg'}),
                         temperature=nx.NXfield(float(header['Temperature']), attrs={'units': '°C'}),
                         count_time=nx.NXfield(np.zeros(len(fileList)), attrs={'units': 's'}),
                         description=header['Comment'],
                         offset=nx.NXfield(float(offset), attrs={'units': 'deg'}),
                         chi=nx.NXfield(chi, attrs={'units': 'deg'})
                         )
    entry.insert(sample)
    nFile = len(fileList)
    data = np.empty((nFile, fileObj.data.shape[0], fileObj.data.shape[1]))
    om = np.empty(nFile)
    count_time = np.empty(nFile)
    for i in range(nFile):
        fileObj = fabio.open(fileList[i])
        header = fileObj.header
        data[i, :, :] = fileObj.data
        count_time[i] = float(header['count_time'])
        om[i] = float(header['om'])
    entry.image_stack = nx.NXdata(nx.NXfield(data, name='data'))
    sample.count_time.nxdata = count_time
    sample.om.nxdata = om
    # directBeam
    db_entry = nx.NXsubentry()
    dbFileobj = fabio.open(directBeamFile)
    header = dbFileobj.header
    db_entry.count_time = nx.NXfield(float(header['count_time']), attrs={'units':'s'})
    db_entry.data = nx.NXdata(nx.NXfield(dbFileobj.data, name='data'))
    db_entry.x0 = nx.NXfield(float(header['Center_1']), attrs={'units': 'pixel'})
    db_entry.y0 = nx.NXfield(float(header['Center_2']), attrs={'units': 'pixel'})
    db_entry.roi_width = nx.NXfield(20, attrs={'units': 'pixel'})
    db_entry.roi_height = nx.NXfield(10, attrs={'units': 'pixel'})
    db_entry.attrs['default'] = 'data'
    entry.direct_beam = db_entry


    #save entry0
    entry.attrs['default'] = 'image_stack'
    root.raw_data = entry
    try:
...
@decorator.decorator
def treatment_function(func, new_entry=True, output_file=False,
                       target_entry=None, *args, **kwargs):
    """
...
@decorator.decorator
def rxtreatment_function(func, output_file=False, *args, **kwargs):
    """
    decorator for nexus file treatment functionfor x ray reflectivity on xeuss.
    Args:
        func (object): treament function handle
        output_file (bool): creat a new file, the function does not modify
        *args: treatment function arguments
...
if __name__ == '__main__':
    ...
    # directeamFile = 'TH1_10_1000_pos1_0_54362.edf'
    # fileList = []
    ...
    # fileList.pop(0)
    #
    # root = build_rxnexus_from_edf(fileList, os.path.join(folder, directeamFile),
    #                               os.path.join(folder, 'TH1_10_1000_pos1.nxs'))

    ############################################################################
    # pos1
    ############################################################################
    folder = '/home/achennev/Bureau/PIL pour tiago/RX_tiago/9_11_2020/ps6k_10gpl_THFPGMEA_bis_pos1'
    fileList = []
    for file in os.listdir(folder):
        if file.endswith(".edf"):
            fileList.append(os.path.join(folder, file))
    fileList.sort()
    directeamFile = fileList.pop(0)
    root = build_rxnexus_from_edf(fileList, directeamFile,
                                  os.path.join(folder, 'PS6k_10gpl_THFPGMEA_bis_pos1.nxs'))
    root.close()
    ############################################################################
    # pos2
    ############################################################################
    folder = '/home/achennev/Bureau/PIL pour tiago/RX_tiago/9_11_2020/wafer_nue_pos2'
    # directeamFile = 'TH10_10_1_pos2_0_54964.edf'
    fileList = []
    for file in os.listdir(folder):
        if file.endswith(".edf"):
            fileList.append(os.path.join(folder, file))
    fileList.sort()
    directeamFile = fileList.pop(-1)
    root = build_rxnexus_from_edf(fileList, directeamFile,
                                  os.path.join(folder, 'wafer_nue_pos2.nxs'))
