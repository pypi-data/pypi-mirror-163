...
// ------------------------------------------------------------------------------------------------------------------------------- //
		...
		console.error(
			`Error on POST /MLProvCodeGen/`+requestname+` ${objBody}.\n${reason}`
		);
    } 
}
...
// setup and HTTP Request test; used to check if the server extension is enabled locally/ ob Binder
    if (settingRegistry) {
      settingRegistry
        ...
        .catch(reason => {
          console.error('Failed to load settings for MLProvCodeGen.', reason);
        });
    }
    ...
    requestAPI<any>('get_example')
      ...
      .catch(reason => {
        console.error(
          `The MLProvCodeGen server extension appears to be missing.\n${reason}`
        );
      });
	...
	provInputFlex.addEventListener('change', event => {
		...
		document.getElementById('provenanceSubmit')!.addEventListener('click', async event => {
			...
			reader.onload = async function() {
				...
				var path = window.location.href + '/tree/MLProvCodeGen/GeneratedNotebooks/'
				var notebookPath = "('"+path+taskName+".ipynb', 'MLProvCodeGen')";
				console.log('path:' +path);
				//var notebookPath = "('http://localhost:8888/lab/tree/MLProvCodeGen/GeneratedNotebooks/"+provenanceDataObj.entity.experiment_info['experimentinfo:task_type']+".ipynb', 'MLProvCodeGen')";
...
// ------------------------------------------------------------------------------------------------------------------------------- //
			...
			if (reply['greetings'] === 'success') {
				var path = window.location.href + '/tree/MLProvCodeGen/GeneratedNotebooks/ImageClassification_PyTorch.ipynb'
				console.log(path)
...
// ------------------------------------------------------------------------------------------------------------------------------- //
			...
			console.log(reply);
			if (reply["greetings"] === 'success') {
				var path = window.location.href + '/tree/MLProvCodeGen/GeneratedNotebooks/MulticlassClassification.ipynb'
				console.log(path)
				const success_message = document.createElement('text');
				content.node.appendChild(success_message);
				success_message.textContent =
