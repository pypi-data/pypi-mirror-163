...
from tailors import move_to_device
...
from tailors_trainer import callbacks, losses, optimizers, schedulers
...
class Trainer:
    ...
    def __init__(self, train_conf: TrainConf) -> None:
        super().__init__()
        self.train_conf = train_conf
        self.trainer_conf = TrainerConf()
    ...
    def get_model(self) -> Tailors:
        ...
        return model_class(model_conf).use_device(self.get_device())
    ...
    def get_optimizer(self):
        ...
        lr_factors = getattr(self.model, 'lr_factors', None)
        if lr_factors is None:
            return optimizers.get(self.train_conf.optimizer, self.model.parameters())

        no_decay = ['bias', 'gamma', 'beta', 'LayerNorm.weight', 'LayerNorm.bias']
        rates = {
            name: min(min_value, self.train_conf.lr * factor) if min_value else self.train_conf.lr * factor
            for name, (factor, min_value) in lr_factors().items()
        }

        no_decay_suffix = '_no_decay'
        groups_default, groups_named, group_meta = collections.defaultdict(list), collections.defaultdict(list), dict()
            ...
            for batch_id, (data, target) in enumerate(batches):
                ...
                if self.scaler:
                    ...
                    if is_optimizer_step:
                        ...
                        batches.set_postfix({'loss/loss': f"{loss:.4f}", 'ts': self.state.ts})
                ...
                self.state.set_metric('loss/loss', loss)
    ...
    def compute_loss(self, encoded, decoded, target):
        criteria = getattr(self.model, 'compute_loss', None)
        if criteria:
            return criteria(encoded, decoded, target)
        else:
            LOGGER.warning('default criteria might not work')
            return self.criteria(decoded, target)

    def compute_metrics(self, preds, targets):
        if hasattr(self.model, 'compute_metrics'):
            metrics = getattr(self.model, 'compute_metrics')(preds, targets)
        else:
            metrics = classification_metrics.calculate(preds, targets)
        self.state.set_metrics(metrics)

    def lr_scheduler_step(self):
        scheduler_args = {'metrics': self.state.metrics.get('loss/train'), 'epoch': self.state.epoch}
