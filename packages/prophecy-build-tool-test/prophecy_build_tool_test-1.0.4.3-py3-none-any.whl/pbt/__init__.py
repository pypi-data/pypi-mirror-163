...
class ProphecyBuildTool:
        ...
        for pipeline_i, (path_pipeline, pipeline) in enumerate(self.pipelines.items()):
            ...
            if rc == 0:
                if (self.project_language == 'python'):
                    path_pipeline_dist = path_pipeline_absolute + '/distrib'
    ...
    def build_python(self, path_pipeline_absolute, pipeline):
        return self._process_sequential(
            [
                ...
                # Generate wheel
                self.process(['python', 'setup.py', 'bdist_wheel', f'--dist-dir ${path_pipeline_absolute}/distrib/'], path_pipeline_absolute)
            ]
        )

    def build_scala(self, path_pipeline_absolute):
        return self._process_communicate(
            subprocess.Popen(['mvn', 'clean', 'package', '-q', '-DskipTests'], stdout=subprocess.PIPE,
    ...
    def _process_sequential(self, subprocesses):
        for process in subprocesses:
            rc = self._process_communicate(process)
    ...
    def process(self, process_args, cwd, std_output=subprocess.PIPE, std_input=subprocess.PIPE, is_shell=False):
        return subprocess.Popen(process_args, stdout=std_output, stderr=std_input, shell=is_shell, cwd=cwd)
