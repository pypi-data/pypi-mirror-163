...
class Filter(BaseFilter):
    ...
    def on_image_ready(  # pylint: disable=too-many-statements,too-many-branches,too-many-locals
        ...
        logger.debug (image_size);
        logger.debug (watermark_sz);
        watermark_sz =  (image_size[0] / 8, watermark_sz[1]/watermark_sz[0] * (image_size[0]/8));
        logger.debug (watermark_sz);
        ...
        self.x = str(math.ceil(watermark_sz[0]/2));
    ...
    async def watermarkdt(self, url, alpha):
        self.url = url
        self.alpha = alpha
        ...
        try:
            ...
            if buffer is not None:
                logger.debug ("buffer cache of logo ok => not download new logo");
                return self.on_image_ready(buffer)
            logger.debug ("process for buffer no");
