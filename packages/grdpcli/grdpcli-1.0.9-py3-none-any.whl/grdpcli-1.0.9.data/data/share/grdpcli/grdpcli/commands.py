...
def command_help():
    ...
    with open(f"{help_filepath}/help.py", "r") as help_file:
         console.print(Markdown(help_file.read()))
    sys.exit(0)

def command_version():
    version = pkg_resources.require("grdpcli-test")[0].version
    logger.info(f"grdp-cli version: {version}")
...
def commandHandler(commands):
    ...
    """
    SUBCOMMANDS = ['update','projects', 'help', '--help', 'version', '--version']
    if len(commands) > 1:
        if commands[1] in SUBCOMMANDS:
            ...
            if commands[1] == 'projects':
                command_projects()
            if commands[1] == 'version' or commands[1] == '--version':
                command_version()
        else:
            del commands[0]
