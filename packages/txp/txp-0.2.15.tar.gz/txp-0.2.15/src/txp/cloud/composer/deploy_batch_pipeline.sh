...
cd ../
rm -rf dist
gsutil cp ./pipelines/batch_pipeline/setup.py $composer_dags_folder
gsutil cp ./pipelines/batch_pipeline/batch_pipeline.py $composer_resources_batch_pipeline_folder
...
if [[ ( $new_version != "") ]]; then
  ...
  sed -i "s/txp-cloud-utils==[0-9].*/txp-cloud-utils==$new_version/" ./composer/requirements.txt
  sed -i "s/version = \"[0-9].*\"/version = \"$new_version\"/" pyproject.toml

  poetry build
  twine upload dist/* -u tranxpert -p txp-1234 #publish pypi package
  cd ./composer
