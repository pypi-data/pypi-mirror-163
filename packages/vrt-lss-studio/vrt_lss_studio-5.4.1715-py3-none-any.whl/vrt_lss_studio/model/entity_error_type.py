...
class EntityErrorType(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Args:
            args[0] (str): Error type:   * `INCONSISTENT_REFERENCE` - bad reference key    * `UNIQUE_IDS_VIOLATION` - all keys must be unique   * `INVALID_ROUTING_MATRIX` - bad routing matrix dimension   * `COMPATIBILITY_CARGO_BOX` - no cargo is compatible with any box   * `COMPATIBILITY_TRANSPORT_PERFORMER` - no transport is compatible with any performer   * `COMPATIBILITY_TRANSPORT_LOCATION` - no transport is compatible with any location   * `COMPATIBILITY_ORDER_PERFORMER` - no order is compatible with any performer     ., must be one of ["INCONSISTENT_REFERENCE", "UNIQUE_IDS_VIOLATION", "INVALID_ROUTING_MATRIX", "COMPATIBILITY_CARGO_BOX", "COMPATIBILITY_TRANSPORT_PERFORMER", "COMPATIBILITY_TRANSPORT_LOCATION", "COMPATIBILITY_ORDER_PERFORMER", ]  
