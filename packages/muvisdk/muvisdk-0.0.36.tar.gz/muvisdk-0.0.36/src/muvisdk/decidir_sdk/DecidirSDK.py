...
class DecidirSDK:
    def __init__(self, merchant: dict, merchant_cadena: dict):
        if 'credentials' not in merchant or 'decidir' not in merchant['credentials']:
    ...
    def payment(self):
        return Payment(self.url, self.private_key, self.public_key, self.merchant_name, self.site_id,
                       self.site_id_cadena)
