...
class Payment:
    ...
    def create(self, payment_data: dict):
        ...
        if payment_response['status'] < 400:
            return ok(_format(payment_response['response']), payment_response['status'])

        payment_response['response']['processor'] = 'mercadopago'
        return error(payment_response['response'], payment_response['status'])

    def get(self, payment_id: int):
        payment_response = self.sdk.payment().get(payment_id)
        if payment_response['status'] < 400:
            return ok(payment_response['response'], payment_response['status'])
