...
class Card:
    ...
    def list_all(self, customer_id: str):
        ...
        if mp_response['status'] > 299:
            return error({'message': 'Customer not found'}, mp_response['status'])
        cards = mp_response['response']
        return ok([_format(card) for card in cards], mp_response['status'])
