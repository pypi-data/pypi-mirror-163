...
from hypernova.solvers import SolverSolution, RK4, RK45, VERLET, YOSHIDA4, YOSHIDA8
...
def propagate_orbit(problem, solver, timestep: float, tol: float) -> SolverSolution:
    ...
    start = perf_counter()
    solution = SolverSolution(propagate_orbit_c(problem, casted_solver, solver_type, timestep, tol))
    end = perf_counter()
    getLogger('hypernova').info('Done. Time: {:.4f} seconds.'.format(end - start))
