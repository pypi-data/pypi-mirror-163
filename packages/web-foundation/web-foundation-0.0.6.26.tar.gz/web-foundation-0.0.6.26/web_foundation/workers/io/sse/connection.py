...
class SseRtConnection(RtConnection):
    ...
    async def accept_connection(cls, input_ctx: InputContext, ping_enable: bool = True, ping_timeout=None,
                                listen_timeout=None, debug: bool = False) -> SseRtConnection:
        writeable = await cls._construct_writable(input_ctx)
        instance = cls(input_ctx=input_ctx, writeable=writeable, ping_enable=ping_enable, ping_timeout=ping_timeout,
                       listen_timeout=listen_timeout, debug=debug)
        return instance
