...
from heaserver.service.testcase.collection import CollectionKey, query_fixture_collection, get_key_from_name
...
if __name__ == '__main__':
    ...
    db_store_[CollectionKey(name='awsaccounts', db_manager_cls=S3Manager)] = db_store_.pop(get_key_from_name(db_store_, 'awsaccounts'))
    aws_account = args.account
    if aws_account is not None:
        my_org = next((org for org in query_fixture_collection(db_store_, 'organizations', strict=False) or [] if org['id'] == '666f6f2d6261722d71757578'), None)
    ...
    swaggerui.run(project_slug='heaserver-organizations', desktop_objects=db_store_, db_manager_cls=DockerMongoManager,
            ...
            (get, '/organizations/{id}/awsaccounts/', service.get_organization_aws_accounts)
                  ...
                  other_microservice_images=[DockerContainerConfig(image=HEASERVER_ACCOUNTS_IMAGE,
                                                                   port=8080,
                                                                   check_path='/ping',
                                                                   db_manager_cls=S3Manager,  # Change to DockerMongoManager for heaserver-accounts version 1.0.0a9 or earlier.
                                                                   resources=[Resource(
                                             ...
                                             DockerContainerConfig(image=HEASERVER_VOLUMES_IMAGE,
                                                                   port=8080,
                                                                   check_path='/volumes',
