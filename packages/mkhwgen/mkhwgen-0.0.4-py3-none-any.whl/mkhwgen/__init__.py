...
def rain_part(rr, old_rain_total):
    ...
    rain_total = 0
    if old_rain_total > 0 and pww >= random.random():
        rain_total = 1
    if old_rain_total == 0 and pwd >= random.random():
        rain_total += 1
...
def wgen(year, monthly_tmin, monthly_tmax, monthly_rain):
    ...
    for month, dates in monthly_dates:
        ...
        for i, current_date in enumerate(dates):
            ...
            dry_tmin, dry_tmax, old_tmin, old_tmax = temperature_part('dry', jday, dry_tmin_mean, dry_tmax_mean, old_tmin, old_tmax)
            wet_tmin, wet_tmax, old_tmin, old_tmax = temperature_part('wet', jday, wet_tmin_mean, wet_tmax_mean, old_tmin, old_tmax)

            generated_rain, old_rain_total = rain_part(rr, old_rain_total)

            if old_rain_total >= 1:
                generated_tmin, generated_tmax = wet_tmin, wet_tmax
            else:
                generated_tmin, generated_tmax = dry_tmin, dry_tmax

            jday_ary[i] = jday
            tmax_ary[i] = generated_tmax
            tmin_ary[i] = generated_tmin
            rain_ary[i] = generated_rain

        tmax_ary += (xx - np.mean(tmax_ary))
        tmin_ary += (nn - np.mean(tmin_ary))
        rain_ary *= (rr / np.sum(rain_ary))

        tmax_ary, tmin_ary = np.maximum(tmax_ary, tmin_ary), np.minimum(tmax_ary, tmin_ary)
        
        tmax_ary = np.round(tmax_ary, 1)
        tmin_ary = np.round(tmin_ary, 1)

        rain_ary[np.isinf(rain_ary)] = 0
...
def generate(year, repeat, *, monthly_tmin, monthly_tmax, monthly_rain, rand_seed=None):
    ...
    return pd.DataFrame(results, columns=['repeat_index', 'day_of_year', 'generated_tmin', 'generated_tmax', 'generated_rain'])
