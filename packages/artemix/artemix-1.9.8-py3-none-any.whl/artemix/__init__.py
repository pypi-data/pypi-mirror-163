...
def Inicializar(n):
  global DatosList,Orden,Estilo,Documento
  ...
  DatosList = [[sympy.pi,3.14]]
  Orden = 8
  Documento = ""
  return DatosList,Orden,Documento
def search(symbolo):
...
def D(elemento,color = "black"):#Por default se imprime en rojo, para indicar que es un derivado.
  ...
  if Tipo == "Ecuacion":#Si hemos identificado el elemento ingresado como una ecuación, entonces la imprimimos en rojo
    ...
    print(Documento)
    print(Documento + " /n"+str(texto))
    #Documento += " /n" + str(texto)
    Documento = "%s %s"%(Documento,texto)
