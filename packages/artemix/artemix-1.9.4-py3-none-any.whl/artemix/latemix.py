...
from sympy.core import Add, Float, Mod, Mul, Number, Symbol
from sympy.core import S as Sq
...
class LatexPrinter(Printer):
    ...
    def _needs_brackets(self, expr):
        ...
        return not ((expr.is_Integer and expr.is_nonnegative)
                    or (expr.is_Atom and (expr is not Sq.NegativeOne
    ...
    def _print_Mul(self, expr):
        ...
        # : _print_Pow calls this routine with instances of Pow...

        if isinstance(expr, Mul):
            args = expr.args
            if args[0] is Sq.One or any(isinstance(arg, Number) for arg in args[1:]):
                return convert_args(args)
        ...
        if denom is Sq.One and Pow(1, -1, evaluate=False) not in expr.args:
    ...
    def _print_Limit(self, expr):
        ...
        if str(dir) == '+-' or z0 in (Sq.Infinity, Sq.NegativeInfinity):
    ...
    def _print_Order(self, expr):
        ...
        if expr.point and any(p != Sq.Zero for p in expr.point) or \
    ...
    def _print_SeqFormula(self, s):
        ...
        if s.start is Sq.NegativeInfinity:
            stop = s.stop
            printset = (dots, s.coeff(stop - 3), s.coeff(stop - 2),
                        s.coeff(stop - 1), s.coeff(stop))
        elif s.stop is Sq.Infinity or s.length > 4:
            printset = s[:4]
    ...
    def _print_ConditionSet(self, s):
        ...
        if s.base_set is Sq.UniversalSet:
    ...
    def _print_FourierSeries(self, s):
        if s.an.formula is Sq.Zero and s.bn.formula is Sq.Zero:
    ...
    def _print_RootSum(self, expr):
        ...
        if expr.fun is not Sq.IdentityFunction:
    ...
    def _print_Diagram(self, diagram):
        if not diagram.premises:
            ...
            return self._print(Sq.EmptySet)
