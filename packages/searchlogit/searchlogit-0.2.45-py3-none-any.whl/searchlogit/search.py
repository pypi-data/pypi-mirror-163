...
class Search():
        ...
        while itr < max_itr:
            ...
            if itr == max_itr:  # Plot on final iteration
                ...
                if self.multi_objective:  # Plot for MOOF
                    ...
                    plt.savefig(self.code_name + "_" + str(self.num_classes) +
                                "_classes_" + current_time + "_MOOF.png",
                    ...
                    plt.savefig(self.code_name + "_" + str(self.num_classes) +
                                "_classes_" + current_time + "_SOOF.png",
                                bbox_extra_artists=(lgd,), bbox_inches='tight')
    ...
    def run_search(self, HMS=10, HMCR_min=0.6, HMCR_max=0.9, PAR_max=0.85,
        ...
        logger.debug("Search ended at: {}".format(str(time.ctime())))
        ...
        logger.debug("Estimating best solution with entire dataset.")
        ...
        if not self.multi_objective:
            logger.debug("best_BICs: {}".format(best_BICs))
            logger.debug("current_BICs: {}".format(current_BICs))
    ...
    def run_search_latent(self, HMS=10, min_classes=2, max_classes=5,
        ...
        for q in range(min_classes, max_classes):
            self.num_classes = q
            logger.debug("Starting search with {} classes".format(q))
        ...
        logger.debug("Model with best BIC had {} classes".format(q-1))

        old_stdout = sys.stdout
        log_file = open(self.logger_name, "a")
        sys.stdout = log_file
        logger.debug("Best model")
        models[best_model_idx].summary()  # -1 for best, -1 for idx
        sys.stdout = old_stdout
        log_file.close()
