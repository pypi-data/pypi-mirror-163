...
def _get_shp_file(
    shp_file: Union[str, gpd.GeoDataFrame], grid_json: CatGrids
    ...
    gdf = gpd.read_file(shp_file) if isinstance(shp_file, str) else shp_file.copy()
    gdf = gdf.to_crs(grid_json.proj)
...
def _check_for_intersection(
    ...
    if not ds_bbox.intersects(box(*np.asarray(bounds).tolist())):
        is_intersect = False
...
def _get_catalog_time_increment(param: pd.DataFrame) -> Tuple[int, str]:
