...
def __virtual__(hub: pop.hub.Hub) -> Tuple[bool, str]:
...
class _LazyPop:
    __lazy_classes = [pop.hub.Hub, pop.hub.Sub, pop.loader.LoadedMod]
    ...
    def __init__(self, obj, lut=None):
        if isinstance(obj, pop.hub.Hub):
            lut = _LookUpTable()
            lut[self._hub_id] = obj
            lut[self._lazy_hub_id] = self
            lut[obj] = self
        elif isinstance(obj, pop.hub.Sub):
            obj._load_all()
    ...
    def __attr_names(self):
        ...
        if isinstance(self.__obj, pop.hub.Hub):
            attrs += list(self.__obj._subs)
        elif isinstance(self.__obj, pop.hub.Sub):
            attrs += list(self.__obj._loaded)
            attrs += list(self.__obj._subs)
        elif isinstance(self.__obj, pop.loader.LoadedMod):
            attrs += list(self.__obj._attrs)
        else:
            raise Exception(f"Standard objects should not be lazy: {str(self.__obj)}")
    ...
    def __setattr__(self, key, value):
        ...
        if (
            isinstance(value, pop.contract.Contracted)
            and value.hub is not self._lazy_hub()
        ):
    ...
    def _orig_to_attr(self, orig):
        ...
        elif isinstance(orig, pop.contract.Contracted):
...
def strip_hub(f: Callable) -> pop.hub.Hub:
    """
...
def mock_hub(hub: pop.hub.Hub) -> "MockHub":
    return MockHub(hub)
...
class NoContractHub(_LazyPop):
    ...
    def _mock_function(self, f: pop.contract.Contracted) -> pop.contract.Contracted:
        return pop.contract.Contracted(
            hub=self._lazy_hub(),
            contracts=None,
            func=f.func,
            ref=f.ref,
...
def mock_attr_hub(hub: pop.hub.Hub) -> "MockAttrHub":
...
class MockAttrHub(_LazyPop):
    ...
    def _mock_function(self, f) -> pop.contract.Contracted:
        return pop.contract.Contracted(
            hub=self._lazy_hub(),
