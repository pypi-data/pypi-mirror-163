from ssl import (SSLContext, Purpose, CERT_REQUIRED, CERT_NONE, PROTOCOL_TLS_CLIENT, PROTOCOL_TLS_SERVER,
    get_default_verify_paths, cert_time_to_seconds)
import asyncio
import datetime
import os
import sys
...
class BaseSSLContext(Protocol):
    purpose: str = field(init=False, default=None)
    protocol: int = field(init=False, default=None)
    ...
    def context(self) -> Optional[SSLContext]:
        if self.ssl:
            ...
            context = SSLContext(self.protocol)
