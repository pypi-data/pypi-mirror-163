...
class BaseProtocolFactory(ProtocolFactoryProtocol):
    ...
    def __post_init__(self):
        if self.preaction:
            self.preaction = replace(self.preaction, quiet=True)
        if self.action:
            self.action = replace(self.action, quiet=self.quiet)
        if self.requester:
    ...
    async def close(self) -> None:
        ...
        await asyncio.wait([asyncio.create_task(self._scheduler.close()), asyncio.create_task(self.wait_num_connected(0))], timeout=self.timeout)
