...
class BaseMessageObject(MessageObject, Protocol):
    ...
    encoded: bytes
    decoded: Any = None
    context: Dict[str, Any] = field(default_factory=dict, compare=False, repr=False, hash=False)
...
class BaseCodec(Codec):
    ...
    async def decode_buffer(self, encoded: bytes, context: BaseContext = None, source: str = 'buffer',
                            extra_log_context: Dict[str, Any] = None, **kwargs) -> AsyncGenerator[MessageObjectType, None]:
        if context:
        ...
        async for msg in self._from_buffer(encoded, context=complete_context, **kwargs):
            if self.log_msgs:
                self.logger.on_msg_decoded(msg, extra=extra_log_context)
            yield msg
            i += 1
