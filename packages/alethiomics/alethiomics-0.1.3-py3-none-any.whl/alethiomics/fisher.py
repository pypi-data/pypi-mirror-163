...
# modules
def calc_fisher_stats(adata, group_column, group_mutant, group_normal, layer = ''):
    ...
    if layer == '':
        ...
        x = adata[adata.obs[group_column] == group_mutant].layers[layer]
        y = adata[adata.obs[group_column] == group_normal].layers[layer]
    ...
    adata.var['non_zero_mutant' + layer] = (c / x.shape[0] * 100).tolist()
    adata.var['non_zero_normal' + layer] = (d / y.shape[0] * 100).tolist()
    adata.var = concat([adata.var, fish], axis = 1)
...
if __name__ == '__main__':
    ...
    adata = dummy_anndata()
    adata = calc_fisher_stats(adata, 'cell_type', 'Monocyte', 'B')
    print(adata.var)
