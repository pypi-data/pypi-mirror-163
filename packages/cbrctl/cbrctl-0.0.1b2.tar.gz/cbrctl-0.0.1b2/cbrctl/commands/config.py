...
from cbrctl.utilities import run_shell_command, run_shell_command_and_forget, get_carbonara_config_filepath, \
...
def _setup_cluster_eks(namespace, cloud_provider, auth_token):
    ...
    with click.progressbar(EKS_CONFIG_ACTIONS.keys()) as bar:
        random_list = list(range(0, len(CARBONARA_QUOTES) - 1))
        total_percentage = 5
        for action in bar:
            bar.update(total_percentage)
            random_number = random.choice(random_list)
            bar.label = CARBONARA_QUOTES[random_number] + "\n"
            bar.color = True
            total_percentage += EKS_CONFIG_ACTIONS[action]
            if '{{MANIFEST_FILE}}' in action:
                action = action.replace('{{MANIFEST_FILE}}', agent_manifest_file)
