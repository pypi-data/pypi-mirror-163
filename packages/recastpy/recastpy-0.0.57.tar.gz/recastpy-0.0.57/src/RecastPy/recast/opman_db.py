    ...
    with _get_connection() as conn:
        with conn.cursor() as cur:
            sql = (
                ...
                "(deployment_id, model_run_id) "
                "VALUES (%s, %s) "
            )
            cur.execute(sql, (deployment_id, model_run_id))
            conn.commit()
    ...
    with _get_connection() as conn:
        with conn.cursor() as cur:
            sql = (
                ...
                "(deployment_task_id, deployment_id, job_id, task_type) "
                "VALUES (%s, %s, %s, %s) "
            )
            cur.execute(sql, (deployment_task_id, deployment_id, job_id, task_type))
