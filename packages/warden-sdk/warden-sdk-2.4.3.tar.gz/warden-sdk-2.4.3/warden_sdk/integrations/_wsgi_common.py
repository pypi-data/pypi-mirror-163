...
from warden_sdk.utils import AnnotatedValue
from warden_sdk._compat import text_type, iteritems
...
def request_body_within_bounds(client, content_length):
  # type: (Optional[warden_sdk.Client], int) -> bool
  if client is None:
    return False

  bodies = client.options["request_bodies"]
  return not (bodies == "never" or
              (bodies == "small" and content_length > 10**3) or
              (bodies == "medium" and content_length > 10**4))
...
def _filter_headers(headers):
  ...
  return {
      k: (v if k.upper().replace(
          "-", "_") not in SENSITIVE_HEADERS else AnnotatedValue(
