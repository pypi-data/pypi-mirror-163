...
Code reference:
- [warden_sdk](https://github.com/getwarden/warden-python/blob/master/warden_sdk/serializer.py)
"""
import sys
import math
...
ReprProcessor = Callable[[Any, Dict[str, Any]], Union[Any, str]]
...
def serialize(event, smart_transaction_trimming=False, **kwargs):
  ...
  def _should_repr_strings():
    ...
    """
        ...
        Generally we just try to make the Warden UI present exactly
  ...
  def _serialize_node_impl(obj, is_databag, should_repr_strings,
    ...
    warden_repr = getattr(type(obj), "__warden_repr__", None)
    ...
    elif callable(warden_repr):
      return warden_repr(obj)
    ...
    if (smart_transaction_trimming and len(path) == 3 and path[0] == "spans"
  ...
  try:
    ...
    if smart_transaction_trimming and sum_span_description_bytes > 0:
      ...
      # as event["_meta"].
      descriptions_budget_bytes = span_count * MAX_STRING_LENGTH
