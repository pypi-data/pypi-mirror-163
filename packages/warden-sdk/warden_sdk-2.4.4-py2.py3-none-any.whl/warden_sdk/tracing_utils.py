...
warden_TRACE_REGEX = re.compile("^[ \t]*"    # whitespace
                                "([0-9a-f]{32})?"    # trace_id
                                "-?([0-9a-f]{16})?"    # span_id
                                "-?([01])?"    # sampled
                                "[ \t]*$"    # whitespace
...
# of the form `warden=xxxx`
warden_TRACESTATE_ENTRY_REGEX = re.compile(
    # either warden is the first entry or there's stuff immediately before it,
    # ending in a comma (this prevents matching something like `coolwarden=xxx`)
    "(?:^|.+,)"
    # warden's part, not including the potential comma
    "(warden=[^,]*)"
...
def is_valid_sample_rate(rate):
  ...
  if not isinstance(rate, Real) or math.isnan(rate):
    logger.warning(
        "[Tracing] Given sample rate is invalid. Sample rate must be a boolean or a number between 0 and 1. Got {rate} of type {type}."
        .format(rate=rate, type=type(rate)))
    return False
...
def maybe_create_breadcrumbs_from_span(hub, span):
  # type: (warden_sdk.Hub, Span) -> None
  if span.op == "redis":
    hub.add_breadcrumb(message=span.description,
                       type="redis",
                       category="redis",
                       data=span._tags)
  elif span.op == "http":
...
def extract_wardentrace_data(header):
  # type: (Optional[str]) -> Optional[typing.Mapping[str, Union[str, bool, None]]]
  """
    Given a `warden-trace` header string, return a dictionary of data.
    """
  if not header:
    return None
  ...
  match = warden_TRACE_REGEX.match(header)
  if not match:
    return None
...
def extract_tracestate_data(header):
  ...
  """
    Extracts the warden tracestate value and any third-party data from the given
    tracestate header, returning a dictionary of data.
    """
  warden_entry = third_party_entry = None
  before = after = ""

  if header:
    # find warden's entry, if any
    warden_match = warden_TRACESTATE_ENTRY_REGEX.search(header)

    if warden_match:
      warden_entry = warden_match.group(1)

      # remove the commas after the split so we don't end up with
      # `xxx=yyy,,zzz=qqq` (double commas) when we put them back together
      before, after = map(lambda s: s.strip(","), header.split(warden_entry))

      # extract warden's value from its entry and test to make sure it's
      # valid; if it isn't, discard the entire entry so that a new one
      # will be created
      warden_value = warden_entry.replace("warden=", "")
      if not re.search("^{b64}$".format(b64=base64_stripped), warden_value):
        warden_entry = None
    else:
      after = header

    # if either part is invalid or empty, remove it before gluing them together
    third_party_entry = (",".join(
        filter(TRACESTATE_ENTRIES_REGEX.search, [before, after])) or None)

  return {
      "warden_tracestate": warden_entry,
...
def compute_tracestate_value(data):
  ...
  """
    ...
    Note: Returns just the base64-encoded data, NOT the full `warden=...`
    tracestate entry.
    """
  ...
  # vendor name and value (`warden=xxx,dogsaregreat=yyy`), to avoid confusion
  # we strip the `=`
...
def compute_tracestate_entry(span):
  ...
  """
    Computes a new warden tracestate for the span. Includes the `warden=`.

    Will return `None` if there's no client.
    """
  data = {}

  hub = span.hub or warden_sdk.Hub.current
  ...
  if client:
    ...
    return "warden=" + compute_tracestate_value(data)
...
def reinflate_tracestate(encoded_tracestate):
  ...
  """
    Given a warden tracestate value in its encoded form, translate it back into
  ...
  if encoded_tracestate:
    ...
    # signs between vendor name and value (`warden=xxx,dogsaregreat=yyy`),
    ...
    try:
      ...
      logger.warning(
          ("Unable to attach tracestate data to envelope header: {err}" +
           "\nTracestate value is {encoded_tracestate}").format(
               err=err, encoded_tracestate=encoded_tracestate),)
...
def has_tracestate_enabled(span=None):
  ...
  client = ((span and span.hub) or warden_sdk.Hub.current).client
...
def has_custom_measurements_enabled():
  # type: () -> bool
  client = warden_sdk.Hub.current.client
  options = client and client.options
  return bool(options and options["_experiments"].get("custom_measurements"))


class Baggage(object):
  __slots__ = ("warden_items", "third_party_items", "mutable")

  WARDEN_PREFIX = "warden-"
  WARDEN_PREFIX_REGEX = re.compile("^warden-")
  ...
  def __init__(
      self,
      warden_items,    # type: Dict[str, str]
      third_party_items="",    # type: str
      mutable=True,    # type: bool
  ):
    self.warden_items = warden_items
  ...
  def from_incoming_header(cls, header):
    """
        freeze if incoming header already has warden baggage
        """
    warden_items = {}
    third_party_items = ""
    mutable = True
    ...
    if header:
      for item in header.split(","):
        ...
        item = item.strip()
        key, val = item.split("=")
        if Baggage.WARDEN_PREFIX_REGEX.match(key):
          baggage_key = unquote(key.split("-")[1])
          warden_items[baggage_key] = unquote(val)
          mutable = False
        else:
          third_party_items += ("," if third_party_items else "") + item

    return Baggage(warden_items, third_party_items, mutable)
  ...
  def dynamic_sampling_context(self):
    ...
    for key in Baggage.DSC_KEYS:
      item = self.warden_items.get(key)
      if item:
        header[key] = item
  ...
  def serialize(self, include_third_party=False):
    ...
    for key, val in iteritems(self.warden_items):
      item = Baggage.WARDEN_PREFIX + quote(key) + "=" + quote(val)
      items.append(item)
