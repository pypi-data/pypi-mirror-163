...
from warden_sdk.profiler import has_profiling_enabled
...
class Span(object):
  ...
  def __init__(
      ...
      hub=None,    # type: Optional[warden_sdk.Hub]
  ...
  def __enter__(self):
    # type: () -> Span
    hub = self.hub or warden_sdk.Hub.current
  ...
  def start_child(self, **kwargs):
    ...
    child = Span(trace_id=self.trace_id,
                 parent_span_id=self.span_id,
                 containing_transaction=self.containing_transaction,
                 **kwargs)

    span_recorder = (self.containing_transaction
                     and self.containing_transaction._span_recorder)
    if span_recorder:
  ...
  def continue_from_environ(
    ...
    """
        ...
        the 'warden-trace', 'baggage' and 'tracestate' headers from the environ (if any)
  ...
  def continue_from_headers(
    ...
    """
        ...
        the 'warden-trace', 'baggage' and 'tracestate' headers).
        """
    ...
    wardentrace_kwargs = extract_wardentrace_data(headers.get("warden-trace"))

    if wardentrace_kwargs is not None:
      kwargs.update(wardentrace_kwargs)
      baggage.freeze()
  ...
  def iter_headers(self):
    ...
    """
        Creates a generator which returns the span's `warden-trace`, `baggage` and
        `tracestate` headers.

        If the span's containing transaction doesn't yet have a
        `warden_tracestate` value, this will cause one to be generated and
        stored.
        """
    yield "warden-trace", self.to_traceparent()
  ...
  def from_traceparent(
    ...
    """
        ...
        Create a Transaction with the given params, then add in data pulled from
        the given 'warden-trace' header value before returning the Transaction.
    ...
    return cls.continue_from_headers({"warden-trace": traceparent}, **kwargs)
  ...
  def to_tracestate(self):
    ...
    """
        ...
        If the containing transaction doesn't yet have a `warden_tracestate`
    ...
    warden_tracestate = self.get_or_set_warden_tracestate()
    third_party_tracestate = (
        self.containing_transaction._third_party_tracestate
        if self.containing_transaction else None)

    if not warden_tracestate:
      return None

    header_value = warden_tracestate
  ...
  def get_or_set_warden_tracestate(self):
    # type: (Span) -> Optional[str]
    """
        Read warden tracestate off of the span's containing transaction.

        If the transaction doesn't yet have a `_warden_tracestate` value,
        compute one and store it.
        """
    transaction = self.containing_transaction

    if transaction:
      if not transaction._warden_tracestate:
        transaction._warden_tracestate = compute_tracestate_entry(self)

      return transaction._warden_tracestate
  ...
  def finish(self, hub=None):
    # type: (Optional[warden_sdk.Hub]) -> Optional[str]
    # XXX: would be type: (Optional[warden_sdk.Hub]) -> None, but that leads
    # to incompatible return types for Span.finish and Transaction.finish.
    if self.timestamp is not None:
      # This span is already finished, ignore.
      return None

    hub = hub or self.hub or warden_sdk.Hub.current
  ...
  def get_trace_context(self):
    ...
    if warden_tracestate:
      rv["tracestate"] = warden_tracestate

    # TODO-neel populate fresh if head SDK
    if self.containing_transaction and self.containing_transaction._baggage:
      rv["dynamic_sampling_context"] = self.containing_transaction._baggage.dynamic_sampling_context(
      )
...
class Transaction(Span):
  __slots__ = (
      ...
      "source",
      "parent_sampled",
    # the warden portion of the `tracestate` header used to transmit
    # correlation context for server-side dynamic sampling, of the form
    # `warden=xxxxx`, where `xxxxx` is the base64-encoded json of the
  ...
  def __init__(
      ...
      warden_tracestate=None,    # type: Optional[str]
    ...
    self._warden_tracestate = warden_tracestate
  ...
  def finish(self, hub=None):
    ...
    if (has_profiling_enabled(hub) and hub.client is not None
      ...
      event["profile"] = {
          ...
          "version_code":
              "",    # TODO: Determine appropriate value. Currently set to empty string so profile will not get rejected.
