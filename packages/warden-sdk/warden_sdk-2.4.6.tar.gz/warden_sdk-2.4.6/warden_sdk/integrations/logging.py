...
_IGNORED_LOGGERS = set(
    ["warden_sdk.errors", "urllib3.connectionpool", "urllib3.connection"])
...
class LoggingIntegration(Integration):
  ...
  def _handle_record(self, record):
    ...
    if (self._breadcrumb_handler is not None
  ...
  def setup_once():
    ...
    def warden_patched_callhandlers(self, record):
      try:
...
def _extra_from_record(record):
  return {
      ...
      if k not in COMMON_RECORD_ATTRS and (
          not isinstance(k, str) or not k.startswith("_"))
  }
...
class EventHandler(logging.Handler, object):
  ...
  def _emit(self, record):
    ...
    # https://github.com/getwarden/warden-python/issues/904
...
class BreadcrumbHandler(logging.Handler, object):
  ...
  def _emit(self, record):
    ...
    Hub.current.add_breadcrumb(_breadcrumb_from_record(record),
                               hint={"log_record": record})
