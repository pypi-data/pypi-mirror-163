...
from warden_sdk.tracing import EnvironHeaders
from warden_sdk.utils import capture_internal_exceptions, safe_repr
...
def _install_httplib():
  ...
  def putrequest(self, method, url, *args, **kwargs):
    ...
    self._wardensdk_span = span

    return rv

  def getresponse(self, *args, **kwargs):
    # type: (HTTPConnection, *Any, **Any) -> Any
    span = getattr(self, "_wardensdk_span", None)
...
def _install_subprocess():
  ...
  def warden_patched_popen_init(self, *a, **kw):
    ...
    with hub.start_span(op="subprocess", description=description) as span:
      ...
      for k, v in hub.iter_trace_propagation_headers(span):
        if env is None:
          env = _init_argument(a, kw, "env", 10,
                               lambda x: dict(x or os.environ))
  ...
  subprocess.Popen.__init__ = warden_patched_popen_init    # type: ignore

  old_popen_wait = subprocess.Popen.wait

  def warden_patched_popen_wait(self, *a, **kw):
  ...
  subprocess.Popen.wait = warden_patched_popen_wait    # type: ignore

  old_popen_communicate = subprocess.Popen.communicate

  def warden_patched_popen_communicate(self, *a, **kw):
    # type: (subprocess.Popen[Any], *Any, **Any) -> Any
    hub = Hub.current
  ...
  subprocess.Popen.communicate = warden_patched_popen_communicate    # type: ignore
