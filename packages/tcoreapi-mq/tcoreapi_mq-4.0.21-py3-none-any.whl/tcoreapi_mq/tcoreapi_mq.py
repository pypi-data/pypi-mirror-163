...
class TradeClass():
    ...
    def trade_connect(self,appid,ServiceKey,port):
        ...
        self.tsocket.setsockopt(zmq.RCVTIMEO, 2000)
        ...
        try:
            ...
            if data["Success"] == "OK":
                ...
                data=json.loads("{\"Success\":\"Connected\"}")
        except:
            self.tsocket.close()
            self.lock.release()
...
class QuoteClass():
    ...
    def quote_connect(self,appid,ServiceKey, port):
        ...
        self.qsocket.setsockopt(zmq.RCVTIMEO, 2000)
...
class TCoreZMQ(TradeClass,QuoteClass):
    def __init__(self,APPID="ZMQ",SKey="8076c9867a372d2a9a814ae710c256e2",quote_port="51864",trade_port="51834",logpath=""):
        ...
        for i in range(1000):
            trade_port=str(int(trade_port)+i)
            result =self.trade_connect(APPID,SKey,trade_port)
            ret = result["Success"]
            if ret != "ConnectError":
                break
        for i in range(1000):
            quote_port=str(int(quote_port)+i)
            result =self.quote_connect(APPID,SKey,quote_port)
            ret = result["Success"]
            if ret != "ConnectError":
                break
