...
PROGRAM_NAME = 'GeotabAvlTranslator'
PROGRAM_VERSION = '1.0'


DEFAULT_GEOTAB_AVL_FEED_INFO_ID = '49253be7-0c6a-4a65-8113-450f9041f989'


def main():
    input_file, output_file = parse_cmd_arguments()

    obj = json.loads(open(input_file, 'r').read())
    wzdx = wzdx_creator(obj)
    try:
...
def parse_cmd_arguments():
    ...
    parser.add_argument('geotabAvlFile', help='geotab file path')
    parser.add_argument('--outputFile', required=False,
                        default='geotab_wzdx_translated_output_message.geojson', help='output file path')

    args = parser.parse_args()
    return args.geotabAvlFile, args.outputFile
...
def wzdx_creator(message, info=None):
    ...
    if not info:
        info = wzdx_translator.initialize_info(
            DEFAULT_GEOTAB_AVL_FEED_INFO_ID)
