...
from revup import github_utils
from revup.github_utils import PrInfo, PrUpdate, PrComment
...
class Review:  # pylint: disable=too-many-instance-attributes
    ...
    # Name for the remote base ref. One of a remote base branch / relative branch / another topic's remote_head
    remote_base: str = ""
...
class TopicStack:
    ...
    # Uploader name used to format remote branch names
    uploader: str
    ...
    def parse_commit_tags(self, commit_msg: str) -> Tuple[Dict[str, Set[str]], str]:
        ...
        for ln in commit_msg.split("\n"):
            ...
            if tag in VALID_TAGS:
                if tag == TAG_BRANCH or tag == TAG_RELATIVE_BRANCH:
    ...
    async def populate_topics(  # pylint: disable=too-many-branches, too-many-statements, too-many-locals
        self,
        force_relative_chain: bool,
        auto_topic: bool,
        ...
        """
        Parse all commits and sort them into individual topics and reviews.
        Verify base branch and relative topic info to ensure it is valid.
        """
        if self.base_branch:
        ...
        for name, topic in list(self.topics.items()):
            ...
            if force_relative_chain and seen_topics:
                ...
                if relative_topic not in seen_topics:
                    if relative_topic in self.topics:
                        ...
                        logging.warning(
                            f"Relative topic '{relative_topic}' not found in stack, assuming it was merged"
                        )
                        relative_topic = ""
            ...
            topic_uploader = (
                min(topic.tags[TAG_UPLOADER]) if topic.tags[TAG_UPLOADER] else self.uploader
            )
            ...
            for branch in topic.tags[TAG_BRANCH]:
                ...
                if topic.relative_topic is not None:
                    ...
                    # Base ref is empty since it doesn't exist yet. It will get populated in create_commits()
                else:
            ...
            if auto_add_users == "r2a" or auto_add_users == "both":
                topic.tags[TAG_ASSIGNEE].update(topic.tags[TAG_REVIEWER])
            if auto_add_users == "a2r" or auto_add_users == "both":
    ...
    async def create_commits(self, no_cherry_pick: bool, trim_tags: bool) -> None:
        """
        ...
        for name, topic, base_branch, review in self.all_reviews_iter():
            ...
            for commit in topic.original_commits:
                if commit.parents[0] == next_parent and not trim_tags:
                    ...
                    if no_cherry_pick:
                        raise RevupUsageException(
                            f"Would require cherry picking {commit.commit_id} to"
                            f" {next_parent} since its actual parent is {commit.parents[0]}"
                        )
    ...
    async def push_git_refs(self, create_local_branches: bool) -> None:
        """
        ...
        if self.last_virtual_diff_target is not None:
            virtual_diff_branch = f"{self.uploader}/revup/virtual_diff_targets"
        ...
        for topic in self.topics.values():
            ...
            for branch, review in topic.reviews.items():
                if review.status == PrStatus.NEW:
                    if not review.base_ref:
                        raise RuntimeError(
                            f"Bug! review {review.topic.name} {review.remote_base} doesn't have a base ref"
                        )
                ...
                if base_branch_name in self.labels_to_ids:
                    # Add the base branch as a tag so it can be used to see pending changes on that branch.
    ...
    def create_review_graph(self) -> Dict[str, List[str]]:
        ...
        def graph_helper(review: Review, back: str, prefix: str) -> int:
            ...
            for i in range(len(review.children)):
                ret[review.children[i].remote_head] = ret[review.remote_head]
    ...
    def num_reviews_changed(self) -> int:
        ...
        for _, topic in reversed(self.topics.items()):
            for _, review in topic.reviews.items():
                if (
                    review.status == PrStatus.NOCHANGE or review.status == PrStatus.MERGED
