...
RE_RAW_DIFF_TREE_LINE = re.compile(
    r"^:(?P<old_mode>[0-9]+) (?P<new_mode>[0-9]+) (?P<old_hash>[0-9a-f]+) (?P<new_hash>[0-9a-f]+) (?P<type>[a-zA-Z]+)\t(?P<path>.*)$",
    re.MULTILINE,
)
...
async def make_git(
    ...
    git_ctx.author = author if author else git_ctx.email.split("@")[0]
...
class Git(object):
    ...
    def __init__(
        ...
        self.temp_dir = tempfile.TemporaryDirectory(prefix="revup_")
        if self.keep_temp:
    ...
    async def verify_branch_or_commit(self, obj: str) -> None:
        if await self.git_return_code("rev-parse", "--verify", "--quiet", obj):
    ...
    async def get_patch_id(
        ...
        """
        Return the patch-id that uniquely identifies this commit's diff (but not its other metadata).
        """
        patch_source = (
            [
                self.git_path,
    ...
    async def merge(
        self, commit: GitCommitHash, commit_msg: str, skip: bool = False, strategy: str = ""
    ) -> List[MergeResult]:
        merge_args = [
            "-c",
            "merge.verbosity=0",
        ...
        if strategy:
            merge_args.append("-X" + strategy)

        merge_args.append(commit)
        await self.git(*merge_args, raiseonerror=False)

        ret = []

        await self.git("commit", "--allow-empty", "-n", "-a", "-m", commit_msg)
        return ret
