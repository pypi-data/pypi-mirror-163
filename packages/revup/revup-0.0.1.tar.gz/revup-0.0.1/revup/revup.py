# aclint: py3
"""
Tool for optimizing git and github workflows.
"""
...
from revup import config
from revup import logs
...
class HelpAction(argparse.Action):
    """
    A custom help action that displays a manpage formatted from the markdown documentation if available.
    """
...
def make_toplevel_parser() -> RevupArgParser:
    ...
    revup_parser.add_argument("--author")
    ...
    revup_parser.add_argument("--pre-upload", "-p")
...
async def main() -> int:  # pylint: disable=too-many-statements, too-many-locals
    ...
    for p in [upload_parser, restack_parser]:
        ...
        p.add_argument("--relative-chain", "-c", action="store_true")
        p.add_argument("--auto-topic", "-a", action="store_true")
    ...
    upload_parser.add_argument("--cherry-pick", action="store_true", default=True)
    ...
    amend_parser.add_argument("--help", "-h", action=HelpAction, nargs=0)
    amend_parser.add_argument("commit", nargs="?")
    amend_parser.add_argument("--skip-reword", "-s", action="store_true")
