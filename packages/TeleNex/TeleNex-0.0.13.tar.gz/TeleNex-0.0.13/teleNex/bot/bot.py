...
from ..types import ReplyBase, InlineKeyboardMarkup
...
class Bot(BaseBot):
    ...
    async def edit_msg_text(
        ...
        data = generate_payload(locals().copy())
        response = await self._api.make_request('editMessageText', data)
        return self._process_response(response, Message)
    ...
    def remove_question(self, chat_id: str) -> bool:
        if self.question_queues.get(chat_id) is not None:
            self.question_queues[chat_id].put_nowait(None)
        return self.question_queues.pop(chat_id, None) is not None
    ...
    def on_callback(self, cb_data: str = None, cond: Callable[[CallbackQuery], bool] = None):
        def decorator(d_func: Callable[[CallbackQuery], Any]):
            if cb_data is not None:
                self._cb_datas[cb_data] = d_func

            if cond is not None:
                self._cb_funcs.append((cond, d_func))
