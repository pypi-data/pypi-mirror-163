...
class Storage:
    ...
    def load(self, content_hash: Union[FileContentHash, str], organization_id: OrganizationId
        ...
        self.metrics.count_storage_access(blob.storage_class, "load", blob.content_type,
                                          organization_id=organization_id, content_hash=content_hash.__str__())
    ...
    def store(self, blob: bytes, content_type: Union[FileType, str], organization_id: OrganizationId
        ...
        if not bucket_blob.exists():
            ...
            self._update_blob_storage_class(bucket_blob)
        self.metrics.count_storage_access(storage_class, "store", content_type.value,
                                          organization_id=organization_id, content_hash=content_hash.__str__())
        return content_hash
    ...
    def update_metadata(self, organization_id: OrganizationId, content_hash: Union[FileContentHash, str],
        ...
        if not blob.metadata == metadata:
            ...
            try:
                ...
                logging.error(
                    f"GoogleCloudError in patching {content_hash.get_value()} for organization {organization_id}: " + e)
                raise e
        self.metrics.count_storage_access(storage_class, "update", blob.content_type,
                                          organizationId=organization_id, contentHash=content_hash.__str__())
        metadata = dict()
