...
for job in pageviews_rotate_jobs:
    ...
    GB("StreamReader", desc=json.dumps(desc_json)). \
        foreach(lambda records: tracker_log(f'-start- {len(records)}->{records}', job.get('name'))). \
        aggregate([],
                  lambda a, r: a + [r['value']],
                  lambda a, r: a + r). \
        map(lambda records: transform(records, job.get('number_of_rotated_keys'))). \
        foreach(lambda records: tracker_log(f'-map- {records}', job.get('name'))). \
...
#     }
#     GB("StreamReader", desc=json.dumps(desc_json)). \
#         foreach(lambda records: tracker_log(f'-start-{records}', job.get('name'))). \
#         filter(lambda record: extract(record, dimension=job.get('dimension'))). \
#         foreach(lambda records: tracker_log(f'-filter-{records}', job.get('name'))). \
#         aggregate([],
#                   lambda a, r: a + [r['value']],
#                   lambda a, r: a + r). \
#         map(lambda records: transform(records, job.get('number_of_rotated_keys'))). \
