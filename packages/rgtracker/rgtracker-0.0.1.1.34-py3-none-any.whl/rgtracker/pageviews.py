...
def load(job_name, records, dimension, write_to_ts, timeseries_name, key_expire_duration_sc, reinject_stream_name,
    ...
    for cms_merge in records.get("merge"):
        if execute('EXISTS', cms_merge.get('name')) != 1:
            ...
            tracker_log(f'Merge Pageviews {cms_merge.get("name")}', job_name)
            ...
            if write_to_ts:
                for id in records.get('id'):
                    ...
                    if dimension == Dimension.WEBSITE.value:
                        ...
                        # tracker_log(f'Get {dimension} infos - {record_infos}', job_name)
                        ...
                        tracker_log(f'Write to Timeseries {id}->{timeseries_key_name}', job_name)
            ...
            for id in records.get('id'):
                ...
                tracker_log(f'Write to OutputStream {id}->{output_stream_name}', job_name)
