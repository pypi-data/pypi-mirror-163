...
class Page:
    ...
    def __post_init__(self):
        ...
        if len(self.last_visited.split('_')) > 1:
            self.dimension_ts_key = f'{Type.JSON.value}::{Dimension.PAGE.value}:{self.id}:{self.last_visited}:'
            self.metric_pageviews_key = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{self.last_visited}:{Metric.PAGEVIEWS.value}'
            self.metric_devices_key = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{self.last_visited}:{Metric.DEVICES.value}'
            ...
            self.dimension_ts_key = f'{Type.JSON.value}::{Dimension.PAGE.value}:{self.id}:{rounded_last_visited}:'
            self.metric_pageviews_key = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{rounded_last_visited}:{Metric.PAGEVIEWS.value}'
            self.metric_devices_key = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{rounded_last_visited}:{Metric.DEVICES.value}'
    ...
    def create(self, website, section):
        ...
        execute('JSON.SET', self.dimension_key, '.', json.dumps({
            ...
            'section': {
                'id': section.id,
                'pretty_name': section.pretty_name,
    ...
    def write_metadata(self, stream_name, previous_keys=None):
        if previous_keys is None:
            execute('XADD', stream_name, '*', 'dimension', Dimension.PAGE.value, 'key',
                    self.dimension_ts_key)
            # log(f'XADD {stream_name} * dimension {Dimension.PAGE.value} key {self.dimension_ts_key}')
        else:
            execute('XADD', stream_name, '*', 'dimension', Dimension.PAGE.value, 'key',
    ...
    def write_metrics(self, timeseries_name, merged_key):
        pg_key = create_key_name(type=Type.TIMESERIES.value, name=timeseries_name, dimension=Dimension.PAGE.value,
                                 record_id=self.id, metric=Metric.PAGEVIEWS.value)
        d_key = create_key_name(type=Type.TIMESERIES.value, name=timeseries_name, dimension=Dimension.PAGE.value,
                                record_id=self.id, metric=Metric.DEVICES.value)
        u_d_key = create_key_name(type=Type.TIMESERIES.value, name=timeseries_name, dimension=Dimension.PAGE.value,
        ...
        page_info = execute('FT.SEARCH', 'sections', f'@id:{{{self.id}}}', 'RETURN', '5', 'website_id', 'website_name',
                            'section_id', 'section_pretty_name', 'article_id')[-1][-1]

        execute('TS.ADD', pg_key, ts, pageviews, 'ON_DUPLICATE', 'LAST', 'LABELS', 'dimension',
                Dimension.PAGE.value, Dimension.METRIC, Metric.PAGEVIEWS.value, 'page_id', self.id, *page_info)
        # log(f'TS.ADD {pg_key} {ts} {pageviews} ON_DUPLICATE LAST LABELS dimension {Dimension.PAGE.value} {Dimension.METRIC.value} {Metric.PAGEVIEWS.value} page_id {self.id} {page_info}')

        execute('TS.ADD', d_key, ts, devices, 'ON_DUPLICATE', 'LAST', 'LABELS', 'dimension', Dimension.PAGE.value,
                Dimension.METRIC, Metric.DEVICES.value, 'page_id', self.id, *page_info)
        # log(f'TS.ADD {d_key} {ts} {devices} ON_DUPLICATE LAST LABELS dimension {Dimension.PAGE.value} {Dimension.METRIC.value} {Metric.DEVICES.value} page_id {self.id} {page_info}')

        execute('TS.ADD', u_d_key, ts, unique_devices, 'ON_DUPLICATE', 'LAST', 'LABELS', 'dimension',
                Dimension.PAGE.value, Dimension.METRIC, Metric.DEVICES.value, 'page_id', self.id, *page_info)
        # log(f'TS.ADD {u_d_key} {ts} {unique_devices} ON_DUPLICATE LAST LABELS dimension {Dimension.PAGE.value} {Dimension.METRIC.value} {Metric.UNIQUE_DEVICES.value} page_id {self.id} {page_info}')
    ...
    def get_previous_key(self, previous_ts):
        x = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{previous_ts}:{Metric.PAGEVIEWS.value}'
    ...
    def create_previous_keys(self, minutes=True, hours=False):
        ...
        for index in range(0, 6):
            ...
            prev_pg_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{prev_ts}:{Metric.PAGEVIEWS.value}')
            prev_devices_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{prev_ts}:{Metric.DEVICES.value}')
            prev_unique_devices_keys.append(
                f'{Type.HLL.value}::{Dimension.PAGE.value}:{self.id}:{prev_ts}:{Metric.UNIQUE_DEVICES.value}')

        first_ts = prev_pg_keys[0].split(':')[RedisNC.TS]
        last_ts = prev_pg_keys[-1].split(':')[RedisNC.TS]
        merge_pg_key_name = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{first_ts}_{last_ts}:{Metric.PAGEVIEWS.value}'
        merge_devices_key_name = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{first_ts}_{last_ts}:{Metric.DEVICES.value}'
        merge_unique_devices_key_name = f'{Type.HLL.value}::{Dimension.PAGE.value}:{self.id}:{first_ts}_{last_ts}:{Metric.UNIQUE_DEVICES.value}'

        x = {
            'key_names': {
                'previous_key': f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{first_ts}_{last_ts}:',
    ...
    def create_previous_keys_concat(self, range, minutes=True, hours=False):
        ...
        for index in range:
            ...
            prev_pg_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{prev_f_ts}_{prev_l_ts}:{Metric.PAGEVIEWS.value}')
            prev_devices_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{prev_f_ts}_{prev_l_ts}:{Metric.DEVICES.value}')
            prev_unique_devices_keys.append(
                f'{Type.HLL.value}::{Dimension.PAGE.value}:{self.id}:{prev_f_ts}_{prev_l_ts}:{Metric.UNIQUE_DEVICES.value}')

        last_merge_ts = prev_pg_keys[-1].split(":")[RedisNC.TS].split("_")[-1]
        merge_pg_key_name = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{last_visited[0]}_{last_merge_ts}:{Metric.PAGEVIEWS.value}'
        merge_devices_key_name = f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{last_visited[0]}_{last_merge_ts}:{Metric.DEVICES.value}'
        merge_unique_devices_key_name = f'{Type.HLL.value}::{Dimension.PAGE.value}:{self.id}:{last_visited[0]}_{last_merge_ts}:{Metric.UNIQUE_DEVICES.value}'

        x = {
            'key_names': {
                'current_key': f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{last_visited[0]}_{last_visited[-1]}:',
                'previous_key': f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{last_visited[0]}_{last_merge_ts}:',
                'previous_pageviews_keys': merge_pg_key_name,
                'previous_devices_keys': merge_devices_key_name,
                'previous_unique_devices_keys': merge_unique_devices_key_name,
            },
    ...
    def delete_previous_key_concat(self, previous_keys, delta, range, minutes=True, hours=False):
        ...
        for index in range:
            ...
            del_pg_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{first}_{last}:{Metric.PAGEVIEWS.value}')
            del_devices_keys.append(
                f'{Type.CMS.value}::{Dimension.PAGE.value}:{self.id}:{first}_{last}:{Metric.DEVICES.value}')
            del_unique_devices_keys.append(
                f'{Type.HLL.value}::{Dimension.PAGE.value}:{self.id}:{first}_{last}:{Metric.UNIQUE_DEVICES.value}')
