...
class Page:
    ...
    def write_metadata(self, stream_names):
        ...
        execute('XADD', stream_names.get('pageviews'), '*', 'dimension', Dimension.PAGEVIEWS.value, 'id', self.id,
                'ts',
                parsed_key.get('ts'))
        execute('XADD', stream_names.get('unique_devices'), '*', 'dimension', Dimension.PAGEVIEWS.value, 'id',
                self.id, 'ts',
                parsed_key.get('ts'))
