...
def load(records, dimension, write_to_ts, timeseries_name, key_expire_duration_sc, reinject_stream_name,
    ...
    for hll_reinject in records.get('reinject'):
        execute('XADD', reinject_stream_name, '*', 'id', hll_reinject.get('id'), 'dimension',
                hll_reinject.get('dimension'), 'ts', hll_reinject.get('ts'))
        ...
        if execute('EXISTS', hll_merge.get('name')) != 1:
            ...
            if write_to_ts:
                ...
                if dimension == Dimension.WEBSITE.value:
                    record_infos = \
                    execute('FT.SEARCH', index_name, f'@id:{{{hll_merge.get("id")}}}', 'RETURN', '1', 'name')[-1]
            ...
            execute('XADD', output_stream_name, '*', 'dimension', dimension, 'id', hll_merge.get('id'), 'ts',
