...
class Section:
    ...
    def write_metadata(self, stream_names):
        ...
        execute('XADD', stream_names.get('pageviews'), '*', 'dimension', Dimension.SECTION.value, 'id', self.id,
                'ts',
                parsed_key.get('ts'))
        tracker_log(f'XADD {stream_names.get("pageviews")} * dimension {Dimension.SECTION.value} id {self.id} ts {parsed_key.get("ts")}')
        execute('XADD', stream_names.get('unique_devices'), '*', 'dimension', Dimension.SECTION.value, 'id',
                self.id, 'ts',
                parsed_key.get('ts'))


def load_section(website, section, device, output_stream_names):
