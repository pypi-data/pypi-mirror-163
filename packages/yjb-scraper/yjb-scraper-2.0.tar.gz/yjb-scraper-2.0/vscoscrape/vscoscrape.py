...
class Scraper(object):
    ...
    def newSiteId(self):
        ...
        if cache is None or self.username not in cache:
            ...
            self.sitecollectionid = res.json(
            )["sites"][0]["site_collection_id"]
    ...
    def makeProfileList(self):
        ...
        url = self.session.get(
            self.profileurl, headers=constants.media).json()["site"]
    ...
    def makeListJournal(self, num, loc):
        ...
        for item in self.jour_found[loc]["body"]:
            ...
            if item["type"] == "image":
                if os.path.exists(
                    os.path.join(
                        os.getcwd(), self.jour_found[loc]["permalink"])
                ):
    ...
    def getImageList(self):
        ...
        :return: none
        """
        self.pbar = tqdm(desc="Finding new posts from %s" %
        ...
        while count > 0:
            for url in z:
                ...
                if (
                    ...
                    continue
                if round(time.time()) - int(str(url["upload_date"])[:-3]) > 2419200:
                    break
                if url["is_video"] is True:
                    self.imagelist.append(
                        [
