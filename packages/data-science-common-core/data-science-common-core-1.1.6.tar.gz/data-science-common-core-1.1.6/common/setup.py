...
def check_parameter_consistency(project_parameters, project_parameters_debug):
    """Check constants and constants_debug has same parameters for consistency."""
    len_diff = len(
        set(project_parameters.keys()) - set(project_parameters_debug.keys())
    )
    return len_diff == 0
...
def setup_params(args=None):
    ...
    logging.info(f"Session id is: {params['session_id']}")
    ...
    if params["train_mode"]:
        ...
        try:
            ...
            logging.warning("Model id is not found in DWH")
        ...
        if os.path.exists(secret_path):
            ...
            logging.warning(f"File {secret_path} does not exist...")
