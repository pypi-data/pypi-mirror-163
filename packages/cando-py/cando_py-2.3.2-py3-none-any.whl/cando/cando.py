...
def get_prot_info(p, org='homo_sapien'):
    pre = os.path.dirname(__file__) + "/data/v2.2+"
    p_file = '{}/prots/{}-metadata.tsv'.format(pre,org)
    if not os.path.exists(p_file):
        print('Downloading proteome metadata for {}...'.format(org))
        ...
        print("{} proteome metadata downloaded.".format(org))
    prots_df = pd.read_csv(p_file,index_col=0,sep='\t')
