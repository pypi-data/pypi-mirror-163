...
from composition import directory, list_apps, storage, api
from composition.models import Application, Action, Context
from composition.storage import get_yaml


def install_application(template="template.yaml", values=None, application_id=None):
    directory.handle_action(Action.INSTALL, template, values, application_id)
...
def generate_template(template_dir, template_file, app_details, application_id, values):
    ...
    path = storage.write_compose(application_id, output_str, app_details, compose_path)
