...
class Statistics:
    ...
    def Accuracy(actual,predicted) -> list:
        if len(actual) == len(predicted):
            rel_x,rel_y,score = [],[],0
            for i in range(0,len(actual)):
                rel_x.append(int(actual[i]))
                rel_y.append(int(predicted[i]))
...
class LoadData:
    def load_data(file:str,ext = "csv") -> str:
        try:
            ...
            return f"{error} | {file}.{ext} not found!"
