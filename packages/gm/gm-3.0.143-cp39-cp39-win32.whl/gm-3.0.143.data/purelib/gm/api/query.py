...
def get_expire_rest_days(symbols=None, trade_date=None, trading_days=False, df=False):
    ...
    if trading_days:
        ...
        for info in infos:
            ...
            idx = _bin_search(exchange_trading_dates[exchange], delisted_date_str)
            trade_date_idx = _bin_search(exchange_trading_dates[exchange], trade_date_str)
            result.append({
                "symbol": info["symbol"],
...
def bond_convertible_get_call_info(symbols=None, start_date=None, end_date=None):
    ...
    查询可转债赎回信息
    """
    infos = fundamentalapi.bond_convertible_get_call_info(symbols, start_date, end_date)
