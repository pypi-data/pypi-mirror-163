...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    vja__gssoc = pq_node.out_vars[0].name
    aseds__ivler = pq_node.out_vars[1].name
    if vja__gssoc not in lives and aseds__ivler not in lives:
        return None
    elif vja__gssoc not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
        pq_node.is_live_table = False
    elif aseds__ivler not in lives:
