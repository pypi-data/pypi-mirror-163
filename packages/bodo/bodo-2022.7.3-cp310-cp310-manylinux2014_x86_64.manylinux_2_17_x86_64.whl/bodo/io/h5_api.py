...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        llc__wqlks = []
        pixp__vbfel = h5g_get_num_objs(obj_id)
        for wtfyx__cmv in range(pixp__vbfel):
            did__hksb = h5g_get_objname_by_idx(obj_id, wtfyx__cmv)
            llc__wqlks.append(did__hksb)
        return llc__wqlks
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    nqgp__ufg = parse_dtype(dtype)
    frk__qsds = np.int32(numba_to_c_type(nqgp__ufg))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            zjk__olbt = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    qebqj__enfon = tuple_to_ptr(starts)
    kigv__dpwmj = tuple_to_ptr(counts)
    guuz__wvi = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, qebqj__enfon,
        kigv__dpwmj, is_parallel, out_arr.ctypes, guuz__wvi)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    kigv__dpwmj = tuple_to_ptr(counts)
    guuz__wvi = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, qebqj__enfon,
        kigv__dpwmj, is_parallel, out_arr.ctypes, guuz__wvi)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ybfst__ovhb = types.Array(dtype, ndim, 'C')
        moiw__tqbr = args[3]
        ybfst__ovhb = self.context.resolve_function_type(operator.getitem,
            [ybfst__ovhb, moiw__tqbr], {}).return_type.copy(layout='C')
        return signature(ybfst__ovhb, *args)
...
def get_filter_read_indices(bool_arr):
    cdryl__qot = bool_arr.nonzero()[0]
    sbh__qvhzh = bodo.libs.distributed_api.get_rank()
    vzxl__unauc = bodo.libs.distributed_api.get_size()
    bny__exz = np.empty(vzxl__unauc, np.int64)
    uaa__jffyz = len(bool_arr)
    bodo.libs.distributed_api.allgather(bny__exz, uaa__jffyz)
    uxi__rda = bny__exz.cumsum()[sbh__qvhzh] - uaa__jffyz
    cdryl__qot += uxi__rda
    ixx__vnv = bodo.libs.distributed_api.dist_reduce(len(cdryl__qot), np.
        int32(sum_op))
    nusd__wubt = bodo.libs.distributed_api.gatherv(cdryl__qot)
    if sbh__qvhzh == 0:
        jxew__etd = nusd__wubt
    else:
        jxew__etd = np.empty(ixx__vnv, cdryl__qot.dtype)
    bodo.libs.distributed_api.bcast(jxew__etd)
    akttp__wli = bodo.libs.distributed_api.get_start(ixx__vnv, vzxl__unauc,
        sbh__qvhzh)
    fvz__nysgo = bodo.libs.distributed_api.get_end(ixx__vnv, vzxl__unauc,
        sbh__qvhzh)
    return jxew__etd[akttp__wli:fvz__nysgo]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        tneqw__hca = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], tneqw__hca)
        return builder.bitcast(tneqw__hca, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    kigv__dpwmj = tuple_to_ptr(counts)
    guuz__wvi = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, qebqj__enfon,
        kigv__dpwmj, is_parallel, out_arr.ctypes, guuz__wvi, read_indices.
