...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        bsf__auci = self.overloads[sig]
        jltxi__sjc = bsf__auci.metadata.get('distributed_diagnostics', None)
        if jltxi__sjc is None:
            ldf__bwqlw = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(ldf__bwqlw)
        jltxi__sjc.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        mciic__vekjf = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(mciic__vekjf)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        nid__vtbes = options.pop('distributed')
        pipeline_class = (pipeline_class if nid__vtbes else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        cbz__ehww = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if cbz__ehww else
            pipeline_class)
    wtbu__ovve = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(wtbu__ovve, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(wtbu__ovve)
        else:
            return master_mode_wrapper(wtbu__ovve)
    else:
        return wtbu__ovve
