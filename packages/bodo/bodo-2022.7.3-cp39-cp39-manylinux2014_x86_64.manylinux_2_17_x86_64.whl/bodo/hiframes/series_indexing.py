...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        gxgnf__ppj = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(gxgnf__ppj)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qykdi__vlzk = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, qykdi__vlzk)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        sqj__ypocq, = args
        fdnsl__mhnk = signature.return_type
        iwt__nace = cgutils.create_struct_proxy(fdnsl__mhnk)(context, builder)
        iwt__nace.obj = sqj__ypocq
        context.nrt.incref(builder, signature.args[0], sqj__ypocq)
        return iwt__nace._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                esrp__aylh = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        gxgnf__ppj = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(gxgnf__ppj)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qykdi__vlzk = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, qykdi__vlzk)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        sqj__ypocq, = args
        fgtk__ahhew = signature.return_type
        lasqj__flxd = cgutils.create_struct_proxy(fgtk__ahhew)(context, builder
            )
        lasqj__flxd.obj = sqj__ypocq
        context.nrt.incref(builder, signature.args[0], sqj__ypocq)
        return lasqj__flxd._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                jfro__crt = bodo.utils.conversion.coerce_to_ndarray(idx)
                axg__gncad = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    jfro__crt]
                tobnx__qsusx = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    jfro__crt]
                gxgnf__ppj = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(axg__gncad,
                    tobnx__qsusx, gxgnf__ppj)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                axg__gncad = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                tobnx__qsusx = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                gxgnf__ppj = bodo.hiframes.pd_series_ext.get_series_name(S)
    ...
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        esrp__aylh = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(esrp__aylh))
                        jfro__crt = bodo.utils.conversion.coerce_to_ndarray(idx
                            )
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            jfro__crt] = s
            ...
            def impl_arr(I, idx, val):
                jfro__crt = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[jfro__crt
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        gxgnf__ppj = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(gxgnf__ppj)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qykdi__vlzk = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, qykdi__vlzk)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        sqj__ypocq, = args
        foszw__tlx = signature.return_type
        evqol__gmzdv = cgutils.create_struct_proxy(foszw__tlx)(context, builder
            )
        evqol__gmzdv.obj = sqj__ypocq
        context.nrt.incref(builder, signature.args[0], sqj__ypocq)
        return evqol__gmzdv._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            axg__gncad = bodo.hiframes.pd_series_ext.get_series_data(S)
            tobnx__qsusx = bodo.hiframes.pd_series_ext.get_series_index(S)
            hrj__nsz = (idx - tobnx__qsusx._start) // tobnx__qsusx._step
            return axg__gncad[hrj__nsz]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    jfro__crt = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(axg__gncad[
                        jfro__crt])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    sxgw__evc = bodo.hiframes.pd_series_ext.get_series_index(S)
                    cyfm__vku = sxgw__evc.get_loc(idx)
                    if cyfm__vku is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(cyfm__vku)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                jfro__crt = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[jfro__crt
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        zhh__fvpk = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(
            refw__utyzy, int) for refw__utyzy in zhh__fvpk):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            kfukl__yia = get_literal_value(idx)
            if kfukl__yia not in zhh__fvpk:
                raise_bodo_error(
                    f"Series label-based getitem: '{kfukl__yia}' not in {zhh__fvpk}"
                    )
            arr_ind = zhh__fvpk.index(kfukl__yia)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    iwt__nace = cgutils.create_struct_proxy(fromty)(context, builder, val)
    ywnu__puwh = context.cast(builder, iwt__nace.obj, fromty.stype, toty.stype)
    yxbft__vxbu = cgutils.create_struct_proxy(toty)(context, builder)
    yxbft__vxbu.obj = ywnu__puwh
    return yxbft__vxbu._getvalue()
