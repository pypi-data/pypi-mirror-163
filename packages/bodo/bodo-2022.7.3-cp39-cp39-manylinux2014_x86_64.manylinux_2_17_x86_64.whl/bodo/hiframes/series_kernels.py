...
def _column_filter_impl(B, ind):
    rnir__msmee = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for yayyp__kykon in numba.parfors.parfor.internal_prange(len(rnir__msmee)):
        if ind[yayyp__kykon]:
            rnir__msmee[yayyp__kykon] = B[yayyp__kykon]
        else:
            bodo.libs.array_kernels.setna(rnir__msmee, yayyp__kykon)
    return rnir__msmee
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    vlsg__lso = len(B)
    gird__cgf = 0
    for yayyp__kykon in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, yayyp__kykon):
            gird__cgf += 1
    srjh__opv = vlsg__lso - gird__cgf
    odz__web = bodo.libs.str_arr_ext.num_total_chars(B)
    rnir__msmee = bodo.libs.str_arr_ext.pre_alloc_string_array(srjh__opv,
        odz__web)
    bodo.libs.str_arr_ext.copy_non_null_offsets(rnir__msmee, B)
    bodo.libs.str_arr_ext.copy_data(rnir__msmee, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(rnir__msmee, -1)
    return rnir__msmee
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    cthbh__oiodi = first_moment / count
    zxb__gbqjx = (third_moment - 3 * second_moment * cthbh__oiodi + 2 *
        count * cthbh__oiodi ** 3)
    uspmr__ynxrl = second_moment - cthbh__oiodi * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * zxb__gbqjx / uspmr__ynxrl ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    cthbh__oiodi = first_moment / count
    yiyc__nxxvm = (fourth_moment - 4 * third_moment * cthbh__oiodi + 6 *
        second_moment * cthbh__oiodi ** 2 - 3 * count * cthbh__oiodi ** 4)
    qnc__fdp = second_moment - cthbh__oiodi * first_moment
    cftge__uuew = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    kdir__xgd = count * (count + 1) * (count - 1) * yiyc__nxxvm
    nujd__phy = (count - 2) * (count - 3) * qnc__fdp ** 2
    s = (count - 1) * (kdir__xgd / nujd__phy - cftge__uuew)
    s = s / (count - 1)
    return s
