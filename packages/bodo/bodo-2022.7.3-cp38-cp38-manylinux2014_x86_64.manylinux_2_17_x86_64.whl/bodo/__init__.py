...
def _global_except_hook(exctype, value, traceback):
    ...
    bjjq__xhr = MPI.COMM_WORLD
    ceoxo__vtopw = bjjq__xhr.Get_rank()
    fmzc__sewe = 3.0
    xndz__avl = True
    krgi__exyyu = bjjq__xhr.Ibarrier()
    wwg__vvz = time.time()
    while time.time() - wwg__vvz < fmzc__sewe:
        time.sleep(0.1)
        if krgi__exyyu.Test():
            xndz__avl = False
            break
    try:
        ...
        if xndz__avl:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {ceoxo__vtopw}. \n')
