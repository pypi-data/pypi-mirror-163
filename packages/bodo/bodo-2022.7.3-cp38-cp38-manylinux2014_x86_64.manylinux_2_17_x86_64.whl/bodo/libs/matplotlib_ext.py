...
def _install_mpl_types():
    uyr__qzxl = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for duoew__gpg, yon__tredh in uyr__qzxl:
        install_py_obj_class(types_name=duoew__gpg, python_type=yon__tredh,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    wpd__mry = ', '.join(f'e{stpze__wqkuc}' for stpze__wqkuc in range(len(
        args)))
    if wpd__mry:
        wpd__mry += ', '
    ioky__huux = ', '.join(f"{qlw__szbx} = ''" for qlw__szbx in kws.keys())
    irvl__qrpzu = 'matplotlib_obj, ' if obj_typ is not None else ''
    rulja__rmdr = f'def mpl_stub({irvl__qrpzu} {wpd__mry} {ioky__huux}):\n'
    rulja__rmdr += '    pass\n'
    oggld__phjc = {}
    exec(rulja__rmdr, {}, oggld__phjc)
    dnss__sew = oggld__phjc['mpl_stub']
    swrj__xff = numba.core.utils.pysignature(dnss__sew)
    oae__auyyh = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(oae__auyyh)).replace(pysig=
        swrj__xff)


def generate_axes_typing(mod_name, nrows, ncols):
    zkbeg__rwcln = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(zkbeg__rwcln.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(zkbeg__rwcln.format(mod_name, 'ncols'))
    iwkt__rjtd = get_overload_const_int(nrows)
    zffzi__mfm = get_overload_const_int(ncols)
    if iwkt__rjtd < 1:
        raise BodoError(zkbeg__rwcln.format(mod_name, 'nrows'))
    if zffzi__mfm < 1:
        raise BodoError(zkbeg__rwcln.format(mod_name, 'ncols'))
    if iwkt__rjtd == 1 and zffzi__mfm == 1:
        cnxnl__zuyt = types.mpl_axes_type
    else:
        if zffzi__mfm == 1:
            ioy__ubjw = types.mpl_axes_type
        else:
            ioy__ubjw = types.Tuple([types.mpl_axes_type] * zffzi__mfm)
        cnxnl__zuyt = types.Tuple([ioy__ubjw] * iwkt__rjtd)
    return cnxnl__zuyt


def generate_pie_return_type(args, kws):
    rnttw__mgte = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if rnttw__mgte == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    fszn__znygc = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(fszn__znygc):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(fszn__znygc):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        tklf__putbi = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, tklf__putbi]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    tklf__putbi = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    duoew__gpg = str(tklf__putbi)
    if not hasattr(types, duoew__gpg):
        duoew__gpg = f'objmode_type{ir_utils.next_label()}'
        setattr(types, duoew__gpg, tklf__putbi)
    rulja__rmdr = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{duoew__gpg}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    oggld__phjc = {}
    exec(rulja__rmdr, {'numba': numba, 'np': np}, oggld__phjc)
    impl = oggld__phjc['impl']
    return impl
