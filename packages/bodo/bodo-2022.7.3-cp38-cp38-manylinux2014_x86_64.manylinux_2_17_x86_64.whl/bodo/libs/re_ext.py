...
def box_regex_flag(typ, val, c):
    jut__laffn = c.box(types.int64, val)
    nrsa__xup = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(nrsa__xup, (jut__laffn,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    jut__laffn = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, jut__laffn)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    xjr__bzwx = context.get_python_api(builder)
    yenie__qnsui = builder.icmp_signed('==', val, xjr__bzwx.borrow_none())
    with builder.if_then(yenie__qnsui):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    xous__uebjh = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(xous__uebjh)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        qdmu__qfsc = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if qdmu__qfsc > 1:
            typ = types.List(types.Tuple([string_type] * qdmu__qfsc))
...
def overload_match_group(m, *args):
    ...
    ysy__xfad = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            qbiz__kmsf = args[0]
            with numba.objmode(out=ysy__xfad):
                out = m.group(qbiz__kmsf)
            return out
        return _match_group_impl_one
    kdo__tiv = 'tuple_str_{}'.format(len(args))
    setattr(types, kdo__tiv, types.Tuple([ysy__xfad] * len(args)))
    txamk__egoe = ', '.join('group{}'.format(jfk__tdzl + 1) for jfk__tdzl in
        range(len(args)))
    clv__vlk = 'def _match_group_impl(m, *args):\n'
    clv__vlk += '  ({}) = args\n'.format(txamk__egoe)
    clv__vlk += "  with numba.objmode(out='{}'):\n".format(kdo__tiv)
    clv__vlk += '    out = m.group({})\n'.format(txamk__egoe)
    clv__vlk += '  return out\n'
    pdra__patmq = {}
    exec(clv__vlk, globals(), pdra__patmq)
    ofv__wnsr = pdra__patmq['_match_group_impl']
    return ofv__wnsr
...
def overload_match_groups(m, default=None):
    sbfp__stkd = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=sbfp__stkd):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(cebjt__gjs is None for cebjt__gjs in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=ysy__xfad):
