...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        kzslc__lizn = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, kzslc__lizn)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            kjfg__gpsf = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(kjfg__gpsf.dtype)
            file_write(unicode_to_utf8(fname), kjfg__gpsf.ctypes, 
                dtype_size * kjfg__gpsf.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            kjfg__gpsf = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(kjfg__gpsf.dtype)
            ayhe__ituv = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                kjfg__gpsf.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), kjfg__gpsf.ctypes,
                start, count, ayhe__ituv)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            solob__mtlyk = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                solob__mtlyk = min(solob__mtlyk, count * dtype_size)
            if solob__mtlyk < 0:
                return -1
            return solob__mtlyk
        return impl
