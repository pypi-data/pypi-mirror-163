...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        vvcy__ayao = len(A.types)
        tsc__srw = 'def f(A):\n'
        vypkb__sjw = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(vvcy__ayao))
        tsc__srw += '  return ({}{})\n'.format(vypkb__sjw, ',' if 
            vvcy__ayao == 1 else '')
        uyl__jyt = {}
        exec(tsc__srw, {'decode_if_dict_array': decode_if_dict_array}, uyl__jyt
            )
        impl = uyl__jyt['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                vvcy__ayao += 1
            rpc__nzro = []
            for i in range(vvcy__ayao):
                rpc__nzro.append(decode_if_dict_array(A[i]))
            return rpc__nzro
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            dmj__citam = bodo.hiframes.pd_series_ext.get_series_data(A)
            otkl__wtom = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            dbpa__tunh = decode_if_dict_array(dmj__citam)
            return bodo.hiframes.pd_series_ext.init_series(dbpa__tunh,
                otkl__wtom, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        regr__daa = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(regr__daa, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        fpyqd__plbu = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(fpyqd__plbu(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        wgm__lgf = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=wgm__lgf)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        qqnl__wvfp = self.get_full_filename(fname)
        return self._get_schema(qqnl__wvfp)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        oop__esjcb = dmm.lookup(bodo.typeof(fe_type.fname))
        zmur__gnev = [(a, mss__isz) for a, mss__isz in zip(oop__esjcb.
            _fields, oop__esjcb._members)]
        super().__init__(dmm, fe_type, zmur__gnev)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    xir__tvjz = t.initial_value
    if isinstance(xir__tvjz, dict):
        xir__tvjz = xir__tvjz.values()
    return not any(isinstance(zuk__iimih, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for zuk__iimih in xir__tvjz)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (zuk__iimih) for zuk__iimih in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (zuk__iimih) for zuk__iimih in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        bkj__zbcjs = []
        for zuk__iimih in val.literal_value:
            cmsn__ieiyp = get_overload_const(zuk__iimih)
            if cmsn__ieiyp == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                bkj__zbcjs.append(cmsn__ieiyp)
        return bkj__zbcjs
    ...
    if isinstance(val, types.BaseTuple):
        bkj__zbcjs = []
        for zuk__iimih in val.types:
            cmsn__ieiyp = get_overload_const(zuk__iimih)
            if cmsn__ieiyp == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                bkj__zbcjs.append(cmsn__ieiyp)
        return tuple(bkj__zbcjs)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    cmsn__ieiyp = get_overload_const(val)
    zjibm__vjcl = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(cmsn__ieiyp, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {zjibm__vjcl} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    cmsn__ieiyp = get_overload_const(val)
    if cmsn__ieiyp not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{cmsn__ieiyp}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    rll__euwe = False
    for a in args_dict:
        qcmiv__wke = get_overload_const(args_dict[a])
        zhy__dki = arg_defaults_dict[a]
        if (qcmiv__wke is NOT_CONSTANT or qcmiv__wke is not None and 
            zhy__dki is None or qcmiv__wke is None and zhy__dki is not None or
            qcmiv__wke != zhy__dki or qcmiv__wke is not _no_input and 
            zhy__dki is _no_input or qcmiv__wke is _no_input and zhy__dki
             is not _no_input):
            onvsi__wxlg = (
                f'{fn_str}: {a} parameter only supports default value {zhy__dki}'
                )
            rll__euwe = True
            break
    if rll__euwe and package_name == 'pandas':
        if module_name == 'IO':
            onvsi__wxlg += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(dslr__pjfra): get_literal_value(
            zuk__iimih) for dslr__pjfra, zuk__iimih in val.literal_value.
            items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    mar__ualnx = [get_overload_const(zuk__iimih) for zuk__iimih in val.
        types[1:]]
    return {mar__ualnx[2 * i]: mar__ualnx[2 * i + 1] for i in range(len(
        mar__ualnx) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(zuk__iimih) for zuk__iimih in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        jcqb__tupx = get_overload_const_str(dtype)
        if jcqb__tupx.startswith('Int') or jcqb__tupx.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(jcqb__tupx), None)
        if jcqb__tupx == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if jcqb__tupx == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(jcqb__tupx))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    roznv__mlw = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        lsih__str = []
        for i, oxphh__pybrj in enumerate(t.names_typ):
            if oxphh__pybrj == types.none:
                lsih__str.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(oxphh__pybrj):
                raise BodoError(roznv__mlw)
            lsih__str.append(get_overload_const_str(oxphh__pybrj))
        return tuple(lsih__str)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        iol__efnix = typ.literal_value
        kjuu__zivn = []
        for awzl__chimj in ('start', 'stop', 'step'):
            hsh__ovtmn = getattr(typ.literal_value, awzl__chimj)
            fepr__obo = c.pyapi.make_none(
                ) if hsh__ovtmn is None else c.pyapi.from_native_value(types
                .literal(hsh__ovtmn), hsh__ovtmn, c.env_manager)
            kjuu__zivn.append(fepr__obo)
        lyv__ivzur = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        gie__hky = c.pyapi.call_function_objargs(lyv__ivzur, kjuu__zivn)
        for a in kjuu__zivn:
            c.pyapi.decref(a)
        c.pyapi.decref(lyv__ivzur)
        return gie__hky
...
def box_list_literal(typ, val, c):
    vir__zjmf = typ.literal_value
    ijrlg__fdjxm = [c.pyapi.from_native_value(types.literal(zuk__iimih),
        zuk__iimih, c.env_manager) for zuk__iimih in vir__zjmf]
    sxsm__mvp = c.pyapi.list_pack(ijrlg__fdjxm)
    for a in ijrlg__fdjxm:
        c.pyapi.decref(a)
    return sxsm__mvp
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    mbjyh__lpx = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(zuk__iimih) for zuk__iimih in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(dslr__pjfra): get_literal_value(
                zuk__iimih) for dslr__pjfra, zuk__iimih in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(zuk__iimih) for zuk__iimih in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(zuk__iimih) for zuk__iimih in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        dmj__citam = types.Array(dtype, 1, 'C')
        if vmvfy__ejvr:
            return to_nullable_type(dmj__citam)
        return dmj__citam
...
def types_equality_exists(t1, t2):
    cbh__kcyui = numba.core.registry.cpu_target.typing_context
    try:
        cbh__kcyui.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    fjgg__yrv = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in fjgg__yrv:
        return True
    cbh__kcyui = numba.core.registry.cpu_target.typing_context
    try:
        cbh__kcyui.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        nulce__mjj = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(nulce__mjj, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    euob__qnib, jphvq__ifsm = get_common_scalar_dtype(scalar_types)
    return jphvq__ifsm
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        tht__vhdz = np.find_common_type([numba.np.numpy_support.as_dtype(t) for
            t in scalar_types], [])
        if tht__vhdz != object:
            return numba.np.numpy_support.from_dtype(tht__vhdz), True
    except numba.core.errors.NumbaNotImplementedError as zpaqv__bug:
        pass
    ...
    nzro__ggyvh = itertools.groupby(scalar_types)
    if next(nzro__ggyvh, True) and not next(nzro__ggyvh, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            rqpwp__lzq.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            rqpwp__lzq.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                rqpwp__lzq.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                rqpwp__lzq.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        rqpwp__lzq.append(typ)
    return rqpwp__lzq
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        sebsb__igcro = getfullargspec(func)
        assert sebsb__igcro.varargs is None, 'varargs not supported'
        assert sebsb__igcro.varkw is None, 'varkw not supported'
        defaults = [
            ] if sebsb__igcro.defaults is None else sebsb__igcro.defaults
        kxa__kgsyt = len(sebsb__igcro.args) - len(defaults)
        args = sebsb__igcro.args[1:] if attr_name else sebsb__igcro.args[:]
        ukff__fwsc = []
        for i, wtzo__ereia in enumerate(sebsb__igcro.args):
            if i < kxa__kgsyt:
                ukff__fwsc.append(wtzo__ereia)
            elif str(defaults[i - kxa__kgsyt]) != '<deprecated parameter>':
                ukff__fwsc.append(wtzo__ereia + '=' + str(defaults[i -
                    kxa__kgsyt]))
            else:
                args.remove(wtzo__ereia)
        if sebsb__igcro.kwonlyargs is not None:
            for wtzo__ereia in sebsb__igcro.kwonlyargs:
                args.append(f'{wtzo__ereia}={wtzo__ereia}')
                ukff__fwsc.append(
                    f'{wtzo__ereia}={str(sebsb__igcro.kwonlydefaults[wtzo__ereia])}'
                    )
        sig = ', '.join(ukff__fwsc)
        args = ', '.join(args)
    else:
    ...
    mnl__tkip = f'self.{attr_name}' if attr_name else f'{func_name}'
    tsc__srw = f'def overload_impl({sig}):\n'
    tsc__srw += f'    def impl({sig}):\n'
    if single_rank:
        tsc__srw += f'        if bodo.get_rank() == 0:\n'
        xlm__thf = '    '
    else:
        xlm__thf = ''
    tsc__srw += f"        {xlm__thf}with numba.objmode(res='{type_name}'):\n"
    if is_function:
        tsc__srw += f'            {xlm__thf}res = {mnl__tkip}({args})\n'
    else:
        tsc__srw += f'            {xlm__thf}res = {mnl__tkip}\n'
    tsc__srw += f'        return res\n'
    tsc__srw += f'    return impl\n'
    uyl__jyt = {}
    navfq__vtjmf = globals()
    if not attr_name:
        navfq__vtjmf[func_name] = func
    exec(tsc__srw, navfq__vtjmf, uyl__jyt)
    beezy__xyxv = uyl__jyt['overload_impl']
    return beezy__xyxv


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        beezy__xyxv = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(beezy__xyxv)
    except Exception as zpaqv__bug:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        beezy__xyxv = _gen_objmode_overload(method, output_type,
            method_name, True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(beezy__xyxv)
    except Exception as zpaqv__bug:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        beezy__xyxv = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(beezy__xyxv)
    except Exception as zpaqv__bug:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, dqtf__ybwnw = args
        if isinstance(dqtf__ybwnw, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[dqtf__ybwnw]),
                *args)
...
def construct_pysig(arg_names, defaults):
    tsc__srw = f'def stub('
    for wtzo__ereia in arg_names:
        tsc__srw += wtzo__ereia
        if wtzo__ereia in defaults:
            if isinstance(defaults[wtzo__ereia], str):
                tsc__srw += f"='{defaults[wtzo__ereia]}'"
            else:
                tsc__srw += f'={defaults[wtzo__ereia]}'
        tsc__srw += ', '
    tsc__srw += '):\n'
    tsc__srw += '    pass\n'
    uyl__jyt = {}
    exec(tsc__srw, {}, uyl__jyt)
    cnga__gfrb = uyl__jyt['stub']
    return numba.core.utils.pysignature(cnga__gfrb)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    rtcr__qpue = len(arg_names)
    nljwk__xmp = len(args) + len(kws)
    if nljwk__xmp > rtcr__qpue:
        pwn__jtp = 'argument' if rtcr__qpue == 1 else 'arguments'
        qpyic__itbaz = 'was' if nljwk__xmp == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {rtcr__qpue} {pwn__jtp}, but {nljwk__xmp} {qpyic__itbaz} provided.'
            )
    cvhp__uix = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        hbqq__bnc = bodo.utils.transform.fold_argument_types(cvhp__uix,
            args, kws)
    except Exception as qyayp__socxr:
        raise_bodo_error(f'{func_name}(): {qyayp__socxr}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                vkr__lpo[arg_name] = hbqq__bnc[i]
                hdb__iot[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, vkr__lpo, hdb__iot)
    return cvhp__uix, hbqq__bnc
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        zmur__gnev = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, zmur__gnev)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [ndpl__ocfx] = sig.args
    [gpxtm__mof] = args
    wtq__gro = context.make_helper(builder, ndpl__ocfx, value=gpxtm__mof)
    upmu__iihn = signature(types.intp, ndpl__ocfx.arr_type)
    iwgdm__ukwca = context.compile_internal(builder, lambda a: len(a),
        upmu__iihn, [wtq__gro.array])
    otkl__wtom = builder.load(wtq__gro.index)
    fix__osxn = builder.icmp_signed('<', otkl__wtom, iwgdm__ukwca)
    result.set_valid(fix__osxn)
    with builder.if_then(fix__osxn):
        btwp__wsag = signature(ndpl__ocfx.yield_type, ndpl__ocfx.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            btwp__wsag, [wtq__gro.array, otkl__wtom])
        result.yield_(value)
        ivqsy__hrm = cgutils.increment_index(builder, otkl__wtom)
        builder.store(ivqsy__hrm, wtq__gro.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    xlo__kdc = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        cbag__pqu = None
    else:
        cbag__pqu = types.StringLiteral(name)
    if xlo__kdc == bodo.hiframes.pd_index_ext.NumericIndexType:
        ekiv__edu = xlo__kdc(elem_dtype, cbag__pqu, arr_typ)
    elif xlo__kdc == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        ekiv__edu = xlo__kdc(bodo.CategoricalArrayType(elem_dtype),
            cbag__pqu, arr_typ)
    else:
        ekiv__edu = xlo__kdc(cbag__pqu, arr_typ)
    return ekiv__edu
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(zuk__iimih, t) for zuk__iimih, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        ycyqb__ahh = pd.Index(val_typ.columns)
        cmm__wevkf = pd.Index(typ.columns)
        qppx__kahi = ycyqb__ahh.argsort()
        inpzv__kslhu = cmm__wevkf.argsort()
        wmz__rcds = val_typ.copy(data=tuple(np.array(val_typ.data)[
            qppx__kahi]), columns=tuple(ycyqb__ahh[qppx__kahi]))
        blf__pfjs = typ.copy(data=tuple(np.array(typ.data)[inpzv__kslhu]),
            columns=tuple(cmm__wevkf[inpzv__kslhu]))
        if wmz__rcds == blf__pfjs:
            val_typ = typ
