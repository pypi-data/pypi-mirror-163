...
def get_new_null_mask_bool_index(old_mask, ind, n):
    jze__ihamp = n + 7 >> 3
    kswp__alydk = np.empty(jze__ihamp, np.uint8)
    bzx__nmx = 0
    for ofsc__vrtn in range(len(ind)):
        if ind[ofsc__vrtn]:
            wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                ofsc__vrtn)
            bodo.libs.int_arr_ext.set_bit_to_arr(kswp__alydk, bzx__nmx,
                wkvr__wfzno)
            bzx__nmx += 1
    return kswp__alydk
...
def array_getitem_bool_index(A, ind):
    ...
    ypqs__fipa = A._data[ind]
    n = len(ypqs__fipa)
    kswp__alydk = get_new_null_mask_bool_index(old_mask, ind, n)
    return ypqs__fipa, kswp__alydk
...
def get_new_null_mask_int_index(old_mask, ind, n):
    ...
    for ofsc__vrtn in range(len(ind)):
        wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            ind[ofsc__vrtn])
        bodo.libs.int_arr_ext.set_bit_to_arr(kswp__alydk, bzx__nmx, wkvr__wfzno
            )
        bzx__nmx += 1
    return kswp__alydk
...
def array_getitem_int_index(A, ind):
    mcukk__tbx = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    ypqs__fipa = A._data[mcukk__tbx]
    n = len(ypqs__fipa)
    kswp__alydk = get_new_null_mask_int_index(old_mask, mcukk__tbx, n)
    return ypqs__fipa, kswp__alydk


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    zlkw__bddp = numba.cpython.unicode._normalize_slice(ind, n)
    bia__awgi = numba.cpython.unicode._slice_span(zlkw__bddp)
    jze__ihamp = bia__awgi + 7 >> 3
    kswp__alydk = np.empty(jze__ihamp, np.uint8)
    bzx__nmx = 0
    for ofsc__vrtn in range(zlkw__bddp.start, zlkw__bddp.stop, zlkw__bddp.step
        ):
        wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            ofsc__vrtn)
        bodo.libs.int_arr_ext.set_bit_to_arr(kswp__alydk, bzx__nmx, wkvr__wfzno
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    ypqs__fipa = np.ascontiguousarray(A._data[ind])
    kswp__alydk = get_new_null_mask_slice_index(old_mask, ind, n)
    return ypqs__fipa, kswp__alydk
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for ofsc__vrtn in range(n):
                A._data[idx[ofsc__vrtn]] = val._data[ofsc__vrtn]
                wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, ofsc__vrtn)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    ofsc__vrtn], wkvr__wfzno)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for ofsc__vrtn in idx:
                A._data[ofsc__vrtn] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            tefuu__ulp = 0
            for ofsc__vrtn in range(n):
                if not bodo.libs.array_kernels.isna(idx, ofsc__vrtn) and idx[
                    ofsc__vrtn]:
                    A._data[ofsc__vrtn] = val._data[tefuu__ulp]
                    wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, tefuu__ulp)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        ofsc__vrtn, wkvr__wfzno)
                    tefuu__ulp += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    zlkw__bddp = numba.cpython.unicode._normalize_slice(idx, n)
    tefuu__ulp = 0
    for ofsc__vrtn in range(zlkw__bddp.start, zlkw__bddp.stop, zlkw__bddp.step
        ):
        wkvr__wfzno = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            tefuu__ulp)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, ofsc__vrtn,
            wkvr__wfzno)
        tefuu__ulp += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            zlkw__bddp = numba.cpython.unicode._normalize_slice(idx, len(A))
            for ofsc__vrtn in range(zlkw__bddp.start, zlkw__bddp.stop,
                zlkw__bddp.step):
                A._data[ofsc__vrtn] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    ofsc__vrtn, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for ofsc__vrtn in idx:
                    bodo.libs.array_kernels.setna(A, ofsc__vrtn)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    liu__brab = bodo.libs.str_arr_ext.pre_alloc_string_array(n,
                        -1)
                    for ofsc__vrtn in numba.parfors.parfor.internal_prange(n):
                        if idx[ofsc__vrtn] or bodo.libs.array_kernels.isna(A,
                            ofsc__vrtn):
                            liu__brab[ofsc__vrtn] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(liu__brab,
                                ofsc__vrtn)
                        else:
                            liu__brab[ofsc__vrtn] = A[ofsc__vrtn]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for ofsc__vrtn in range(n):
                    if not bodo.libs.array_kernels.isna(idx, ofsc__vrtn
                        ) and idx[ofsc__vrtn]:
                        bodo.libs.array_kernels.setna(A, ofsc__vrtn)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for ofsc__vrtn in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, ofsc__vrtn)
                        continue
                    A[ofsc__vrtn] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        fwy__bnlbz = context.make_helper(builder, val_t, args[0])
        cspe__trtu = fwy__bnlbz.data
        context.nrt.incref(builder, val_t.type, cspe__trtu)
        return cspe__trtu
