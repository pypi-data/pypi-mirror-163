...
def get_source_info(run_path):
    ...
    from example_demo.program_process.process_sftp import SFTPBase

    class SFTPProcess(SFTPBase):
        def __init__(self):
            # 1. sftp 请求信息
            self.sftp_info = {
                "sftp_host": '',
            ...
            super().__init__(self.sftp_info, self.file_download_info)
        # 整理 返回抓取list
        # def get_all_source(self):
        #     pass
    return SFTPProcess().get_all_source()
...
def download_parse_info(sour, run_path=''):
    ...
    from example_demo.program_process.process_sftp import SftpParseBase
    class ParseSource(SftpParseBase):
    ...
    for source in sour:
        ...
        if info:
            download_res = SFTPDownloader(info).download()
