...
def hooks_download(sour, run_path):
    import sys
    sys.path.insert(0, run_path)
    from example_demo.request_downloader.sql_downloader import SQLDownloader
    return SQLDownloader(sour).download()


# 3. 其他必要的任务（邮件，电话等）
# 3. 数据解析
@win_virtualenv(python_version=PY_VERSION, requirements=REQUIREMENTS, psrp_conn_id=CONN_ID,
                task_id="parser_file_info", do_xcom_push=True, pip_install_options=PIP_INSTALL_OPTIONS)
...
with DAG(
    ...
    for source in all_resource:
        ...
        if info:
            # task download_file
            download_data = hooks_download(info)
            source["download_data"] = download_data
