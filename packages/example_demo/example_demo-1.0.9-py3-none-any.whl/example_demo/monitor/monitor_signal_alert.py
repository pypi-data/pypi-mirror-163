...
class email_logit(logit):
    ...
    def notify(self):
        from_addr = "zhangyf@jindefund.com"  #
...
def send_mail(sub, content, email_host=None, email_to_list=None, email_from=None, payload_file=None, debug=False):
    """
        发送邮件
        :param:str sub: 主题
    ...
    email_host = email_host or setting.EMAIL_HOST
    ...
    if payload_file:
        ...
        email.attach(att1)
    if debug:
        print('send_email_host:%s, from:%s,to:%s'%(email_host, email_from, ";".join(email_to_list)))
    try:
        s = smtplib.SMTP()
        s.connect(email_host)  # 连接smtp服务器
