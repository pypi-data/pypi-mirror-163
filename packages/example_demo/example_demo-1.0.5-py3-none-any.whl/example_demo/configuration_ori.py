...
class OnedatautilConfigParser(ConfigParser):
    ...
    def __init__(self, default_config: Optional[str] = None, *args, **kwargs):
        ...
        if default_config is not None:
            ...
            default = self.onedatautil_defaults.get('logging', 'log_filename_template', fallback=None)
            print(default)
            if default:
    ...
    def get(self, section: str, key: str, **kwargs) -> Optional[str]:  # type: ignore[override]
        ...
        key = str(key).lower()
        print(f"self.deprecated_options:{self.deprecated_options}")
        deprecated_section, deprecated_key, _ = self.deprecated_options.get(
            (section, key), (None, None, None)
        )
        print(deprecated_section, deprecated_key, _)
