...
# These were proposed in https://jira.iter.org/browse/IMAS-3843, https://jira.iter.org/projects/IMAS/issues/IMAS-4207, and https://jira.iter.org/projects/IMAS/issues/IMAS-4248 and specified by https://git.iter.org/projects/IMAS/repos/data-dictionary/pull-requests/491/.
# The differences between total_chi_squared and chi_squared_all come from how EFIT outputs the results.  It is unlikely both will be allowed to enter the IMAS standard, but still are useful in the context of EFIT.
_extra_structures = {
    'equilibrium': {
        'equilibrium.time_slice[:].convergence.grad_shafranov_deviation_expression': {
            ...
            "full_path": "equilibrium/time_slice(itime)/convergence/grad_shafranov_deviation_expression",
        },
        'equilibrium.time_slice[:].convergence.grad_shafranov_deviation_value': {
            ...
            "full_path": "equilibrium/time_slice(itime)/convergence/grad_shafranov_deviation_value",
        },
        'equilibrium.time_slice[:].constraints.total_chi_squared': {
            "data_type": "FLT_0D",
            "documentation": "A sum of the chi squared values for measurements " "used to constrain the equilibrium.",
            "full_path": "equilibrium/time_slice(itime)/constraints/total_chi_squared",
        },
        'equilibrium.time_slice[:].constraints.chi_squared_all': {
            "data_type": "FLT_0D",
            "documentation": "A sum of all the chi squared values for measurements.",
            "full_path": "equilibrium/time_slice(itime)/constraints/chi_squared_all",
        },
        'equilibrium.time_slice[:].constraints.rotation': {
            "data_type": "STRUCT_ARRAY",
            "documentation": "Set of rotational pressure estimates.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].measured': {
            "data_type": "FLD_0D",
            "documentation": "Measured value.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/measured",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].measured_error_upper': {
            "data_type": "FLD_0D",
            "documentation": "Uncertainty in measured value.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/measured_error_upper",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].reconstructed': {
            "data_type": "FLD_0D",
            "documentation": "Value calculated from the reconstructed equilibrium.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/reconstructed",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].weight': {
            "data_type": "FLD_0D",
            "documentation": "Weight given to the measurement.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/weight",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].chi_squared': {
            "data_type": "FLD_0D",
            "documentation": "Squared error normalized by the standard deviation considered in the minimization process : chi_squared = weight^2 *(reconstructed - measured)^2 / sigma^2, where sigma is the standard deviation of the measurement error.",
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/chi_squared",
        },
        'equilibrium.time_slice[:].constraints.rotation[:].exact': {
            "data_type": "INT_0D",
            ...
            "full_path": "equilibrium/time_slice(itime)/constraints/rotation(ix)/exact",
        },
    }
}
...
class OMFITgeqdsk(SortedDict, OMFITascii):
    ...
    def from_uda_mastu(self, shot=99999, time=0.0, device='MAST', pfx='epm'):
        ...
        _cpasma = client.get(f'/{pfx}/OUTPUT/GLOBALPARAMETERS/PLASMACURRENT', shot)
    ...
    def from_efitpp(self, ncfile=None, shot=99999, time=0.0, device='MAST'):
        ...
        if 'output' not in rootd.groups.keys():
            self.from_efitpp_mastu(ncfile=ncfile, shot=shot, time=time, device=device)
            return self
...
class OMFITmeqdsk(OMFITnc):
    ...
    # Full set of quantities is only available in EFIT-AI (as long as the old version is tested they can't be used here though
    #    signal_info = {  # Calc   measured   weight  uncert short_desc letter
    #        'mag': ['cmpr2', 'expmpi', 'fwtmp2', 'sigmp2', 'magnetics', 'm'],  # Magnetic probes
    #        'ecc': ['cecurr', 'eccurt', 'fwtec', 'sigec', 'E-coil current', 'e'],  # E-coil currents
    #        'fcc': ['ccbrsp', 'fccurt', 'fwtfc', 'sigfc', 'F-coil current', 'f'],  # F-coil currents
    #        'lop': ['csilop', 'silopt', 'fwtsi', 'sigsi', 'flux loops', 'l'],  # Flux loops
    ...
    #        'prw': ['cpresw', 'presw', 'fwtprw', 'sigprw', 'rotation', 'w'],  # Rotational pressure constraint
    #        'xxj': ['aux_calxxj', 'aux_mxxj', 'aux_fwtxxj', 'aux_sigxxj', 'current density', 'j'],  # Current density
    ...
    def residual_normality_test(self, which='mag', is_tmp='some'):
        ...
        :param is_tmp: string
            ...
            'some', 'none', or 'all'
        """
        # EFIT-AI extends the list of available test parameters
        #            Parameter to do test on. Options: ['mag', 'ecc', 'fcc', 'cur', 'lop', 'ref', 'gam', 'pre', 'prw', 'xxj']
    ...
    def combo_rsq_tests(self, is_tmp='some'):
        ...
        descriptions = {
            ...
            #            'alm': 'all magnetics data (probes, flux loops, coils, current)', # EFIT-AI
    ...
    def to_omas(self, ods=None, time_index=0, time_index_efit=0):
        ...
        with omas_environment(ods, cocosio=1):
            ...
            if 'shot' in self:
                ods['dataset_description.data_entry.pulse'] = self['shot']['data'][time_index_efit]
            ...
            if 'sigmp2' in self:
                for i in range(nconstr):
                    constr['bpol_probe'][i]['measured_error_upper'] = self['sigmp2']['data'][time_index_efit, i]
            ...
            if 'chigam' in self:
                ...
                for i in range(nconstr):
                    ...
                    constr['mse_polarisation_angle'][i]['measured_error_upper'] = self['siggam']['data'][time_index_efit, i]
            ...
            if 'sigsi' in self:
                for i in range(nconstr):
                    constr['flux_loop'][i]['measured_error_upper'] = self['sigsi']['data'][time_index_efit, i]
            ...
            if 'cpresw' in self:
                ...
                for i in range(nconstr):
                    constr['rotation'][i]['chi_squared'] = self['saiprw']['data'][time_index_efit, i]
                    constr['rotation'][i]['measured'] = self['presw']['data'][time_index_efit, i]
                    constr['rotation'][i]['reconstructed'] = self['cpresw']['data'][time_index_efit, i]
                    constr['rotation'][i]['exact'] = 0
                    constr['rotation'][i]['measured_error_upper'] = self['sigprw']['data'][time_index_efit, i]
                    constr['rotation'][i]['weight'] = self['fwtprw']['data'][time_index_efit, i]
            ...
            if 'sigec' in self:
                icoil_ai = 0
                for i in range(nconstr):
                    constr['pf_current'][icoil_ai]['measured_error_upper'] = self['sigec']['data'][time_index_efit, i]
                    constr['pf_current'][icoil_ai]['chi_squared'] = self['saiec']['data'][time_index_efit, i]
                    icoil_ai += 1
            ...
            if 'sigfc' in self:
                for i in range(nconstr):
                    constr['pf_current'][icoil_ai]['measured_error_upper'] = self['sigfc']['data'][time_index_efit, i]
                    constr['pf_current'][icoil_ai]['chi_squared'] = self['saifc']['data'][time_index_efit, i]
                    icoil_ai += 1
            ...
            # Only in EFIT-AI version
            if 'sigcur' in self:
                constr['ip.measured_error_upper'] = self['sigcur']['data'][time_index_efit]
                constr['ip.weight'] = self['fwtcur']['data'][time_index_efit]
                constr['ip.reconstructed'] = self['cpasma']['data'][time_index_efit]
                constr['ip.chi_squared'] = self['saiip']['data'][time_index_efit]
            ...
            # Only in EFIT-AI version
            if 'fwtdlc' in self:
                constr['diamagnetic_flux.weight'] = self['fwtdlc']['data'][time_index_efit]
                constr['diamagnetic_flux.reconstructed'] = self['cdflux']['data'][time_index_efit]
                constr['diamagnetic_flux.chi_squared'] = self['chidlc']['data'][time_index_efit]

            conv = ods['equilibrium.time_slice'][time_index]['convergence']
            # Quality metrics
            i = self['cerror']['data'].shape[1]
            conv['iterations_n'] = i
            # The following quantities are not yet in the IMAS standard but are being proposed
            conv['grad_shafranov_deviation_expression'] = 3
            conv['grad_shafranov_deviation_value'] = self['cerror']['data'][time_index_efit, i - 1]
            constr['total_chi_squared'] = self['cchisq']['data'][time_index_efit, i - 1]
            # Only in EFIT-AI version
            if 'chitot' in self:
                constr['chi_squared_all'] = self['chitot']['data'][time_index_efit]
...
class OMFITkeqdsk(OMFITnamelist):
    ...
    def get_weights(self, fitweight, ishot=None):

        if ishot is None:
            ishot = self['IN1']['ISHOT']

        fits = OMFITd3dfitweight(fitweight)
        fitshot = -1
        for shot in fits:
            if shot < ishot:
                fitshot = shot
        for item in fits[fitshot]:

            self['IN1'][item.upper()] = np.array(fits[fitshot][item])
