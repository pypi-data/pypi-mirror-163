...
class ValidityRange(ExtensibleModel):
    ...
    def calendarweeks(self):
        """Calendar weeks within the validity range."""
        return CalendarWeek.weeks_within(self.date_start, self.date_end)
...
class Lesson(ValidityRangeRelatedExtensibleModel, GroupPropertiesMixin, TeacherPropertiesMixin):
    ...
    @property
    def planned_lessonperiods_datetimes(self):
        """Datetimes of past and future Lesson Periods planned to take place"""
        datetimes = []
        for calendarweek in self.validity.calendarweeks:
            for lesson_period in self.lesson_periods.all():
                datetimes.append(
                    {
                        "year": calendarweek.year,
                        "week": calendarweek.week,
                        "datetime_start": lesson_period.period.get_datetime_start(calendarweek),
                        "datetime_end": lesson_period.period.get_datetime_end(calendarweek),
