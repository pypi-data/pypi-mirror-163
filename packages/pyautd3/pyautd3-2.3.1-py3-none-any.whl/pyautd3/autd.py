...
class SOEM:
    ...
    def build(self):
        ...
        LinkSOEM().dll.AUTDLinkSOEM(byref(link), self._ifname.encode('mbcs') if self._ifname is not None else None,
                                    self._dev_num, self._sync0_cycle, self._send_cycle, self._freerun, onlost, self._high_precision)
        return Link(link)
...
class Emulator:
    ...
    def build(self):
        link = c_void_p()
        LinkEmulator().dll.AUTDLinkEmulator(byref(link), self._port)
        return Link(link)
