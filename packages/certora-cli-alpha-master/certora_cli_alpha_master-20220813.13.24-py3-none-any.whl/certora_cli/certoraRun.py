...
from Shared.certoraLogging import setup_log_arguments, DebugLogHandler
...
def run_certora(args: List[str], is_library: bool = False) -> None:
    ...
    try:
        ...
        pretty_args = [f"'{arg.__str__()}'" if ' ' in arg.__str__() else arg.__str__() for arg in args if arg.__str__()
                       not in '--debug']
        cl_args = ' '.join(pretty_args)
...
def __suggest_contract_name(err_msg: str, contract_name: str, all_contract_names: Set[str],
    ...
    err_str += ' \nNote: To specify a contract in a differently-named spec file, you can ' \
