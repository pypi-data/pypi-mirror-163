...
def get_certora_internal_dir() -> Path:
    root = Path("")
    if 'CERTORA_INTERNAL_DIR' in os.environ:
        root = Path(os.environ['CERTORA_INTERNAL_DIR'])
    return root


def reset_certora_internal_dir() -> None:
    build_uuid = f"{datetime.now().strftime('%y_%m_%d_%H_%M_%S')}_{uuid.uuid4().hex.upper()[0:3]}"
    dir = str(Path(CERTORA_INTERNAL_ROOT) / build_uuid)
    dir = ""  # this line will be deleted once the server is updated with support of multiple build directories
    os.environ['CERTORA_INTERNAL_DIR'] = dir


def path_in_certora_internal(path: Path) -> Path:
    root = get_certora_internal_dir()
    return path if (path.parent == root) else root / path


CERTORA_CONFIG_DIR = path_in_certora_internal(Path(".certora_config"))  # folder
CERTORA_SOURCES_DIR = path_in_certora_internal(Path(".certora_sources"))
CERTORA_BUILD_FILE = path_in_certora_internal(Path(".certora_build.json"))
CERTORA_VERIFY_FILE = path_in_certora_internal(Path(".certora_verify.json"))
CERTORA_METADATA_FILE = path_in_certora_internal(Path(".certora_metadata.json"))
RESOURCE_ERRORS_FILE = path_in_certora_internal(Path("resource_errors.json"))
...
def run_local_spec_check(with_typechecking: bool) -> int:
    ...
    if with_typechecking:
        typecheck_cmd = f"java -jar {path_to_typechecker} -b {CERTORA_BUILD_FILE} " \
                        f"-v {CERTORA_VERIFY_FILE} -m {CERTORA_METADATA_FILE}"
    else:
        typecheck_cmd = f"java -jar {path_to_typechecker} -v {CERTORA_VERIFY_FILE} " \
                        f"-m {CERTORA_METADATA_FILE}"
...
class IsDebugging(metaclass=Singleton):
    """
    Holding --debug input argument.
    """
    ...
    @is_debugging.setter
    def is_debugging(self, is_debug: bool) -> None:
        self._is_debugging = is_debug
