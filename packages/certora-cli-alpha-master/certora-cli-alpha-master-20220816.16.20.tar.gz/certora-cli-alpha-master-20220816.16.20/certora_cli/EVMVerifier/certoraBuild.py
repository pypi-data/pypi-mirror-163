...
from Shared.certoraUtils import CERTORA_CONFIG_DIR, CERTORA_BUILD_FILE, CERTORA_VERIFY_FILE, SolcCompilationException, \
    abs_posix_path_relative_to_root_file, convert_path_for_solc_import, Mode, normalize_double_paths
...
from Shared.certoraUtils import read_json_file, replace_file_name, PACKAGE_FILE
from Shared.certoraUtils import get_closest_strings, flatten_set_list, IsDebugging, CERTORA_SOURCES_DIR
...
def get_compiler_collector(file_name: str) -> CompilerCollector:
    """
    returns [CompilerCollector] instance, based on type of the file [file_name]
    :param file_name: name of the file to create [CompilerCollector] for
    """
    if file_name.endswith(".vy"):
        return CompilerCollectorVy("vyper")
    elif file_name.endswith(".sol"):
        ''' this returned instance contains 0.0.0 as solc version.
        The version should be set later in order to be able to use
        some of the functions defined in this [CompilerCollector] '''
        return CompilerCollectorSol()
    else:
...
class FinderGenerator(object):
    def __init__(self, internal_id: int, compiler_collector: CompilerCollector):
...
class SDC:
    ...
    def sources_as_absolute(self) -> Set[Path]:
        return set(map(lambda x: Path(os.path.abspath(x)), self.original_srclist.values()))
...
def generate_full_assembly_finder(f: Func, internal_id: int, sym: int,
        ...
        Optional[str]:
    finder_gen = FinderGenerator(internal_id=internal_id, compiler_collector=compiler_collector)
    to_ret = "assembly { "
...
def generate_modifier_finder(f: Func, internal_id: int, sym: int, compiler_collector: CompilerCollectorSol) -> \
    ...
    finder_gen = FinderGenerator(internal_id, compiler_collector)
...
class CertoraBuildGenerator:
    ...
    def collect_source_type_descriptions(self, contract_file: str, contract_name: str, build_arg_contract_file: str,
        ...
        base_contract_files = [(base_contract_file, base_contract_name) for
                               ...
                               self.retrieve_base_contracts_list(
                                   ...
                                   contract_file,
                                   contract_name, compiler_collector_for_contract_file)]  # type: List[Tuple[str, str]]
    ...
    def collect_funcs(self, data: Dict[str, Any], contract_file: str,
        ...
        def get_function_selector(f_entry: Dict[str, Any], f_name: str,
                                  input_types: List[SolidityType], compiler_collector: CompilerCollector) -> str:
            ...
            assert expected_result == compiler_collector.normalize_func_hash(result), \
        ...
        base_contract_files = self.retrieve_base_contracts_list(
            build_arg_contract_file,
            contract_file,
            contract_name, compiler_collector_for_contract_file)  # type: List[Tuple[str, str, bool]]
        ast_logger.debug(
            f"build arg contract file {build_arg_contract_file} and base contract files {base_contract_files}")
        if not compiler_collector_for_contract_file.smart_contract_lang == CompilerLangVy():
    ...
    def retrieve_base_contracts_list(self, build_arg_contract_file: str, contract_file: str, contract_name: str,
        ...
        :param contract_file:
        :param contract_name:
        :param compiler_collector_for_contract_file
    ...
    def get_contract_def_node_ref(self, build_arg_contract_file: str, contract_file: str, contract_name: str) -> int:
        ...
        node's reference.
        """
        compiler_collector = get_compiler_collector(contract_file)
        contract_file_ast = self.asts[build_arg_contract_file][contract_file]
        ast_logger.debug(f"contract file ast {contract_file_ast}")
        return compiler_collector.get_contract_def_node_ref(contract_file_ast, contract_file, contract_name)
    ...
    def get_standard_json_data(self, sdc_name: str, compiler_collector: CompilerCollector) -> Dict[str, Any]:
        json_file = compiler_collector.get_standard_json_filename(sdc_name, self.config_path)
    ...
    def cleanup_standard_json_data(self, sdc_name: str, compiler_collector: CompilerCollector) -> None:
        json_file = compiler_collector.get_standard_json_filename(sdc_name, self.config_path)
    ...
    def collect_for_file(self,
        ...
        is_vyper = compiler_collector == CompilerLangVy()
        ...
        input_for_solc = self.standard_json(Path(file_abs_path), build_arg_contract_file, remappings,
                                            compiler_collector.smart_contract_lang)
        ...
        standard_json_data = self.get_standard_json_data(sdc_name, compiler_collector)
        # Unless we're in debug mode, we prefer to exclude the stdout file which is potentially huge
        if not IsDebugging().is_debugging:
            self.cleanup_standard_json_data(sdc_name, compiler_collector)
        ...
        while contract_work_list:
            ...
            compiler_collector_for_contract_file = get_compiler_collector(contract_file)
                ...
                for contract_name in contract_list:
                    ...
                    for c_file, _, _ in base_contracts:
                        if c_file not in seen_link_refs:
                            build_logger.debug(f"Adding a base contract link ref {c_file} to worklist")
                    ...
                    for link_ref in link_refs:
                        if link_ref not in seen_link_refs:
                            build_logger.debug(f"Adding link ref {link_ref} to worklist")
                    ...
                    for ref in set(internal_refs):
                        ...
                        contract_files_resolved = [Path(x).resolve() for x in
                                                   contract_refs]
                        ...
                        if len(set(contract_files_resolved)) != 1:
                            ...
                            link_ref = str(contract_files_resolved[0])
                            # we keep the original to handle symlinks
                            resolved_to_orig[link_ref] = contract_refs[0]
                            if link_ref not in seen_link_refs:
                                build_logger.debug(f"Adding internal link ref {link_ref} to worklist")
                                contract_work_list.add(link_ref)
                                seen_link_refs.add(link_ref)
        ...
        sdc_lst_to_return = []
        srclist, report_srclist = self.build_srclist(data, sdc_name, compiler_collector.smart_contract_lang)
    ...
    def get_contract_in_sdc(self,
        ...
        compiler_lang = get_compiler_lang(contract_file)
        ...
        funcs = self.collect_funcs(contract_data, contract_file, contract_name, build_arg_contract_file,
                                   compiler_collector_for_contract_file, types)
        ...
        deployed_bytecode = compiler_collector_for_contract_file.normalize_deployed_bytecode(
            deployed_bytecode)
        ...
        constructor_bytecode = compiler_collector_for_contract_file.normalize_deployed_bytecode(
    ...
    def add_auto_finders(self, contract_file: str, contractsInSDCs: List[ContractInSDC],
        ...
        for c in contractsInSDCs:
            ...
            if len(version_matches) != 1:
                ...
                return None
            compiler_collector.compiler_version = version_matches[0]
            instrumentation_logger.debug(f"Inferred {version_matches[0]} for contract {c.name}")
            for f in c.internal_funcs.union(c.public_funcs):
                ...
                if len(mods) > 0:
                    ...
                    # source code
                    mod_inst = generate_modifier_finder(f, internal_id, function_symbol, compiler_collector)
                    if mod_inst is None:
                    ...
                    finder_res = generate_full_assembly_finder(f, internal_id, function_symbol, compiler_collector)
    ...
    def build(self, args: argparse.Namespace) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            compiler_collector = get_compiler_collector(build_arg_contract_file)
            sdc_pre_finders = self.collect_for_file(build_arg_contract_file, i, compiler_collector)
            ...
            if isinstance(compiler_collector, CompilerCollectorSol):
                added_finders_to_sdc = self.instrument_auto_finders(build_arg_contract_file, i,
                                                                    typing.cast(CompilerCollectorSol,
                                                                                compiler_collector),
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int, compiler_collector: CompilerCollectorSol,
        ...
        try:
            new_sdcs = self.collect_for_file(new_file, i, compiler_collector, None, fail_on_compilation_error=False)
    ...
    def build_source_tree(self) -> None:
        shutil.rmtree(CERTORA_SOURCES_DIR, ignore_errors=True)
        if len(self.all_contract_files) == 0:
            return

        if PACKAGE_FILE.exists():
            self.all_contract_files.add(Path(os.path.abspath(PACKAGE_FILE)))
        common_path = Path(os.path.commonpath([p.resolve() for p in self.all_contract_files]))
        # if common path is a file (common path of a single file is the path of the file) we take the parent
        if common_path.is_file():
            common_path = common_path.parent
        for source_file in self.all_contract_files:
            # copy file to the path of the file from the common root under the sources directory
            target_file = CERTORA_SOURCES_DIR / source_file.relative_to(common_path)
    ...
    def resolve_slot(self, primary_contract: str, slot_name: str) -> str:
        ...
        if slot_result == CertoraBuildGenerator.SlotResolution.SLOT_RESOLVED:
            ...
            msg = f"Link to a var that doesnt exist on the contract." \
                  f" Failed to resolve slot for {slot_name} in {primary_contract}"
            build_logger.critical(msg)
            raise Exception(msg)
        elif slot_result == CertoraBuildGenerator.SlotResolution.SLOT_FOUND_MULTIPLE:
            raise Exception(f"Cannot link, Found multiple matches for {slot_name} "
