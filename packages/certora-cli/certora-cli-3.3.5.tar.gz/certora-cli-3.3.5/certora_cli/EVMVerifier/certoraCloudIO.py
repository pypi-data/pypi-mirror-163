...
from Shared.certoraUtils import DEFAULT_CLOUD_ENV, DEFAULT_STAGING_ENV, PRODUCTION_PACKAGE_NAME
from Shared.certoraUtils import Mode
from Shared.certoraUtils import Singleton
from Shared.certoraLogging import DebugLogHandler
        ...
        if "." in latest and "." in version:
            ...
            if remote_main > local_main:
                if package_name == PRODUCTION_PACKAGE_NAME:
                    raise AttributeError(must_upgrade_msg)
...
class CloudVerification(metaclass=Singleton):
    ...
    def __compress_and_upload_zip_files(self) -> bool:
        ...
        if self.args.mode == Mode.TAC:
            ...
            result = compress_files(self.logZipFileName, DebugLogHandler.CERTORA_DEBUG_LOG_FILE,
                                    short_output=self.args.short_output)
            ...
            result = compress_files(self.ZipFileName, CERTORA_BUILD_FILE, CERTORA_VERIFY_FILE, CERTORA_CONFIG_DIR,
                                    CERTORA_METADATA_FILE, Path(self.logZipFileName),
                                    short_output=self.args.short_output)
