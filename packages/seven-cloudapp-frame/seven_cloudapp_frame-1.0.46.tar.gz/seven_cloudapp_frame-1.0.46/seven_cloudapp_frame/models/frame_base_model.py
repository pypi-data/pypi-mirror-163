...
@LastEditTime: 2022-07-22 16:24:47
...
class FrameBaseModel():
    ...
    def check_act_info(self, act_id):
        """
        :description: 检验活动信息
        :param act_id:活动标识
        :return:invoke_result_data
    ...
    def check_act_module(self, module_id):
        """
        :description: 检验活动模块
    ...
    def business_process_executing(self, app_id, act_id, module_id, user_id, login_token, handler_name, check_new_user=False, check_user_nick=True, continue_request_expire=5, acquire_lock_name="", request_limit_num=0, request_limit_time=1, source_object_id="",check_act_info=True,check_act_module=True,check_user_info=True):
        """
        ...
        if check_act_info == True:
            invoke_result_data = self.check_act_info(act_id)
        ...
        if check_act_module == True:
            invoke_result_data = self.check_act_module(module_id)
            if invoke_result_data.success == False:
                return invoke_result_data
