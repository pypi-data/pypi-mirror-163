...
from typing import Any, Callable, Dict, Generator, List, NamedTuple, Optional, Protocol, Tuple, Type, Union, overload
...
def _delete_jobs(runs: List[PlatformRun]):
...
def _delete_config_maps(runs: List[PlatformRun]):
    # all runs must belong to the same context
    if len(runs) == 0:
        return
    context = runs[0].context
...
def _delete_service(run: PlatformRun):
    context = run.context
    with use_context(context.name):
        core = client.CoreV1Api()
...
def _delete_pod_group(run: PlatformRun):
    context = run.context
    with use_context(context.name):
        if context.namespace is None:
...
def delete_runs(runs: List[PlatformRun], check_only_job: bool = True) -> bool:
    ...
    context_runs: Dict[KubeContext, List[PlatformRun]] = {}
    for run in runs:
    ...
    chunked_run_lists: List[List[PlatformRun]] = []
