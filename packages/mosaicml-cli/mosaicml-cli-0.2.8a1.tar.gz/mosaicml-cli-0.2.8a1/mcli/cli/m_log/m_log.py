...
from mcli.api.kube.runs import delete_runs, get_run_logs, get_runs
...
def get_logs(
    ...
    **kwargs,
    ...
    try:
        ...
        with console.status('Getting run details...') as spinner:
            if run_name is None:
                ...
                runs = get_runs([run_name], future=False)
                if not runs:
        ...
        if run.status == RunStatus.FAILED_PULL:
            ...
            with console.status('Deleting failed run...'):
                delete_runs([run.name], _runs=[run])
            return 1
            ...
            logger.error(f'{FAIL} Run has not yet started. You can check the status with `mcli get runs` '
                         'and try again later.')
            return 1
        for line in get_run_logs(run, rank=rank, follow=follow):
