...
from gias3.fieldwork.field.tools import mesh_fitter
...
def fitBone(scan_number, data, GF, params, align=True, HMF=True, nodal=True, PCF=False, pc=None):
    fit = mesh_fitter.MeshFitter(scan_number)
    ...
    # rigid
    if align:
        fit.align(init_translation=None, sample_data=1000)
...
def main(scan):
    ...
    from gias3.examples.fieldwork.fit_whole_pelvis_data import LH_params as LHParams
    from gias3.examples.fieldwork.fit_whole_pelvis_data import RH_params as RHParams
    from gias3.examples.fieldwork.fit_whole_pelvis_data import sac_params as sacParams
    ...
    if LHParams.PCFile is not None:
        LHPC = PCA.loadPrincipalComponents(LHParams.PCFile)
    else:
        LHPC = None
    LHFit = fitBone(scan, dataLH, LHGF, LHParams, HMF=True, PCF=False, nodal=True, pc=LHPC)
    if save:
    ...
    if RHParams.PCFile is not None:
        RHPC = PCA.loadPrincipalComponents(RHParams.PCFile)
    else:
        RHPC = None
    RHFit = fitBone(scan, dataRH, RHGF, RHParams, HMF=True, PCF=False, nodal=True, pc=RHPC)
    if save:
    ...
    if sacParams.PCFile is not None:
        sacPC = PCA.loadPrincipalComponents(sacParams.PCFile)
    else:
        sacPC = None
    sacFit = fitBone(scan, dataSac, sacGF, sacParams, HMF=True, PCF=False, nodal=True, pc=sacPC)
    if save:
    ...
    if visualise:
        ...
        V.addData('LH', dataLH, render_args={'mode': 'point'})
        V.addData('RH', dataRH, render_args={'mode': 'point'})
        V.addData('Sacrum', dataSac, render_args={'mode': 'point'})
