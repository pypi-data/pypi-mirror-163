...
ss = pd.read_csv(FILE_DIR+'/ss_planets.csv')
...
class NASA_Exoplanet_Archive:
    def __init__(self, data='/exo_list.csv'):
        #BASE_DIR = os.path.dirname(os.path.dirname(__file__))
        self.d = pd.read_csv(FILE_DIR+data, usecols=good_cols)
        ...
        if add_ss_planets:
            try:
                ...
                for p in range(len(xs)):
                    path_holder = planet_names[p]
                    imscatter(xs[p], ys[p], FILE_DIR+'/ss_pics/%s' % path_holder, ax=ax,
                                zoom=planet_sizes[p])
            except:
                pass
            #imscatter(xs[0], ys[0], FILE_DIR+'/ss_pics/mercury.pdf', ax=ax,
            #                zoom=planet_sizes[0])
        # AXIS
