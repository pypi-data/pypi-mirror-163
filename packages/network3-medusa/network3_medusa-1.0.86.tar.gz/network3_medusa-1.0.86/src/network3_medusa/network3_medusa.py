...
class Network3Medusa():
    ...
    def network3_medusa(self):
        ...
        if self.command == "show running-config":
            ...
            it = iter(json_dict)
            size = 6000
            # for i in range(0, len(json_dict), size):
            #     if first:
            #         first = False
            #         payload = yield {k:json_dict[k] for k in islice(it, size)}
            #         self.add_file_to_hedera(payload)
            #     else:
            #         payload = yield {k:json_dict[k] for k in islice(it, size)}
            #         self.append_file_to_hedera(payload)
            # query = FileInfoQuery().setFileId(self.fileId)
            # info = query.execute(self.client)
            # print("File size according to `FileInfoQuery`: ", info.size)            
            # if WEBEX_ROOM:
