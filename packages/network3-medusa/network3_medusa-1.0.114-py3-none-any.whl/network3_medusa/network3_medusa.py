...
from hedera import Client, AccountId, PrivateKey, Hbar, FileCreateTransaction, FileAppendTransaction, FileInfoQuery
...
class Network3Medusa():
    ...
    def network3_medusa(self):
        ...
        if self.command == "show running-config":
            first = True
            ...
            size = 6000
            chunks = [raw_json[i:i+size] for i in range (0, len(raw_json), size)]
            first = True
            for chunk in chunks:
                if first == True:
                    first = False
                    print("first")
                    self.add_file_to_hedera(chunk)            
                else:
                    print("next")
                    self.append_file_to_hedera(chunk)           
            if WEBEX_ROOM:
                self.send_webex_notification()          
    def py2output(self):
        self.output = subprocess.run(["python", "call_cli.py", self.command], capture_output=True)

    def add_file_to_hedera(self, chunk):
