...
class SafeProtocolError(ProtocolError):
    """
    This is a probably hacky way to prevent the XML-RPC URL from being
    "leaked" by way of displaying the raw error text.
    """

    def __init__(self, error):
        # nb. replace original URL with "elided"
        super(SafeProtocolError, self).__init__("(URL ELIDED)",
                                                error.errcode,
                                                error.errmsg,
                                                error.headers)
