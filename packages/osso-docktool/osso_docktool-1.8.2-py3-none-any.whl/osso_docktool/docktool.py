...
def registered_disk_actions(action=None, **kwargs):
    # Python2-compatible forced kwargs. Temporary fix until we clean up this
    # code.
    hdd_dock = kwargs.pop('hdd_dock')
    ...
    if action == 'secure-erase':
        if 'SECURE ERASED' in inventory.current_health_status:
            print(
                'Skipping automatic secure erase, health says it\'s been done')
        else:
            print('Starting automatic secure erase')
            manager.best_erase()
            inventory.flush()
...
def dev_menu(devname, action):
    ...
    registered_disk_actions(
        ...
        author=author, location=location, action=action)
...
class AutoServer:
    def __init__(self, action, gui, ignore_devices):
        self.action = action
        self.gui = gui
        ...
        if not gui:
            if not os.environ.get('TMUX'):
                os.execvp('tmux', [
                    ...
                    f'{self.docktool_exec} auto {action}'])
            else:
                # Add the environment to the tmux session.
                subprocess.check_call(
                    ['tmux', 'set', 'remain-on-exit', 'on'])
                subprocess.check_call(
                    ['tmux', 'setenv', 'EMAIL', author])
                subprocess.check_call(
                    ['tmux', 'setenv', 'LOCATION', location])
    ...
    def listen(self):
        with socket(AF_INET, SOCK_STREAM) as sock:
            ...
            print('Waiting for new disk notifications...')
            while True:
    ...
    def handle_client(self, connection):
        ...
        if client_data:
            device = Path(client_data.decode())
            if device.is_block_device():
                if self.is_ignored(device):
                    print(f'Ignoring notification for {device}')
                else:
                    self.start_docktool(device)
            else:
    ...
    def is_ignored(self, device):
        # Check the resolved path against the ignore list.
        return bool(device.resolve() in self.ignore_devices)

    def start_docktool(self, device):
        if self.gui:
            self.start_gui(device)
        else:
            self.start_cli(device)

    def start_gui(self, device):
        print(f'Starting GUI terminal for {device}')
        # Double fork to detach terminal from pid group.
        if os.fork() == 0:
            args = [
                f'osso-docktool-terminal [{device}]', '-e',
                'sudo', '-E', self.docktool_exec, 'dev', device, '--wait']
    ...
    def start_cli(self, device):
        print(f'Creating new tmux window for {device}')
        if not os.environ.get('TMUX'):
            print('Error: TMUX environment variable is empty', file=sys.stderr)
        else:
            args = ['tmux', 'new-window', '-n', device]
            cmd = (
                f'sudo -E {self.docktool_exec} dev {shlex.quote(str(device))} '
                '--wait')
            if self.action != 'start':
                cmd += f' --action {self.action}'
            subprocess.check_call(args + [cmd])
        os._exit(0)


def auto_notify(device):
    with socket(AF_INET, SOCK_STREAM) as sock:
        try:
            sock.connect(('127.0.0.1', 1451))
            sock.sendall(str(device).encode())
...
class DocktoolArgumentParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        devparser.add_argument(
            '-a', '--action', choices=['secure-erase'],
            help='Automatically execute the action.')
        devparser.add_argument(
        ...
        autoparser.add_argument(
            'action', choices=['start', 'secure-erase'], default='start',
            help='Start docktool or secure erase newly inserted disks')

        notifyparser = self.subparsers.add_parser(
            'auto-notify',
            help='Notify the auto process of the device')
    ...
    def block_device(self, name):
        ...
        if not device.is_absolute():
            ret = self.block_device('/dev' / device)
...
def main():
    ...
    try:
        if args.command == 'dev':
            dev_menu(args.device, args.action)
        elif args.command == 'serial':
            db_menu(args.serial)
        elif args.command == 'auto':
            AutoServer(args.action, args.gui, args.ignore).start()
        elif args.command == 'auto-notify':
            auto_notify(args.device)
    except KeyboardInterrupt:
        sys.exit(0)
    except SystemExit:
