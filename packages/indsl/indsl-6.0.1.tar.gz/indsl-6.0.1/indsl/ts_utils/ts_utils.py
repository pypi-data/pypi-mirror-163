...
def get_fixed_freq(
    ...
    """Create a Timeseriesindex with a fixed frequency.
...
def above_below(series: pd.Series, upper_limit: float, lower_limit: float):
    ...
    Count the number of points above/below upper_limit/lower_limit.
...
def num_vals_in_boxes(series: pd.Series, num_boxes: int):
    """Count the number of points in each of the boxes between 0 and 100 %.
...
def fill_gaps(
    ...
    """Mask nan/fill gaps bases on the time size of missing data.
...
def scalar_to_pandas_series(data: Union[float, pd.Series]) -> pd.Series:
    ...
    if isinstance(data, float):
        # IDEA: Use const_value function instead, but have to avoid circular import
        # Needs at least two values covering the whole time span of interest to work with auto_align
