...
import src.certfr_NVD as certfr_NVD
import src.certfr_scrapper as certfr_scrapper
from src.certfr_sqlite import Sqlite
from src.certfr_version_parser import systems_filter
...
class Router:
    ...
    def alerts_to_database(self, filename, source, text):
        ...
        """
        systems_affectes = certfr_scrapper.systems_parser(text)
        _date = certfr_scrapper.date_parser(text)
        summary = certfr_scrapper.header_summary_parser(text)
        documentation_texte, documentation_liens = certfr_scrapper.documentation_parser(text)

        # NVD score
        CVE = certfr_NVD.checkForCVE(documentation_liens, documentation_texte)
        if self.verbose:
            print("traitement NVD de", filename)
            print(CVE)
        link = "https://services.nvd.nist.gov/rest/json/cve/1.0/" + CVE if CVE != "" else ""
        score = certfr_NVD.getNVDScore(link) if link != "" else 0.0

        # get details link
        details = certfr_scrapper.define_details(score, CVE, documentation_liens)
