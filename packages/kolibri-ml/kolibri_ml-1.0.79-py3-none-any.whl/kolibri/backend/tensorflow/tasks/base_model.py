...
#            'label_indexer': self.label_indexer.to_dict(),
    ...
    def save(self, model_path: str) -> str:
        ...
        with open(os.path.join(full_model_path, 'model_config.json'), 'w') as f:
            info_dict = self.to_dict()
            f.write(json.dumps(info_dict, indent=2, default=str, ensure_ascii=False))
            f.close()

        self.tf_model.save_weights(os.path.join(full_model_path, 'model_weights.h5'))  # type: ignore
        logger.info('model saved to {}'.format(os.path.abspath(full_model_path)))
