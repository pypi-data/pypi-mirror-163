...
#添加pad
def add_pad(job, step, layers, layer, symbol, location_x, location_y, polarity, dcode, orient, attributes):
    if symbol=="":
        return
...
def add_text(job, step, layer, symbol, fontname, text, xsize, ysize, linewidth, location_x, location_y, polarity,
            orient, version, layers, attributes):
    data = {
        'func': 'ADD_TEXT',
        'paras': [{'job': job},
                  {'step': step},
...
# 输出drill
def drill2file(job, step, layer,path,isMetric):
    data = {
        'func': 'DRILL2FILE',
        'paras': {
            'job': job,
