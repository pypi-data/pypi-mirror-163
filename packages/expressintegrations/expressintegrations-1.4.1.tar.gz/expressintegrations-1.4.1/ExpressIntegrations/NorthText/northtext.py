...
class northtext:
  ...
  def send_message(
      ...
      to: list = None,
      drip_campaign: int = None,
      system_number: int = None
  ):
    data = {
        "body": body,
        "attachment": attachment,
        "sendOn": send_on.strftime('%Y-%m-%dT%H:%M:%SZ'),
        "to": to,
  ...
  def get_contacts(self, limit: int = 100, page: int = 0, order: str = 'ASC', phone_number: str = None):
    phone_param = f"&PhoneNumber={phone_number}" if phone_number else ''
    return self.api_call('get', f"/api/v2/contact?Limit={limit}&Page={page}&Order={order}{phone_param}")

  def create_contact(self, **kwargs):
    return self.api_call('post', f"/api/v2/contact", data=kwargs)

  def get_contact(self, contact_id: int):
    return self.api_call('get', f"/api/v2/contact{contact_id}")

  def update_contact(self, contact_id: int, **kwargs):
    return self.api_call('put', f"/api/v2/contact{contact_id}", data=kwargs)
